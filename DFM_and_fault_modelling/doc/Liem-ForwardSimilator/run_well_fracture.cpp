...
int main( int argc, char* argv[] )
{
  ...
  double time_end = 400.*days;
  double time_save =  2.*days;
  double time_step =  2.*days;


  cout << "Run run_well_fracture.cpp with:" << endl;
  ...
  std::string target_measurement_folder= "output_measurement/" + model_name + "/";
  std::string target_output_data_folder= "output_data/"        + model_name + "/";
  if (use_tmp_folder){
  ...
  // Create three rectancular measurement regions in the interior of the domain
  MeasureDataForEnKF::CreateMeasRegion( model, 3 );
  ...
  MeasureDataForEnKF::MeasurePressure(      model,"WELL_FRACTURE", write_measurement_folder );
