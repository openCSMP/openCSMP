...
//
//  CapillaryMembrane_Example.cpp
//  Open CSMP++SAMG
//
//  Created by Anne-Laure Tertois in 2022.
//


// NOT PORTED TO OPEN-CSMP++ YET 

#if 0

/**
     ANLOR bubble migration through a capillary membrane example
*/
void CapillaryMembrane_Example::Run() {
...
bool CapillaryMembrane_Example::ImportModelAndRunChecks(
...
bool CapillaryMembrane_Example::RunChecks( const Model<3U>& model ) const {
  ...
  if( nb_things == 0 ) {
      ...
      for(
          map<string,Region<3U> >::const_iterator
...
=======
void CapillaryMembrane_Example::RunSimulation( Model<3U>& model ) const {
>>>>>>> f1171f99af48ad8765b86abd2ccd22558a98877e
    ...
    */
    ...
    string file_name = model_name + "-input_data";
    ...
    while ( model_time < end_time )
    {
      ...
      if ( model_time >= next_save_time ) {
        ...
        if (DES) {
          string runtime_file_name(model_name + "-DES_runtime_output");
          ...
          string runtime_file_name(model_name + "-TDS_runtime_output");
...
//  template void DES2PhaseFlowWithSplitBoundary_Example::RunSimulation(Model<3U>& model);
  ...
  void CapillaryMembrane_Example::Compute2PhaseFlowProperties(
...
template void CapillaryMembrane_Example::Compute2PhaseFlowProperties( Model<3U>&, FlowFunctionsModule1<3U>&, bool, bool ) const;
...
void CapillaryMembrane_Example::ComputeSteadyStatePressure(
      Model<3U>& mdl, bool with_gravity, bool with_tensor_k
  ) const {
    bool verbose(false);

    if(verbose) {
      cout << "\nCapillaryMembrane_Example::ComputeSteadyStatePressure: Input parameters: " << endl;
...
void computeFV_Diameter_Normal_VerticalExtent( Model<dim>& model, const string& region_name )
 {
...
*/
/*template<uint32_t dim>
void computeSpillPointSaturation( Model<dim>& model, const string& region_name )
 {
    csmp::ErrorHandler& csmp_error( ErrorHandler::Instance() );
    if ( !model.ContainsRegion( region_name ) )
