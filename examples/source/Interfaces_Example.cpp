...
void Interfaces_Example::Specifications()
{
  SetTitle( "Build CSMP native format models using various interfaces");
  SetDifficulty( 1 );
  SetCategory( "Input / Output Interfaces" );
  AddAuthor( "Qi Shao" );
  ...
  AddRequirement( "source files: 'Interfaces_Example.cpp' and '*.h'" );
...
/**
 ...
 - 'bitmaps_and_other/' contains misc files that can serve the creation of CSMP native models
...
*/
void Interfaces_Example::Run()
{
  // checking working directory and illustrating C++ exception handling
  try {
      // set path to current directory 'input_meshes/'
      if(!is_directory("input_meshes")) {
        string message("\nInterfaces_Example::Run(): 'input_meshes/' directory is missing from your current working directory: '");
        message += (current_path()).string();
      ...
      } else current_path("input_meshes");
    }
  catch( const std::exception& ex ) {
      cerr <<"\nInterfaces_Example::Run: caught runtime error: "<< ex.what() << endl;
      throw csmp::Exception( FATAL_ERROR, "Interfaces_Example::Run", "Terminating examples program.");
    }
  ...
  do {
    ...
    //gOcad_SKUA models
      ...
      BuildFromSKUAModel<3U>("SKUA_intersecting_split_boundary_surfaces");
      BuildFromSKUAModel<3U>("SKUA_model");
      BuildFromSKUAModel<3U>("SKUA_split_boundary_layer");
      BuildFromSKUAModel<3U>("SKUA_split_boundary_layer_with_well");
      BuildFromSKUAModel<3U>("SKUA_split_boundary_surface");
      BuildFromSKUAModel<3U>("SKUA_box_snowflake_split_boundaries");

// TODO:      BuildFromEclipseModel("NPD5"); // fails to build NPD5 in ConstructPillarsAndColumns()

// TODO:      BuildFromGeoModellerModel("Mansfield_H8_NoOrphans.mesh"); // fails in MeshManager::Initialise()

// TODO:      BuildFromRhinoModel("example20"); // fails in RhinoSurfaceReader::CreateNeighborPData() line 288 because it uses old VData where bflags was map!
...
/**
   helper function that navigates to input_meshes/ directory to retrieve input files

  @note getting info where we are in the directory tree
  path currrentPath = current_path();
  cout << "exists() = " << exists(currrentPath) << "\n"
     << "root_name() = " << currrentPath.root_name() << "\n"
     << "root_path() = " << currrentPath.root_path() << "\n"
     << "relative_path() = " << currrentPath.relative_path() << "\n"
     << "parent_path() = " << currrentPath.parent_path() << "\n"
     << "filename() = " << currrentPath.filename() << "\n"
...
static bool stayInInputMeshes() {
    if ( current_path().filename() != "input_meshes" ) {
         ...
         if ( exists(current_path()) && is_directory(current_path()) ) {
              // switching into input_meshes/  directory
              const bool directory_existed_before = !create_directory("input_meshes");
              if ( !directory_existed_before )
                throw runtime_error("Interfaces_Example::BuildFromANSYS2DModel: directory 'input_meshes/' did not exist but was now created.");
              current_path("input_meshes");
              cerr <<"\nBuildFromANSYS2DModel: currently in directory: "<< current_path().string();
           }
      }
    return true;
 }
...
void Interfaces_Example::BuildFromANSYS2DModel(const string& model_name) {
  ...
  createAndMoveToNativeBinaryFileDirectory();
...
void Interfaces_Example::BuildFromANSYS3DModel(const string& model_name) {
...
void Interfaces_Example::BuildFromTRIANGLEModel(const string& model_name) {
...
void Interfaces_Example::BuildFromTriangulatorModel(const string& model_name, double extent1, double extent2) {
...
void Interfaces_Example::BuildFromQuadrilateratorModel(const string& model_name, double x, double y) {
...
void Interfaces_Example::BuildFromSKUAModel(const string& model_name) {
...
// neighbor connectivity already contained in SKUA model:  vset.EstablishElementConnectivity3D();
...
template void Interfaces_Example::BuildFromSKUAModel<1U>(const string&);
template void Interfaces_Example::BuildFromSKUAModel<2U>(const string&);
template void Interfaces_Example::BuildFromSKUAModel<3U>(const string&);


void Interfaces_Example::BuildFromEclipseModel(const string& model_name) {
...
void Interfaces_Example::BuildFromGeoModellerModel(const string& model_name) {
...
void Interfaces_Example::BuildFromRhinoModel(const string& model_name) {
...
template<uint32_t dim>
void Interfaces_Example::OutputRegionIDToVTU (Model<dim>& model)
{
...
template void Interfaces_Example::OutputRegionIDToVTU (Model<1U>& model);
template void Interfaces_Example::OutputRegionIDToVTU (Model<2U>& model);
template void Interfaces_Example::OutputRegionIDToVTU (Model<3U>& model);
