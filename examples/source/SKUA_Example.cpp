...
void SKUA_Example::Run()
{
  string input;
  do{
      cout << endl << "Please enter model name, 's' to run super-simple example, 'q' to exit or anything else to list available models: ";
      cin >> input;
      if( input == string( "q" ) || input == string( "Q" ) ) {
          return;
      }
      if( input == string( "s" ) || input == string( "S" )  ) {
          RunSuperSimple();
          return;
      }
      if( !fs::exists( input ) ) {
...
void SKUA_Example::RunSuperSimple() {
  ...
  Model<3U>  model( model_topology, vset, variables_file.c_str(), false );
  PropertyDatabase<3U>& p_ref(model.Database());  // constant reference to the property database
  ...
  right.InputPropertyValue( "fluid pressure", makeScalar(DIRICH,1.0e+07) );
    ...
    const double     max_time(72.0*hour); // run for 3 days
  ...
  cout <<"\nSKUA_Example Super Simple: That's it!\n";
