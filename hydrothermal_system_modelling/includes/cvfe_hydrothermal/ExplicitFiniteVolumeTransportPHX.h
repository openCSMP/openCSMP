...
public:
    ...
    void UpdateProjection( );     // updating projection of "velocity", i.e. pressure gradient, onto facet normals
    ...
    //Benoit add
    void DetermineFacetFluxRatesOnce( std::vector<DenseMatrix<DM_MIN> >& upwind_visitor );
    void RescaleFacetFluxToTimestep( const double& time_increment );
...
protected:
    ...
    // protected: raw (unscaled) per-node outflow, snapshot once per stage so
    // sub-steps can rescale it without rebuilding the stencil
    std::vector<std::vector<double> > raw_flux_out_vectors;
};
