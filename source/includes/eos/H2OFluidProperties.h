...
namespace csmp
{
  ...
  public:
	...
	// PW May 2016 - access functions for convergence speed-up
    bool                  AboveCriticalPressureH2O(double& pressure_external);
    double              BoilingPointAtPressure(double& pressure_external);
    double              VaporEnthalpyVL(double& pressure_external);
    double              LiquidEnthalpyVL(double& pressure_external);

    //Benoit
    double  SaturationPressureFromT(double temperature_C);

    // Rock liquidus/solidus for Grant & Sorey compressibility
