...
namespace csmp {
  ...
  public:
    ...
    double64    VaporProperty(const long& it, const int& property_index); 
    double64    LiquidProperty(const long& it, const int& property_index); 
    double64    VaporProperty(const double64& t, const int& property_index); 
    double64    LiquidProperty(const double64& t, const int& property_index); 
    double64    SinglePhaseProperty(const long& it, const long& ip, const int& property_index);
    double64    SinglePhaseProperty(const double64& t, const double64& p, const int& property_index);
    double64    TfromP(const double64& press);

    // for given p
    double64    SaturationTemperatureFromP( double64 p );
    double64    LiquidDensityFromP( double64 p );
    double64    VaporDensityFromP( double64 p );
    double64    LiquidEnthalpyFromP( double64 p );
    double64    VaporEnthalpyFromP( double64 p );
    double64    LiquidHeatCapacityFromP( double64 p );
    double64    VaporHeatCapacityFromP( double64 p );
    double64    LiquidViscosityFromP( double64 p );
    double64    VaporViscosityFromP( double64 p );
    double64    LiquidCompressibilityFromP( double64 p );
    double64    VaporCompressibilityFromP( double64 p );
    
    // for given t
    double64    SaturationPressureFromT( double64 t );
    double64    DSaturationPressureFromTDT( double64 t );
    double64    LiquidDensityFromT( double64 t );
    double64    VaporDensityFromT( double64 t );
    double64    LiquidEnthalpyFromT( double64 t );
    double64    VaporEnthalpyFromT( double64 t );
    double64    LiquidHeatCapacityFromT( double64 t );
    double64    VaporHeatCapacityFromT( double64 t );
    double64    LiquidViscosityFromT( double64 t );
    double64    VaporViscosityFromT( double64 t );
    double64    LiquidCompressibilityFromT( double64 t );
    double64    VaporCompressibilityFromT( double64 t );
    
    // for given p, t
    double64    Density( double64 t, double64 p );
    double64    HeatCapacity( double64 t, double64 p );
    double64    Enthalpy( double64 t, double64 p );
    double64    Viscosity( double64 t, double64 p );
    double64    Compressibility( double64 t, double64 p );
    
    // TD's additions from Feb 2007
    double64    MolarVolume(double64 t, double64 p);
    double64    LiquidMolarVolumeFromP(double64 p);
    double64    VaporMolarVolumeFromP(double64 p);
    double64    Dvdt(double64 t, double64 p);
    double64    Dvdpbar(double64 t, double64 p);
  ...
  private:
    ...
    double64    DynamicViscosity( const double64& T, const double64& rho )  const;

    long        GetTemperatureIndex(const double64& t);
    long        GetPressureIndex(const double64& p);

    bool        BoilingCurveInInterpolationCell();
    void        GetIndex_iA(const double64& tcurrent, const double64& pcurrent, const int& property_index);
    double64    NormalInterpolation(const double64& tcurrent, const double64& pcurrent, const int& property_index );
    double64    LiquidInterpolationNearBoilingCurve(const double64& t, const double64& p, const int& property_index);
    double64    VaporInterpolationNearBoilingCurve(const double64& t, const double64& p, const int& property_index);
    double64    NearCriticalInterpolation(const double64& tcurrent, const double64& pcurrent , const int& property_index);
    double64    AccidentalBoilingCurveEncounter(const double64& t, const double64& p, const int& property_index);
    ...
    char        filename[6][60];
    double64    tcurrent,pcurrent,pdummy;
    double64    t_res,p_res,T,tboil,myt;
    double64    v_top,v_bottom,v_before,v_behind,v_boil,v_boil_before,v_boil_behind;
    double64    v_iA,v_iB,v_iC,v_iD,v_interpolated;
    double64    pboil,p_boil_low,p_boil_high,t_iA,t_iB,t_iC,t_iD,p_iA,p_iB,p_iC,p_iD,pboil_iB,pboil_iA;
    double64    dvdt;
    double64    ak[4], bij[6][5];


    std::vector<double64>   satvap; 
    std::vector<double64>   satliq;
    std::vector<double64>   singlephase;
