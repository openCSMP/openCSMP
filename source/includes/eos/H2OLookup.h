...
namespace csmp
{
  ...
  class H2OLookup
  {
    public:
      ...
      double    SinglePhaseProperty(const long &it, const long &ip,
                                    const int &property_index);
      double    SinglePhaseProperty(const double &t, const double &p,
                                    const int &property_index);
      ...
      double    alpha( double t, double p); // thermal expansion coefficient

      Fluidproperties UpdatePropertiesSinglePhase (double t, double p);
    ...
    private:
      ...
      void        GetIndex_iA(const double &tcurrent, const double &pcurrent);
      void        GetIndex_iA(const double &tcurrent, const double &pcurrent,
                              const int &property_index);
      double    NormalInterpolation(const double &tcurrent, const double &pcurrent,
                                    const int &property_index );
      void    NormalInterpolation(const double &tcurrent, const double &pcurrent);
      double    LiquidInterpolationNearBoilingCurve(const double &t, const double &p,
                                                    const int &property_index);
      double    VaporInterpolationNearBoilingCurve(const double &t, const double &p,
                                                   const int &property_index);
      double    NearCriticalInterpolation(const double &tcurrent, const double &pcurrent,
                                          const int &property_index);
      double    AccidentalBoilingCurveEncounter(const double &t, const double &p,
                                                const int &property_index);

      long        t_dim, p_dim;
      long        n_tables, table_id;
      long        i, it, ip, iA, iB, iC, iD, i_dummy, i_min, i_max, i_guess;
      long iAA[max_index], iBA[max_index], iCA[max_index], iDA[max_index];
      double vA_interpolated[max_index];
      ...
      double    pboil, p_boil_low, p_boil_high, t_iA, t_iB, t_iC, t_iD, p_iA, p_iB, p_iC,
                p_iD, pboil_iB, pboil_iA;
      double    dvdt;
      double    ak[4], bij[6][5];
      ...
      //std::map<double, std::pair<double,double>> tgrid_map, pgrid_map;
