...
template<uint32_t,template<uint32_t> class> class CVFEM_MathOperatorLHS;
template<uint32_t,template<uint32_t> class> class CVFEM_MathOperatorRHS;
...
class CVFEM_Visitor : public Visitor<dim> {
  public:
    ...
    virtual ~CVFEM_Visitor();
    
    virtual void Visit( Element<dim>* );
    
    void Add( const PropertyDatabase<dim>&,
