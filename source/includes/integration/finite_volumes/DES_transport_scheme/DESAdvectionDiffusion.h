...
class DESAdvectionDiffusion {
  ...
  public:
    DESAdvectionDiffusion ( Model<dim>& m, const char* target_region, double64 cfl_multiplier, double64 PEP_multiplier, bool tensor_k);

    void AdvectVariable_DES( double64 model_time, size_t num_threads=1 );
    void AdvectVariable_TDS( double64 time_interval, size_t num_threads=1 );
  ...
  private:
    ...
    bool Schedule(Event<dim>* event, double64 t_end);
    void Update_DES(Event<dim>* event, double64 t_clock);
    void Update_TDS(Event<dim>* event, double64 delta_t);
    void Synchronize(Event<dim>* event,double64 t_clock,double64& t_remove);
    
    Model<dim>& sg_;
    Region<dim>& gref_;
    PropertyDatabase<dim>& db_;
    double64 upper_limit_, lower_limit_; ///< range in which the result is allowed to vary
    double64 CFL_multiplier_, PEP_multiplier_;
