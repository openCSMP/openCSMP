...
class FV_IntegrationPointsAndWeights {
  public:
    ...
    void FacetIntegrationWeights( std::vector<std::vector<double> >&  facet_integration_weights ) const;

    void SectorIntegrationPoints( std::vector<std::vector<Point<dim> > >&  volume_integration_points ) const;
    void SectorIntegrationWeights( std::vector<std::vector<double> >&  volume_integration_weights ) const;
    
    void ProjectionWeights( std::vector<std::vector<double> >& projection_weights ) const;
    
    void FacetNormals( std::vector<Point<dim> >& facet_normals ) const;

    void FacetNormalTransformations( std::vector<std::vector<std::pair<double,double>>>& facet_normal_xforms ) const;

    void FacetsSurroundingNode( std::vector<std::vector<uint32_t> >&  facets_surrounding_node ) const;
  ...
  private:
    FV_IntegrationPointsAndWeights();
    void Resize( uint32_t n_isrf, uint32_t srfs_per_node, uint32_t n_ivol, uint32_t n_spts, uint32_t n_vpts );
    ...
    std::vector<std::vector<double> >          m_facet_integration_weights;     ///< [isrf][spts]
    std::vector< Point<dim> >                  m_facet_normals;                 ///< [isrf][dim]
    std::vector<std::vector<std::pair<double,double>>> m_facet_normal_xforms;   ///< [isrf][node]
    ...
    std::vector<std::vector<std::pair<uint32_t,uint32_t> > > m_edge_of_sectors; ///< edges of FV sector on outside of the finite element
