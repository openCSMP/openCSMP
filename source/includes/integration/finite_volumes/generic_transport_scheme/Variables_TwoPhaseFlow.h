#ifndef CSMP_VARIABLES_TWO_PHASE_FLOW_H
#define CSMP_VARIABLES_TWO_PHASE_FLOW_H

/**
@attention DUMMY TEMPORARY FILE THAT IS ONLY THERE TO FACILITATE COMPILATION, @todo replace with data from new spreadsheet (SKM, Feb 18,2018)
...
struct Variables_TwoPhaseFlow {
  ...
  // multiphase flow, SKM added by hand
  csmp::INDEX<SCALAR,ELEMENT> swr_key; // irreducible saturation wetting phase
  csmp::INDEX<SCALAR,ELEMENT> snr_key; // residual saturation non-wetting phase
  csmp::INDEX<SCALAR,ELEMENT> sw_key;  // water saturation
  csmp::INDEX<SCALAR,ELEMENT> sn_key;  // non-wetting phase saturation
  csmp::INDEX<SCALAR,ELEMENT> pd_key;  // capillary entry pressure (Pa)
  csmp::INDEX<SCALAR,ELEMENT> bcp_key; // Brooks-Corey parameter
...
#endif // CSMP_VARIABLES_TWO_PHASE_FLOW_H
