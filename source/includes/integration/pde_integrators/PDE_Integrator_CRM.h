...
/**
@class PDE_Integrator_CRM PDE_Integrator_CRM "main_library/PDE_Integrator_CRM.h"
...
The PDE_Integrator_CRM class is a base class from which specific PDE_Integrators can
be inherited to test improvements of its functionality.


@section applicability Applicability

PDE_Integrator_CRM objects can be used to carry out any 2D and 3D computation which
...
Each PDE_Integrator_CRM contains maps of pointers to left and righthand operators
...
passes the PDE_Integrator_CRM on to a group if its application has been restricted
to Regions. Inside the Model, the following steps take place when an
PDE_Integrator_CRM is applied by calling Model::Apply():


First, the Model checks whether the PDE_Integrator_CRM must be handed down to
a specific group. If not, the PDE_Integrator_CRM is allowed to build or update its
vectors and matrices:

@code
PDE_Integrator_CRM::EstablishMatrixSetup( mesh, phys_vars );
@endcode

Now the PDE_Integrator_CRM accumulates the contributions to the global solution
matrices seqentially:

@code
PDE_Integrator_CRM::Accumulate ( mesh, property_collection );
@endcode
...
@code
PDE_Integrator_CRM::AssignInitialConditions( mesh, property_collection );
@endcode
...
@code
PDE_Integrator_CRM::AssignEssentialConditions( mesh, property_collection );
@endcode
...
@code
PDE_Integrator_CRM::LateAccumulate( mesh, property_collection );
@endcode

Now the solver is invoked to invert the solution matrix:

@code
PDE_Integrator_CRM::Solve();
@endcode

The results from the computation are mapped back into property storage:

@code
PDE_Integrator_CRM::OutputResults( mesh, property_collection );
@endcode

and post-processing operations are applied:

@code
PDE_Integrator_CRM::PostProcess( mesh, property_collection );
@endcode
...
The simplemost computation with an PDE_Integrator_CRM is a 3D steady-state
...
PDE_Integrator_CRM<2U>  fluid_pressure;
...
@code
PDE_Integrator_CRM<2U>  deformation;
