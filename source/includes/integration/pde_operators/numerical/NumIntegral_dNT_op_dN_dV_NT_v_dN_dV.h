...
#ifndef NumIntegral_dNT_op_dN_dV_NT_v_dN_dV_h
#define NumIntegral_dNT_op_dN_dV_NT_v_dN_dV_h
...
class NumIntegral_dNT_op_dN_dV_NT_v_dN_dV : public MathOperatorLHS<dim,CELL> {
  public:
    NumIntegral_dNT_op_dN_dV_NT_v_dN_dV( const PropertyDatabase<dim>&,
  ...
  private:
    void ReadElementMultiplier( const CELL<dim>&, DenseMatrix<dim>& MULT );
    
    DenseMatrix<DM_MIN>                DN, DNT; // derivatives of basis functions
    DenseMatrix<dim>                   VIP,     // 'v' (velocity) vector<double> variable
                                       EMULT;   // element based multiplier for 'v'
    std::vector<double>                IPOL;    // basis function values (at integration point)
    std::vector<DenseMatrix<DM_MIN> >  NT3;     // IPOL(nodes) at integration points (ips) stored in columns of matrix NTNTNT
