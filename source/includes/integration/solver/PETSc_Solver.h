...
//TODO: Check the solution (add exeptions) whether the solution is out of range (for now there is no information about the solution variable range, maybe should be done outside of the solver class).
...
// -----------------------------------------------------------------------------
// KSP CONVERGENCE / DIVERGENCE REASONS
// -----------------------------------------------------------------------------

inline constexpr std::array kKnownKSPReasons{
    // Converged
    ErrorInfo{1,   "KSP_CONVERGED_RTOL_NORMAL_EQUATIONS",   "Requested decrease in the residual of the normal equations"},
    ErrorInfo{2,   "KSP_CONVERGED_RTOL",                  "Requested decrease in the residual"},
    ErrorInfo{3,   "KSP_CONVERGED_ATOL",                  "Requested absolute value in the residual"},
    ErrorInfo{4,   "KSP_CONVERGED_ITS",                   "Requested number of iterations"},
    ErrorInfo{5,   "KSP_CONVERGED_NEG_CURVE",              "Negative curvature was detected"},
    ErrorInfo{6,   "KSP_CONVERGED_STEP_LENGTH",            "Converged because of the computed step length"},
    ErrorInfo{7,   "KSP_CONVERGED_HAPPY_BREAKDOWN",        "Happy breakdown: early convergence of the Krylov method occurred"},
    ErrorInfo{8,   "KSP_CONVERGED_USER",                  "The user indicated convergence for an arbitrary reason"},
    ErrorInfo{9,   "KSP_CONVERGED_ATOL_NORMAL_EQUATIONS",   "Requested absolute value in the residual of the normal equations"},

    // Diverged
    ErrorInfo{-2,  "KSP_DIVERGED_NULL",                   "Breakdown while solving the Hessenberg system within KSPGMRES"},
    ErrorInfo{-3,  "KSP_DIVERGED_ITS",                    "Requested number of iterations was reached without convergence"},
    ErrorInfo{-4,  "KSP_DIVERGED_DTOL",                   "Large increase in the residual norm indicates that the solution is diverging"},
    ErrorInfo{-5,  "KSP_DIVERGED_BREAKDOWN",              "Breakdown in the Krylov method"},
    ErrorInfo{-6,  "KSP_DIVERGED_BREAKDOWN_BICG",          "Breakdown in the KSPBCGS Krylov method"},
    ErrorInfo{-7,  "KSP_DIVERGED_NONSYMMETRIC",            "The operator or preconditioner was not symmetric for a KSP method that requires symmetry"},
    ErrorInfo{-8,  "KSP_DIVERGED_INDEFINITE_PC",           "The preconditioner was indefinite for a KSP method that requires a definite preconditioner"},
    ErrorInfo{-9,  "KSP_DIVERGED_NANORINF",               "NaN or infinity was detected in a vector during the computation"},
    ErrorInfo{-10, "KSP_DIVERGED_INDEFINITE_MAT",          "The operator was indefinite for a KSP method that requires a definite operator"},
    ErrorInfo{-11, "KSP_DIVERGED_PC_FAILED",               "The action of the preconditioner failed"},
    ErrorInfo{-12, "KSP_DIVERGED_USER",                   "The user indicated divergence for an arbitrary reason"},

    // Still running
    ErrorInfo{0,   "KSP_CONVERGED_ITERATING",              "The Krylov solver is still iterating"}
};

inline constexpr ErrorInfo kUnknownKSPReason{
    0,  "KSP_REASON_UNKNOWN",   "Unknown or version-specific KSP convergence reason"
};


template <std::size_t N>
[[nodiscard]] constexpr ErrorInfo describeCode(
    int code,
    const std::array<ErrorInfo, N>& table,
    ErrorInfo unknown) noexcept
{
...
[[nodiscard]] constexpr ErrorInfo describeKSPReason(int code) noexcept
{
    return describeCode(
        code,
        kKnownKSPReasons,
...
inline void printInfo(const ErrorInfo& info)
{
    // std::println(
    //     "{} ({}) — {}",
    //     info.symbol,
    ...
    //     info.description
    // );
    std::cout << info.symbol << " (" << info.code << ") — " << info.description << "\n"; // to make it compatible with GCC<14
}
...
/** Usage
    ...
    petsc::printError(-99);
    // PETSC_ERR_UNKNOWN (-99) — Unknown, non-standard, or version-specific PETSc error code
 */
inline void printError(int code)
{
    printInfo(describe(code));
}


inline void printKSPReason(int code)
{
    printInfo(describeKSPReason(code));
}
