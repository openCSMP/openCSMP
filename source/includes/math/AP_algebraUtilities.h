...
double64 dotProduct( const std::vector<double64>&, const std::vector<double64>& );
...
double64 normOfVector( const std::vector<double64>& );

template<size_t dim>
void vertexCenterOfMass( const std::vector< Point<dim> >&, Point<dim>& );


void vertexCenterOfMass3Vertices(const std::vector<double64>&, 
...
void euclideanNormalize(std::vector<double64>&);
...
inline void euclideanNormalize( std::vector<double64>& vectorToNormalize )
{
	...
	if(fNorm != 0)
	  std::transform(vectorToNormalize.begin(), vectorToNormalize.end(), vectorToNormalize.begin(), 
	                 std::bind2nd(std::divides<double64>(), fNorm)); 
}
