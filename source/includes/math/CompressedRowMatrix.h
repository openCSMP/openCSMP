...
/**
    ...
    @section Sparsity Pattern Generation and Split Boundaries

    The sparsity pattern generated by generateSparsityPattern is derived exclusively from the cell connectivity of the supplied ModelSubDomain. It is therefore essential that the domain passed to this function contains all mesh entities whose degrees of freedom must be coupled in the global system matrix.

    @section Split Boundaries and NodeManifolds

    A SplitBoundary is a node-matching internal contact between two otherwise independent mesh patches. In CSMP, such contacts are represented by NodeManifold objects, each of which groups the collocated nodes from the two patches into a single topological entity.

    The presence of one or more NodeManifolds in a region is a reliable indicator that the region contains a SplitBoundary. At a SplitBoundary, the weak form of the governing PDE introduces coupling terms between degrees of freedom on opposite sides of the contact. These cross-patch connections must be present in the sparsity pattern before matrix assembly begins; they cannot be inserted afterwards without a full rebuild of the pattern.

    @section Containment Condition

    Whether the sparsity pattern correctly captures SplitBoundary coupling depends on the following containment condition:

    All nodes referenced by the NodeManifolds of a region must themselves be contained within that region.

    Condition satisfied: Both sides of every SplitBoundary are present in the domain. The cell loop in generateSparsityPattern will visit cells from both patches, and ActiveEquationIndices will expose the cross-patch DOF pairs naturally. The resulting sparsity pattern is complete.
    Condition not satisfied: One or more NodeManifold nodes lie outside the computational domain. Those degrees of freedom do not participate in the weak form over this domain and must be excluded from the sparsity pattern. Allocating matrix entries for absent DOFs would waste memory and introduce uninitialised columns into the system matrix.
    
    @section Practical Consequence
    
    CSMP always defines a non-unique region called "Model" that encompasses the entire mesh, including all SplitBoundaries. Because the Model region contains all mesh entities by definition, the containment condition is always satisfied when generateSparsityPattern is called with the Model region as its computational domain. Unique (non-overlapping) sub-regions that do not span a SplitBoundary are unaffected, as they may contain NodeManifolds only along their boundaries where connected outside nodes should be ignored.

    @section Summary

    NodeManifolds present in region?
            │
            ├─ No  → Standard cell-local sparsity pattern is complete.
            │
            └─ Yes → Are all NodeManifold nodes contained in the region?
                            │
                            ├─ Yes → Cross-patch DOF pairs are captured
                            │        by the cell loop. Pattern is complete.
                            │
                            └─ No  → Outside nodes are not part of this
                                     computational domain. Exclude them.
                                     Pattern is complete for this domain.

    @attention This function is general in that it can be applied to poly-element type meshes where node connections will exist in the matrix even if element nodes are not connected directly (e.g., hexahedron, prism, quadratic elements etc.)
