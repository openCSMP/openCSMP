...
inline size_t SparseMatrix::Entries() const 
 { 
    return entries; 
 }

/// iterator over the matrix rows
inline std::vector<std::map<size_t,double64> >::const_iterator  SparseMatrix::Begin() const
 {
    return data.begin();
 }
...
inline std::vector<std::map<size_t,double64> >::const_iterator  SparseMatrix::End() const
 {
    return data.end();
 }


inline std::map<size_t,double64>::const_iterator  SparseMatrix::RowBegin( size_t i ) const
 {
    return data[i].begin();
 }


inline std::map<size_t,double64>::const_iterator  SparseMatrix::RowEnd( size_t i ) const
 {
    return data[i].end();
 }

inline size_t SparseMatrix::Rows() const
  {
     return data.size();
  }


inline size_t SparseMatrix::Cols() const
  {
     return data.size();
  }


inline double64 SparseMatrix::operator()( size_t i, size_t j ) const
 {
    if ( i >= data.size() ) {
...
inline double64 SparseMatrix::At( size_t i, size_t j ) const
 {
    return (*this)(i,j);
 }
