...
class VectorVariable<3U> {
  public:
    ...
    VectorVariable( VARIABLE_FLAG f1, VARIABLE_FLAG f2, VARIABLE_FLAG f3, 
                    double64 val1, double64 val2, double64 val3 );  // 3D constructor
    ...
    // this avoids creation of temporary variable
    ...
    Point<3U>       P() const; // NB: else name clash in GNU
    ...
    // projections
    double64        DotProduct( const csmp::Point<3U>& p ) const;
    double64        DotProduct( const VectorVariable& v ) const;
    VectorVariable  CrossProduct( const csmp::Point<3U>& p ) const;
    ...
    // data assignment and modification
