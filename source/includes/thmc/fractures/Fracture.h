...
public:
    ...
    void                            SetToDC_Tips();
    void                            SetToHydraulicAsymptoteTips();
    ...
    //Querying
    void                                PullNodesApartBy( Node<dim>* n_ptr, double aperture );
    double                            FractureLength(INTERFACE_SIDE side) ;
    double                            FluidLag( BOX_BOUNDARY side_of_tip, const char* pressure) ;
    double                            DistanceFromTip(std::map<Point<dim>,Node<dim>*>&, Node<dim>* N, BOX_BOUNDARY side = RIGHT);
    double                            MaxAperture(const char* aperture = "aperture", INTERFACE_SIDE side = MIDDLE);
    ...
    //void TestPropertyInitialization();                          //Method causes memory error within IndexTracker because of PropList()
