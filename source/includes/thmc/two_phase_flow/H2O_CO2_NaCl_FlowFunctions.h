...
class H2O_CO2_NaCl_FlowFunctions {
  public:
    ...
    bool InterpolateAqueousPhase( Element<dim>* const, double& sw, double& rhow, double& muw ) const;

    /// returns carbonic phase properties at element barycentre, if there is a value at at least a single node, else returns false
    bool InterpolateCarbonicPhase( Element<dim>* const, double& snw, double& rhon, double& mun ) const;
    ...
    /// lambda overbar: mobility product l_overbar = (li * rhow * lj * rhonw) / (li*rhow + lj*rhonw),  also known as G
    double MobilityProduct_at( Element<dim>* const, double sw ) const;
    ...
    double CapillaryDiffusionMultiplier_at( Element<dim>* const, double sw ) const;
    
    double Dpcds_at( Element<dim>* const, double sw ) const;

    // TODO: used when? double CapillaryDiffusionMultiplier_Phase( Element<dim>* const, size_t phase ) const;
  ...
  private:
    ...
    double InterpolateAqueousPhaseSaturation( const Element<dim>* const ) const;
  
    /// interpolates saturations of all mobile phases to barycenter and returns their sum which should be 1 - halite_saturation
    double InterpolateSaturations( Element<dim>* const, double& sw, double& snw ) const;
  ...
  private:
    mutable double sw_,   ///< saturation aqueous phase
