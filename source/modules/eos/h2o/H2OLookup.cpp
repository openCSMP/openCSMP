...
namespace csmp
{
  ...
  H2OLookup::H2OLookup()
  {
    //New version of GetTemperatureIndex and GetPressureIndex:
    //not used, as gives no obvious speedup. Maybe can be used in the future, as code is much cleaner that way.
    //@author Alina Yapparova, Jan 2017
    //here initialize tgrid_map and pgrid_map;
    /*
       double tvals[] = {0.,250.,350.,360.,370.,372.,378.,380.,390.,400.,450.,550.,1000.,2000.};
       double tres[]  = {5.,2.  ,1.  ,0.5 ,0.2 ,0.1 ,0.2 ,0.5 ,1.  ,2.  ,5.  ,10. ,50., 0.};
       double tshift[]= {0.,50. ,100.,110.,130.,140.,200.,210.,230.,240.,265.,285.,330., 0.};

       std::pair<double,double> grid_pair;

       for (int i=0;i<14;i++)
       {
         grid_pair = std::make_pair(tres[i],tshift[i]);
         tgrid_map.insert(std::pair<double, std::pair<double,double>>(tvals[i],grid_pair));
       }

       double pvals[] = {0., 20.e5, 210.e5, 215.e5, 225.e5, 230.e5, 250.e5, 300.e5, 400.e5, 700.e5, 1000.e5, 2000.e5, 5000.e5, 10000.e5};
       double pres[] = {0.5e5,  1.e5,  0.5e5,  0.1e5,  0.5e5,   1.e5,   2.e5,   5.e5,  10.e5,  25.e5,   50.e5,  100.e5,  500.e5, 0.};
       double pshift[] = { 0., 40., 230., 240., 340., 350., 370., 395., 415., 445.,  457.,  477.,  507., 0.};

       for (int i=0;i<14;i++)
       {
         grid_pair = std::make_pair(pres[i],pshift[i]);
         pgrid_map.insert(std::pair<double, std::pair<double,double>>(pvals[i],grid_pair));
       }
    */
    //end
  ...
  double H2OLookup::LiquidDensityFromP( double p )
  {
    myt = TfromP(p);
    return LiquidProperty(myt, density_index) ;
  }
  double H2OLookup::LiquidEnthalpyFromP( double p )
  {
    myt = TfromP(p);
    return LiquidProperty(myt, enthalpy_index) ;
  }
  double H2OLookup::LiquidHeatCapacityFromP( double p )
  {
    myt = TfromP(p);
    return LiquidProperty(myt, heatcapacity_index) ;
  }
  double H2OLookup::LiquidViscosityFromP( double p )
  {
    myt = TfromP(p);
    return LiquidProperty(myt, viscosity_index) ;
  }
  double H2OLookup::LiquidCompressibilityFromP( double p )
  {
    myt = TfromP(p);
    return LiquidProperty(myt, compressibility_index);
  }
  double H2OLookup::VaporDensityFromP( double p )
  {
    myt = TfromP(p);
    return VaporProperty(myt, density_index);
  }
  double H2OLookup::VaporEnthalpyFromP( double p )
  {
    myt = TfromP(p);
    return VaporProperty(myt, enthalpy_index);
  }
  double H2OLookup::VaporHeatCapacityFromP( double p )
  {
    myt = TfromP(p);
    return VaporProperty(myt, heatcapacity_index) ;
  }
  double H2OLookup::VaporViscosityFromP( double p )
  {
    myt = TfromP(p);
    return VaporProperty(myt, viscosity_index);
  }
  double H2OLookup::VaporCompressibilityFromP( double p )
  {
    myt = TfromP(p);
    return VaporProperty(myt, compressibility_index);
  }
  double H2OLookup::SaturationPressureFromT( double t )
  {
    return LiquidProperty(t, pressure_index);
  }
  double H2OLookup::DSaturationPressureFromTDT( double t )
  {
    if (t <= 5.0)
      return (SaturationPressureFromT(t + 0.01) - SaturationPressureFromT(t)) / 0.01;
    else
      return (SaturationPressureFromT(t + 0.01) - SaturationPressureFromT(
                t)) / 0.01;
  }
  double H2OLookup::LiquidDensityFromT( double t )
  {
    return LiquidProperty(t, density_index);
  }
  double H2OLookup::LiquidEnthalpyFromT( double t )
  {
    return LiquidProperty(t, enthalpy_index);
  }
  double H2OLookup::LiquidHeatCapacityFromT( double t )
  {
    return LiquidProperty(t, heatcapacity_index);
  }
  double H2OLookup::LiquidViscosityFromT( double t )
  {
    return LiquidProperty(t, viscosity_index);
  }
  double H2OLookup::LiquidCompressibilityFromT( double t )
  {
    return LiquidProperty(t, compressibility_index);
  }
  double H2OLookup::VaporDensityFromT( double t )
  {
    return VaporProperty(t,  density_index);
  }
  double H2OLookup::VaporEnthalpyFromT( double t )
  {
    return VaporProperty(t,  enthalpy_index);
  }
  double H2OLookup::VaporHeatCapacityFromT( double t )
  {
    return VaporProperty(t,  heatcapacity_index);
  }
  double H2OLookup::VaporViscosityFromT( double t )
  {
    return VaporProperty(t,  viscosity_index);
  }
  double H2OLookup::VaporCompressibilityFromT( double t )
  {
    return VaporProperty(t,  compressibility_index);
  }
  double H2OLookup::Density( double t, double p )
  {
    return SinglePhaseProperty(t, p, density_index);
  }
  double H2OLookup::HeatCapacity( double t, double p )
  {
    return SinglePhaseProperty(t, p, heatcapacity_index);
  }
  double H2OLookup::Enthalpy( double t, double p )
  {
    return SinglePhaseProperty(t, p, enthalpy_index);
  }
  double H2OLookup::Viscosity( double t, double p )
  {
    return SinglePhaseProperty(t, p, viscosity_index);
  }
  double H2OLookup::Compressibility( double t, double p )
  {
    return SinglePhaseProperty(t, p, compressibility_index);
  }
  double H2OLookup::MolarVolume( double t, double p)
  {
    return DensityAndX2MolarVolume(SinglePhaseProperty(t, p, density_index), 0.0);
  }
  double H2OLookup::LiquidMolarVolumeFromP( double p)
  {
    myt = TfromP(p);
    return DensityAndX2MolarVolume(LiquidProperty(myt, density_index), 0.0);
  }
  double H2OLookup::VaporMolarVolumeFromP( double p)
  {
    myt = TfromP(p);
    return DensityAndX2MolarVolume(VaporProperty(myt, density_index), 0.0);
  }
  double H2OLookup::Dvdt( double t, double p)
  {
    if (t > cp_h2o.Temperature())
      dvdt = (MolarVolume(t + 0.01, p) - MolarVolume(t, p)) / 0.01;
    else
      {
        if (p > SaturationPressureFromT(t))
          {
            if (t < 1.0e0)
              ...
              dvdt = (MolarVolume(t, p) - MolarVolume(t - 0.01, p)) / 0.01;
          }
        else
          dvdt = (MolarVolume(t + 0.01, p) - MolarVolume(t, p)) / 0.01;
      }
  ...
  double H2OLookup::alpha( double t, double p) // thermal expansion coefficient
  {
    if (t > cp_h2o.Temperature())
      dvdt = (MolarVolume(t + 0.01, p) - MolarVolume(t, p)) / 0.01;
    else
    ...
    return 2.*dvdt / (MolarVolume(t + 0.01, p) + MolarVolume(t, p));
  }

  double H2OLookup::Dvdpbar( double t, double p)
  {
    return -18015.0e0 * Compressibility(t, p) / Density(t, p) * 1.0e5;
  }
  ...
  double H2OLookup::LiquidProperty(const long &it, const int &property_index)
  {
    return satliq[t_dim_table[0] * property_index + it];
  }

  double H2OLookup::VaporProperty(const long &it, const int &property_index)
  {
    return satvap[t_dim_table[1] * property_index + it];
  }

  double H2OLookup::SinglePhaseProperty(const long &it, const long &ip,
  ...
  double H2OLookup::LiquidProperty(const double &t, const int &property_index)
  {
    ...
    tnorm /= (satliq[t_dim_table[0] * temperature_index + it + 1] - satliq[t_dim_table[0] *
                                                                           temperature_index + it]); //TD: added brackets, April 24, 2016!!!
    return satliq[t_dim_table[0] * property_index + it] + tnorm * (satliq[t_dim_table[0] *
                                                                          property_index + it + 1] - satliq[t_dim_table[0] * property_index + it]);
  }
  ...
  double H2OLookup::VaporProperty(const double &t, const int &property_index)
  {
    ...
    tnorm /= (satvap[t_dim_table[0] * temperature_index + it + 1] - satvap[t_dim_table[0] *
                                                                           temperature_index + it]);//TD: added brackets, April 24, 2016!!!
    return satvap[t_dim_table[0] * property_index + it] + tnorm * (satvap[t_dim_table[0] *
                                                                          property_index + it + 1] - satvap[t_dim_table[0] * property_index + it]);
  }

  double H2OLookup::SinglePhaseProperty(const double &t, const double &p,
                                        const int &property_index)
  {
    ...
    if (t >= 374.0e0)
      return NormalInterpolation(t, p, property_index);
        ...
        if (BoilingCurveInInterpolationCell() == true)
          {
            ...
            //cout<<"\n pboil_old = "<<pboil<<" p_old = "<<p<<endl;
            if (p > pboil)
              return LiquidInterpolationNearBoilingCurve(t, p, property_index);
            else if (p < pboil)
              return VaporInterpolationNearBoilingCurve(t, p, property_index);
            else
              return AccidentalBoilingCurveEncounter(t, p, property_index);
          }
        else
          return NormalInterpolation(t, p, property_index);
      }
  ...
  Fluidproperties H2OLookup::UpdatePropertiesSinglePhase (double t,
                                                          double p) //single phase
  {
    ...
    // first, check for extreme temperature
    if ( t >= 2000.0e0 || p >= 10000.e5)
      {
        csmp_err.Note( WARNING,
                       "H2OLookup::SinglePhaseProperty(const double& t, const double& p, const int& property_index) -",
                       "temperature > 2000C or pressure > 1000MPa, i.e., out of valid range.");
        cout << t << "\t" << p << endl;
        //cin >> yesno;
        it = GetTemperatureIndex(t);
        ip = GetPressureIndex(p);

        for (int property_index = density_index; property_index < max_index; property_index++)
          vA_interpolated[property_index] = singlephase[t_dim_table[2] * p_dim * property_index +
                                                        p_dim * it + ip];
      }

    // both temperature and pressure checks passed, now do normal interpolation
    if ( (t >= 373.9e0) && (t < 374.0e0) && (p >= 220.5e5) && (p < 220.6e5))
      {
        for (int property_index = density_index; property_index < max_index; property_index++)
          {
            GetIndex_iA(t, p, property_index);
            vA_interpolated[property_index] = NearCriticalInterpolation(t, p, property_index);
          }
      }
    else
      {
            ...
            if (p > pboil)
              {
                for (int property_index = density_index; property_index < max_index; property_index++)
                  {
                    GetIndex_iA(t, p, property_index);
                    vA_interpolated[property_index] = LiquidInterpolationNearBoilingCurve(t, p,
                                                                                          property_index);
                  }
              }
            else if (p < pboil)
              {
                for (int property_index = density_index; property_index < max_index; property_index++)
                  {
                    GetIndex_iA(t, p, property_index);
                    vA_interpolated[property_index] = VaporInterpolationNearBoilingCurve(t, p,
                                                                                         property_index);
                  }
              }
            else
              // this function only returns an error message, so doesn't matter which property_index to pass to it
              AccidentalBoilingCurveEncounter(t, p, density_index);
          }
        else
          {
            GetIndex_iA(t, p);
            NormalInterpolation(t, p);
          }
      }

    bulk.rho  = vA_interpolated[density_index];
    bulk.h    = vA_interpolated[enthalpy_index];
    bulk.cp   = vA_interpolated[heatcapacity_index];
    bulk.beta = vA_interpolated[compressibility_index];
    bulk.mu   = vA_interpolated[viscosity_index];

    //cout<<"\n UpdatePropertiesSinglePhase t = "<<t<<" p = "<<p<<endl;
    return bulk;
  }


  // some auxilliary functions
  double H2OLookup::AccidentalBoilingCurveEncounter(const double &t, const double &p,
                                                    const int &property_index)
  {
    ...
    cout << "\nt was " << t << endl;
    cout << "\np was " << p << endl;
    throw csmp::Exception( FATAL_ERROR, "H2OLookup::AccidentalBoilingCurveEncounter",
                           "missed to find interpolation type in case 2, returning bogus value." );
    //    long myinput;
    //    cin >> myinput;
    return 9.e99;
  }
  ...
  bool H2OLookup::BoilingCurveInInterpolationCell()
  {
    ...
    if ( (p_boil_low <= p_iD ) && (p_boil_high >= p_iB))
      return true;
    else
      return false;
  }

  void H2OLookup::GetIndex_iA(const double &tcurrent, const double &pcurrent,
                              const int &property_index)
  {
  ...
  void H2OLookup::GetIndex_iA(const double &tcurrent, const double &pcurrent)
  {
    ...
    for (int property_index = density_index; property_index < max_index; property_index++)
      {
        iAA[property_index]  = t_dim_table[2] * p_dim * property_index + it * p_dim + ip;
        iBA[property_index]  = iAA[property_index] + p_dim;
        iCA[property_index]  = iBA[property_index] + 1;
        iDA[property_index]  = iAA[property_index] + 1;
      }
  ...
  double H2OLookup::TfromP(const double &press)
  {
    ...
    while ( i_max - i_guess > 1)
      {
        ...
        else if (press < satliq[i_guess + t_dim_table[0]*pressure_index])
          {
            i_max   = i_guess;
    ...
    pnorm = ( press - satliq[i_guess + t_dim_table[0] * pressure_index] ) /
            (satliq[i_guess + 1 + t_dim_table[0] * pressure_index] - satliq[i_guess + t_dim_table[0] *
                                                                            pressure_index]);
    return satliq[i_guess] + pnorm * (satliq[i_guess + 1] - satliq[i_guess]);
  }


  double H2OLookup::NormalInterpolation( const double &tcurrent,
                                         const double &pcurrent, const int &property_index )
  {
  ...
  void H2OLookup::NormalInterpolation( const double &tcurrent, const double &pcurrent )
  {
    //d cout << "Using H2OLookup::NormalInterpolation( const int& property_index ) ...\n";
    // iA etc. must be known before this is called!
    //   P
    ...
    for (int i = density_index; i < max_index; i++)
      {
        v_bottom       =  singlephase[iAA[i]] + tnorm * (singlephase[iBA[i]] -
                                                         singlephase[iAA[i]]);
        v_top          =  singlephase[iDA[i]] + tnorm * (singlephase[iCA[i]] -
                                                         singlephase[iDA[i]]);
        vA_interpolated[i] =  v_bottom + pnorm * (v_top - v_bottom);
      }
  }

  double H2OLookup::NearCriticalInterpolation(const double &tcurrent,
                                              const double &pcurrent, const int &property_index)
  {
        ...
        if (tcurrent >= cp_h2o.Temperature())
          {
            ...
            v_top    = SinglePhaseProperty(it_A, ip_D, property_index)
                       + tnorm * (SinglePhaseProperty(it_B, ip_D, property_index) - SinglePhaseProperty(it_A,
                                  ip_D, property_index));
            v_behind = SinglePhaseProperty(it_B, ip_A,
                                           property_index) + (pcurrent - 220.5e5) / 0.1e5 *
                       (SinglePhaseProperty(it_B, ip_D, property_index) - SinglePhaseProperty(it_B, ip_A,
                           property_index));
            it       = GetTemperatureIndex(374.001);
            v_interpolated = LiquidProperty(it, property_index) + pnorm * (v_top - LiquidProperty(it,
                                                                           property_index));
        ...
        else
          {
            ...
            v_top    = SinglePhaseProperty(it_A, ip_D, property_index)
                       + tnorm * (SinglePhaseProperty(it_B, ip_D, property_index) - SinglePhaseProperty(it_A,
                                  ip_D, property_index));
            v_before = SinglePhaseProperty(it_A, ip_A,
                                           property_index) + (pcurrent - 220.5e5) / 0.1e5 *
                       (SinglePhaseProperty(it_A, ip_D, property_index) - SinglePhaseProperty(it_A, ip_A,
                           property_index));
            it       = GetTemperatureIndex(374.001);
            v_behind = LiquidProperty(it, property_index) + pnorm * (v_top - LiquidProperty(it,
                                                                                            property_index));
        ...
        if (tcurrent >= cp_h2o.Temperature())
          {
            ...
            v_bottom = VaporProperty(tboil, property_index)
                       + tnorm * (SinglePhaseProperty(it_B, ip_A, property_index) - VaporProperty(tboil,
                                  property_index));
            v_behind = SinglePhaseProperty(it_B, ip_A,
                                           property_index) + (pcurrent - 220.5e5) / 0.1e5 *
                       (SinglePhaseProperty(it_B, ip_D, property_index) - SinglePhaseProperty(it_B, ip_A,
        ...
        else
          {
            if (pcurrent > LiquidProperty(tcurrent, pressure_index))
              ...
              return AccidentalBoilingCurveEncounter(tcurrent, pcurrent, property_index);
          }
      }
  ...
  double H2OLookup::LiquidInterpolationNearBoilingCurve(const double &tcurrent,
                                                        const double &pcurrent, const int &property_index)
  {
  ...
  double H2OLookup::VaporInterpolationNearBoilingCurve(const double &tcurrent,
                                                       const double &pcurrent, const int &property_index )
  {
  ...
  void H2OLookup::BuildTable0And1()
  {
    ...
    for (tcurrent = 0.0e0; tcurrent < cp_h2o.Temperature(); tcurrent += t_res)
      {
        ...
        if (tcurrent == 0.0e0)
          sat_t(T + 0.011, liqprops,
                vapprops); // insurance against non-convergence for derived properties below triple point (0.01 C)

        pcurrent = 0.5 * (liqprops->p +
                          vapprops->p); // *** PROST CAUTION liqprops->p != vapprops->p !!! ***
        ...
        vap_vector[t_dim_table[0] * compressibility_index + it] = 1.0e0 / vapprops->d /
                                                                  (vapprops->dp->d_CT); //?
        vap_vector[t_dim_table[0] * viscosity_index       + it] = DynamicViscosity(T,
                                                                                   vapprops->d);
        ...
        if (tcurrent > 373.85)
          {
            ...
            vap_vector[t_dim_table[0] * compressibility_index + it] = 1.0e0 / properties->d /
                                                                      (properties->dp->d_CT); //?
          }
        ...
        liq_vector[t_dim_table[0] * compressibility_index + it] = 1.0e0 / liqprops->d /
                                                                  (liqprops->dp->d_CT); //?
        liq_vector[t_dim_table[0] * viscosity_index       + it] = DynamicViscosity(T,
                                                                                   liqprops->d);
      }
    ...
    vap_vector[t_dim_table[0] * enthalpy_index + it] = cp_h2o.Enthalpy();
    ...
    vap_vector[t_dim_table[0] * heatcapacity_index + it] =
      0.1; // since infinity is not wanted
    vap_vector[t_dim_table[0] * compressibility_index + it] = 0.1;
    vap_vector[t_dim_table[0] * viscosity_index + it] = DynamicViscosity(
                                                          cp_h2o.Temperature() + 273.15, cp_h2o.Density());
    ...
    liq_vector[t_dim_table[0] * enthalpy_index + it] = cp_h2o.Enthalpy();
    //  liq_vector[t_dim_table[0] * heatcapacity_index + it] = 1.0e10;
    //    liq_vector[t_dim_table[0] * compressibility_index + it] = 1.0e10;
    liq_vector[t_dim_table[0] * heatcapacity_index + it] = 0.1;
    liq_vector[t_dim_table[0] * compressibility_index + it] = 0.1;
    liq_vector[t_dim_table[0] * viscosity_index + it] = DynamicViscosity(
                                                          cp_h2o.Temperature() + 273.15, cp_h2o.Density());
        ...
        for (pcurrent = 0.0e0; pcurrent <= 10000.1e5; pcurrent += p_res)
          {
            ...
            */
            ...
            singlephase_vector[t_dim_table[2]*p_dim * temperature_index     + p_dim * it + ip] =
              tcurrent;
            singlephase_vector[t_dim_table[2]*p_dim * pressure_index        + p_dim * it + ip] =
              pcurrent;
            singlephase_vector[t_dim_table[2]*p_dim * composition_index     + p_dim * it + ip] =
              0.0e0;

            if (tcurrent < cp_h2o.Temperature())
              {
                // PW April 2016: changed from bar to MPa
                //                reg = region_tp(T, pcurrent*0.1, &dl, &dv, &MLiq, &MPro);
                reg = region_tp(T, pcurrent * 1.0e-6, &dl, &dv, &MLiq, &MPro);

                if (reg == 4)
                  {
                    if (pcurrent < psat)
                      {
                        singlephase_vector[t_dim_table[2]*p_dim * density_index         + p_dim * it + ip] =
                          vapprops->d;
                        singlephase_vector[t_dim_table[2]*p_dim * enthalpy_index        + p_dim * it + ip] =
                          vapprops->h;
                        singlephase_vector[t_dim_table[2]*p_dim * heatcapacity_index    + p_dim * it + ip] =
                          vapprops->cp;
                        singlephase_vector[t_dim_table[2]*p_dim * compressibility_index + p_dim * it + ip] = 1.0e0
                            / vapprops->d / (vapprops->dp->d_CT);
                        ...
                        singlephase_vector[t_dim_table[2]*p_dim * viscosity_index       + p_dim * it + ip] =
                          DynamicViscosity(T, vapprops->d);
                      }
                    else
                      {
                        singlephase_vector[t_dim_table[2]*p_dim * density_index         + p_dim * it + ip] =
                        ...
                        singlephase_vector[t_dim_table[2]*p_dim * heatcapacity_index    + p_dim * it + ip] =
                          liqprops->cp;
                        singlephase_vector[t_dim_table[2]*p_dim * compressibility_index + p_dim * it + ip] = 1.0e0
                            / liqprops->d / (liqprops->dp->d_CT);
                        //           cout << "liquid compressibility: " << 1.0e0/liqprops->d/(liqprops->dp->d_CT) << endl;
                        singlephase_vector[t_dim_table[2]*p_dim * viscosity_index       + p_dim * it + ip] =
                          DynamicViscosity(T, liqprops->d);
                      }
                  }
                else
                  {
                    if (pcurrent < psat)
                      water_tp(T, pcurrent, 200.0e0, 1.0e-8, properties);
                    else
                      water_tp(T, pcurrent, 500.0e0, 1.0e-8, properties);
  ...
  long H2OLookup::GetTemperatureIndex(const double &t)
  {
    ...
    if (     t <    0.0e0)
      {
        cout << "\nH2OLookup::GetTemperatureIndex(const double& t) : t < 0 (t = " << t <<
             "), better terminate ...\n";
        throw csmp::Exception(FATAL_ERROR, "H2OLookup::GetTemperatureIndex(const double& t)",
                              "temperature < 0. Not good");
      }
    else if (t <= 250.0e0)
      {
        t_res =  5.0;
        it =     static_cast<long>( (t -   0.0) / t_res );
      }
    else if (t <= 350.0e0)
      {
        t_res =  2.0;
        it =  50 + static_cast<long>( (t - 250.0) / t_res );
      }
    else if (t <= 360.0e0)
      {
        t_res =  1.0;
        it = 100 + static_cast<long>( (t - 350.0) / t_res );
      }
    else if (t <= 370.0e0)
      {
        t_res =  0.5;
        it = 110 + static_cast<long>( (t - 360.0) / t_res );
      }
    else if (t <= 372.0e0)
      {
        t_res =  0.2;
        it = 130 + static_cast<long>( (t - 370.0) / t_res );
      }
    else if (t <= 378.0e0)
      {
        t_res =  0.1;
        it = 140 + static_cast<long>( (t - 372.0) / t_res );
      }
    else if (t <= 380.0e0)
      {
        t_res =  0.2;
        it = 200 + static_cast<long>( (t - 378.0) / t_res );
      }
    else if (t <= 390.0e0)
      {
        t_res =  0.5;
        it = 210 + static_cast<long>( (t - 380.0) / t_res );
      }
    else if (t <= 400.0e0)
      {
        t_res =  1.0;
        it = 230 + static_cast<long>( (t - 390.0) / t_res );
      }
    else if (t <= 450.0e0)
      {
        t_res =  2.0;
        it = 240 + static_cast<long>( (t - 400.0) / t_res );
      }
    else if (t <= 550.0e0)
      {
        t_res =  5.0;
        it = 265 + static_cast<long>( (t - 450.0) / t_res );
      }
    else if (t <= 1000.0e0)
      {
        t_res = 10.0;
        it = 285 + static_cast<long>( (t - 550.0) / t_res );
      }
    else if (t <= 2000.0e0)
      {
        t_res = 50.0;
        it = 330 + static_cast<long>( (t - 1000.0) / t_res );
      }
    else
      {
        it = 350;
      }; //                  t_res = 10.0; it = 285+static_cast<long>( (t-550.0)/t_res ); /* throw out of range ? */ }

    // t_dim is therefore (2000-1000)/50+330 + 1 = 351
    return it;

    //New version of GetTemperatureIndex and GetPressureIndex:
    //not used, no obvious speedup. Maybe can be used in the future, as code is much cleaner that way.
    //@author Alina Yapparova, Jan 2017
    /*
    std::map<double, std::pair<double,double>>::iterator low;

    if (t>2000.)
      return 350;
    else {
      low = tgrid_map.lower_bound (t);
      --low;
      //it =        tshift           + (t -   tval    )/  tres
      it = (long)(low->second.second + (t - low->first)/low->second.first);
      return it;
    }*/
  }
  ...
  long H2OLookup::GetPressureIndex(const double &p)
  {
    ...
    */
    if (     p <=    20.0e5)
      {
        p_res =   0.5e5;
        ip =     static_cast<long>( (p         ) / p_res );
      }
    else if (p <=   210.0e5)
      {
        p_res =   1.0e5;
        ip =  40 + static_cast<long>( (p -  20.0e5) / p_res );
      }
    else if (p <=   215.0e5)
      {
        p_res =   0.5e5;
        ip = 230 + static_cast<long>( (p - 210.0e5) / p_res );
      }
    else if (p <=   225.0e5)
      {
        p_res =   0.1e5;
        ip = 240 + static_cast<long>( (p - 215.0e5) / p_res );
      }
    else if (p <=   230.0e5)
      {
        p_res =   0.5e5;
        ip = 340 + static_cast<long>( (p - 225.0e5) / p_res );
      }
    else if (p <=   250.0e5)
      {
        p_res =   1.0e5;
        ip = 350 + static_cast<long>( (p - 230.0e5) / p_res );
      }
    else if (p <=   300.0e5)
      {
        p_res =   2.0e5;
        ip = 370 + static_cast<long>( (p - 250.0e5) / p_res );
      }
    else if (p <=   400.0e5)
      {
        p_res =   5.0e5;
        ip = 395 + static_cast<long>( (p - 300.0e5) / p_res );
      }
    else if (p <=   700.0e5)
      {
        p_res =  10.0e5;
        ip = 415 + static_cast<long>( (p - 400.0e5) / p_res );
      }
    else if (p <=  1000.0e5)
      {
        p_res =  25.0e5;
        ip = 445 + static_cast<long>( (p - 700.0e5) / p_res );
      }
    else if (p <=  2000.0e5)
      {
        p_res =  50.0e5;
        ip = 457 + static_cast<long>( (p - 1000.0e5) / p_res );
      }
    else if (p <=  5000.0e5)
      {
        p_res = 100.0e5;
        ip = 477 + static_cast<long>( (p - 2000.0e5) / p_res );
      }
    else if (p <= 10000.0e5)
      {
        p_res = 500.0e5;
        ip = 507 + static_cast<long>( (p - 5000.0e5) / p_res );
      }
    else
      {
        ip = 517;
      }//                  p_res = 100.0e5; ip = 476+static_cast<long>( (p-2000.0e5)/p_res );/* throw out of range ? */ }

    // p_dim is therefore (10000e5-5000e5)/500e5+506 + 1 = 518
    return ip;

    //New version of GetTemperatureIndex and GetPressureIndex:
    //not used, as no obvious speedup. Maybe can be used in the future, as code is much cleaner that way.
    //@author Alina Yapparova, Jan 2017
    //std::map<double, std::pair<double,double>>::iterator low;
    ...
    //  return 517;
    //else {
    //  low = pgrid_map.lower_bound (p);
    //  --low;
    //  //ip =        pshift           + (p -   pval    )/  pres
    //  ip = (long)(low->second.second + (p - low->first)/low->second.first);
    //  return ip;
    //}
