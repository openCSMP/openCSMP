    ...
    if( !infile.is_open() )
      {
        ...
        if (!outfile.is_open())
          {
            //cerr << "could not open it for writing, please stop program and debug !!!!\n";
            //char yesno;
            csmp_error.Note(FATAL_ERROR,"CriticalCurveLookup(const double& externaltemperature)","could not open it for writing, please stop program and debug !"," ");
            //cout << "";
            //cin  >> yesno;
          }
        else
  ...
  void CriticalCurveLookup::GetTemperatureIndexCriticalCurve(const double& t)
  {
    ...
    if(!definitelyLessThan(    t, cp_h2o.Temperature(), numeric_limits<double>::epsilon() ) &&
    ...
    else
      {
        if( definitelyLessThan (t, cp_h2o.Temperature(),numeric_limits<double>::epsilon() ) )
          {
			  // PW July 2016 - temporarily downgraded to warning - needs to be fixed
...
//				  "temperature is less than critical temperature for H2O and therefore out of range, terminating ...\n");
              ...
              cerr <<endl<< "CriticalCurveLookup: t=" << t
                   << " < T_crit=" << cp_h2o.Temperature()
                   << "  delta=" << (t - cp_h2o.Temperature())
                   << "  called from temperature=" << temperature
                   << endl;

                  csmp_error.Note(WARNING,
                      "CriticalCurveLookup::GetTemperatureIndexCriticalCurve(const double& t) -",
					  "temperature is less than critical temperature for H2O and therefore out of range ...\n");
				  GetTemperatureIndex(t);
				it -= it_min;
				return;
		}
