...
double H2ONaClFluidProperties::BulkEnthalpy()
{
    ...
    if(definitelyLessThan( xcurrent, 1.0*numeric_limits<double>::epsilon() ) )
    //if( essentiallyEqual( xcurrent, 0.0, numeric_limits<double>::epsilon() ) )
    {
        ...
        if( definitelyLessThan(tcurrent, tp_nacl.Temperature() ))
        {
            if( definitelyGreaterThan( pcurrent, vlh_v.Pmax() ) )
            {
                    ...
                    )
                {
                    ...
                    if (!definitelyGreaterThan(XNaCl2Massfraction(xcurrent), twophase_v.MassFractionNaCl()))
                    {
                        UpdateEnthalpyV();
                        return bulk.h;
                    }
                    ...
                    else if (definitelyLessThan(XNaCl2Massfraction(xcurrent), twophase_l.MassFractionNaCl()))
                    {
                        UpdateEnthalpyVL();
                        return bulk.h;
                    }
                    ...
                    else if (!definitelyGreaterThan(XNaCl2Massfraction(xcurrent), liquidus.MassFractionNaCl()))
                    {
                        UpdateEnthalpyL();
                        return bulk.h;
                    }
                ...
                else
                {
                    ...
                    if (!definitelyGreaterThan(XNaCl2Massfraction(xcurrent), liquidus.MassFractionNaCl()))
                    {
                        UpdateEnthalpyF();
                        return bulk.h;
                    }
            ...
            else if ( essentiallyEqual( pcurrent, vlh_v.Pmax() ) )
            {
                ...
                if( essentiallyEqual( tcurrent, vlh_v.Tmax() ) )
                {
                    ...
                    if (!definitelyGreaterThan(XNaCl2Massfraction(xcurrent), vlh_v.MassFractionNaCl()))
                    {
                        UpdateEnthalpyV();
                        return bulk.h;
                    }
            ...
            else
            {
                ...
                if( definitelyGreaterThan( tcurrent, t_vlh_high ) )
                {
                    ...
                    if (definitelyLessThan(XNaCl2Massfraction(xcurrent), twophase_l.MassFractionNaCl()))
                    {
                        UpdateEnthalpyVL();
                        return bulk.h;
                    }
                    ...
                    )
                {
                    ...
                    if (!definitelyGreaterThan(XNaCl2Massfraction(xcurrent), naclsatvap.MassFractionNaCl()))
                    {
                        UpdateEnthalpyV();
                        return bulk.h;
                    }
                ...
                else
                {
                    ...
                    // *** DEBUG info: double check the following conditions !(...)
                             ...
                             definitelyLessThan(Weight2XNaCl(100.0*twophase_l.MassFractionNaCl()), twophase_l.MinXResolution())
                             // &&
    ...
    throw csmp::Exception( FATAL_ERROR, "H2ONaClFluidProperties::BulkEnthalpy",
                          "No branch matched for the current P-T-X state" );
}
...
void H2ONaClFluidProperties::UpdateEnthalpyVH(){
    ...
    vap.smf = naclsatvap.MassFractionNaCl();
...
void H2ONaClFluidProperties::UpdateEnthalpyLH(){
    ...
    liq.smf = liquidus.MassFractionNaCl();
...
void H2ONaClFluidProperties::UpdateProperties()
{
    ...
    //if( essentiallyEqual( xcurrent, 0.0, numeric_limits<double>::epsilon() ) )
    {
        //cout << "H2ONaClFluidProperties::UpdateProperties() - pure water case\n";
        bulk               = fluid_water.BulkPropertiesFromTHP();
...
void H2ONaClFluidProperties::UpdatePropertiesAtVLH_For_P(double enthalpy){
    ...
    vap.smf          = vlh_v.MassFractionNaCl();
    vap.wt           = 100.0*vap.smf;
    vap.x            = Weight2XNaCl(vap.wt);
    ...
    liq.smf          = vlh_l.MassFractionNaCl();
    ...
    cptot           *= phi;
    if(with_rock_liquidus_solidus) //BenoitLC add boolean
        cptot           += (1.0-phi)*rock.HeatCapacity(tcurrent, tl, ts)*rr;
    else
        cptot           += (1.0-phi)*rock.HeatCapacity(tcurrent)*rr;

    // check PW 22.3.2016
    //    double dpdt   = vlh_l.DPressureDT()*1.0e5;
    double dpdt = vlh_l.DPressureDT();
...
double H2ONaClFluidProperties::TwophaseCompressibility()
{
    ...
    if(with_rock_liquidus_solidus) //BenoitLC add boolean
        b  = ( 1.0 - phi ) * rock.HeatCapacity(tcurrent, tl, ts) * rr;
    else
...
double H2ONaClFluidProperties::VLH_Pmax()
{
    return vlh_v.Pmax();
}

double H2ONaClFluidProperties::VLH_Tmax()
{
    return vlh_v.Tmax();
}

double H2ONaClFluidProperties::VLH_T_low(double& pressure_extrernal)
{
    return vlh_v.TfromP(pressure_extrernal, 100.0);
}

double H2ONaClFluidProperties::VLH_T_high(double& pressure_extrernal)
{
    return vlh_v.TfromP(pressure_extrernal, 700.0);
}
// end PW May 2016 added functions

void H2ONaClFluidProperties::WithRockLiquidusSolidus(bool with_rock_liquidus_solidus_)
{
    with_rock_liquidus_solidus = with_rock_liquidus_solidus_;
    fluid_water.WithRockLiquidusSolidus(with_rock_liquidus_solidus_);
}

void H2ONaClFluidProperties::SetRockLiquidusSolidusTemperatures(double tl_, double ts_)
{
    tl = tl_;
    ts = ts_;
    fluid_water.SetRockLiquidusSolidusTemperatures(tl_, ts_);
}

void H2ONaClFluidProperties::SetRockHeatCapacity( double mini_cp )
{
    rock.SetRockHeatCapacity( mini_cp );
    fluid_water.SetRockHeatCapacity( mini_cp );
}

void H2ONaClFluidProperties::SetRockCrystallizationCurve(double nu_coefficient, double sigma1_coefficient,
                                                         double latent_heat_of_fusion, double b_coefficient, std::string crystallization_curve_)
{
    ...
    fluid_water.SetRockCrystallizationCurve(nu_coefficient, sigma1_coefficient,
                                            latent_heat_of_fusion, b_coefficient, crystallization_curve_);
}

double H2ONaClFluidProperties::SaturationPressureFromT(double temperature_C)
{
    // Benoit add for calculation of humid air inflow at boundary
    // Used for converting relative humidity of air to a vapour mass fraction in air
    return water.SaturationPressureFromT(temperature_C);
}

double H2ONaClFluidProperties::SaturationVaporEnthalpyFromT(double temperature_C)
{
    // Benoit add for calculation of humid air inflow at boundary
    // Vapor enthalpy on the saturation curve at temperature_C.
    // Goes via p_sat → VaporEnthalpyFromP to stay on the VL curve,
    const double p_sat = water.SaturationPressureFromT(temperature_C);

    return water.VaporEnthalpyFromP(p_sat);
}

double H2ONaClFluidProperties::SaturationLiquidEnthalpyFromT(double temperature_C)
{
    // Benoit add
    const double p_sat = water.SaturationPressureFromT(temperature_C);
    return water.LiquidEnthalpyFromP(p_sat);
