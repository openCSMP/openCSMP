...
namespace csmp
{
  ...
  VLH_VaporLookup::VLH_VaporLookup(const double64& externaltemperature)
      ...
      csmp_error( ErrorHandler::Instance() )
  {
    ...
    // what follows is supposed to be a solid check for NaN (http://bytes.com/topic/c/answers/588254-how-check-double64-inf-nan)
  ...
  double64 VLH_VaporLookup::DPressureDT()
  {
  ...
  double64 VLH_VaporLookup::DEnthalpyDT()
  {
    // May be in error near Tmax, Pmax!!!
    GetTemperatureIndex(tcurrent);
    if(it == t_dim-1)
  ...
  double64 VLH_VaporLookup::TfromP(const double64& press, const double64& t_estimate)
  {
    ...
    if( essentiallyEqual( press, pmax, numeric_limits<double64>::epsilon() ) )
      return tmax;
             ...
             &&
             !definitelyLessThan( press, storage_vector[it_p_max+1+tdim_times_pindex], numeric_limits<double64>::epsilon() ) )
      {
        ...
        return  tmax + pnorm * ( storage_vector[it_p_max+1]-tmax );
             ...
             &&
             !definitelyLessThan( press, storage_vector[it_p_max+tdim_times_pindex], numeric_limits<double64>::epsilon() ) )
      // we are in the lookup cell that contains pmax at t<tmax. If so, compute directly and return
      //
      //       |                 |              ( /--  represents VLH curve in T-P space )
  ...
  double64 VLH_VaporLookup::Temperature(){     return ValueOf(temperature_index); }
  double64 VLH_VaporLookup::Pressure(){        return ValueOf(pressure_index); }
  double64 VLH_VaporLookup::MassFractionNaCl(){     return ValueOf(composition_index); }
  double64 VLH_VaporLookup::Density(){         return ValueOf(density_index); }
  double64 VLH_VaporLookup::Enthalpy(){        return ValueOf(enthalpy_index); }
  double64 VLH_VaporLookup::HeatCapacity(){    return ValueOf(heatcapacity_index); }
  double64 VLH_VaporLookup::Compressibility(){ return ValueOf(compressibility_index); }
  double64 VLH_VaporLookup::Viscosity(){       return ValueOf(viscosity_index); }
    
    
  double64 VLH_VaporLookup::ValueOf(const int& property_index)
  {
  ...
  // data indexing
  void VLH_VaporLookup::GetTemperatureIndex(const double64& t)
  {
    // new version
