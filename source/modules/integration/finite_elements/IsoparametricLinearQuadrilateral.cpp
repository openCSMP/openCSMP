...
IsoparametricLinearQuadrilateral::IsoparametricLinearQuadrilateral( uint32_t dimensions )
  // CSMP_FEM_TYPE, isoparametric(y/n), uses_local_coordinates(y/n), order_of_shape_functions
  : FiniteElement( ISOPARAMETRIC_LINEAR_QUADRILATERAL, true, true, 1U ),
...
*/
void
IsoparametricLinearQuadrilateral::N_AtIntegrationPoint( uint32_t ip, vector<double>& N )
 {
    assert( ip < gpe );
...
vector<uint32_t>  IsoparametricLinearQuadrilateral::CornerNodesOfFace( uint32_t face_id ) const
 {
		switch (face_id) {
        case 0: return vector<uint32_t>{0,1};
        case 1: return vector<uint32_t>{1,2};
...
vector<uint32_t>  IsoparametricLinearQuadrilateral::NodesConnectedTo( uint32_t node_id ) const
  {
		switch ( node_id ) {
...
void
IsoparametricLinearQuadrilateral::NodesOfSegment( uint32_t segm_id, vector<uint32_t>& snids ) const
 {
    snids.resize(2);
    if ( segm_id == 0 ) {
         snids[0] = 0;
    ...
    cerr <<"\nIsoparametricLinearQuadrilateral::NodesOfSegment: Erratic segment id requested: "<< segm_id << endl;
...
*/
void
IsoparametricLinearQuadrilateral::CornerNodes( vector<uint32_t>& ids ) const
 {
    ids.resize(4);
    ids[0] = 0;
    ids[1] = 1;
...
*/
void  IsoparametricLinearQuadrilateral::MidSideNodes( vector<uint32_t>& ids ) const
 {
    ids.resize(1);
    ids[0] = 0;
...
void  IsoparametricLinearQuadrilateral::CounterClockwiseNodes( vector<uint32_t>& ids ) const
 {
    ids.resize(npe);
    ids[0] = 0;
    ids[1] = 1;
...
CSMP_FEM_TYPE
IsoparametricLinearQuadrilateral::ElementTypeOfFace( uint32_t )  const
 {
...
double IsoparametricLinearQuadrilateral::WeightAtIntegrationPoint( uint32_t i ) const { return W[i]; }

void  IsoparametricLinearQuadrilateral::N_AtBaryCenter( vector<double>& N )
 {
    N.resize(npe);
    Nrs(0.0,0.0,N);
 }
...
IsoparametricLinearQuadrilateral::AspectRatio()
{
   ...
   for ( auto i{0U}; i<spe; i++ ) segms.insert( vec[i] );
...
*/
void
IsoparametricLinearQuadrilateral::EdgeLengths( vector<double>& len )
{
    double sum=0.0;
    len.resize(spe);
     ...
     if ( use2Dto3Djacobi )
       {
          ...
          for ( uint32_t i{0U}; i<npe; i++ ) {
                ...
                DN4(0,i)  = det_inverse * JMAT(0,0) * ( EFG[2] * DNR[i] - EFG[1] * DNS[i] );
                DN4(0,i) += det_inverse * JMAT(1,0) * ( EFG[0] * DNS[i] - EFG[1] * DNR[i] );
                DN4(1,i)  = det_inverse * JMAT(0,1) * ( EFG[2] * DNR[i] - EFG[1] * DNS[i] );
                DN4(1,i) += det_inverse * JMAT(1,1) * ( EFG[0] * DNS[i] - EFG[1] * DNR[i] );
                DN4(2,i)  = det_inverse * JMAT(0,2) * ( EFG[2] * DNR[i] - EFG[1] * DNS[i] );
                DN4(2,i) += det_inverse * JMAT(1,2) * ( EFG[0] * DNS[i] - EFG[1] * DNR[i] );
             }
           return;
        }
...
*/
void
IsoparametricLinearQuadrilateral::ParametricToPhysical(vector<double>& rst, vector<double>& xyz)
{
                                        ...
                                        )
{
    ...
    vector<double> distanceFromGivenPointLinf(3U,0.0);
    ...
    if( dim == 3 )
    {
        distanceFromGivenPointLinf[0] = abs( outxyz[0]-xyz[0] );
        distanceFromGivenPointLinf[1] = abs( outxyz[1]-xyz[1] );
        distanceFromGivenPointLinf[2] = abs( outxyz[2]-xyz[2] );
    ...
    //    (distanceFromGivenPointLinf[2] > geometricTolerance)  )
    {
        ...
        {
            cout<<" IsoparametricLinearQuadrilateral::PhysicalToParametric: Tolerance = "
                <<geometricTolerance<<endl;

            if( dim ==3 )
            {
                cout<<" IsoparametricLinearQuadrilateral::PhysicalToParametric: Real Point:\t"
                    <<"x = "<<xyz[0]<<" ;\t"
                    <<"y = "<<xyz[1]<<" ;\t"
                    <<"z = "<<xyz[2]<<"\n";
                cout<<" IsoparametricLinearQuadrilateral::PhysicalToParametric: Found Point:\t"
                   <<"x = "<<outxyz[0]<<" ;\t"
                   <<"y = "<<outxyz[1]<<" ;\t"
                   <<"z = "<<outxyz[2]<<"\n";
                cout<<" IsoparametricLinearQuadrilateral::PhysicalToParametric: Distance:\t"
            ...
            cout<<" IsoparametricLinearQuadrilateral::PhysicalToParametric Parametric Point:\t"
               <<"r = "<<rstHatK[0]<<" ;\t"
               <<"s = "<<rstHatK[1]<<"\n";

            vector<double> N(npe,0.0);
...
IsoparametricLinearQuadrilateral::dN( DenseMatrix<DM_MIN>& DN2, const vector<double>& xyz  )
  {
    ...
    double detJ(numeric_limits<double>::quiet_NaN());
    ...
    /////////////////////////////// Debug printout ///////////////////////////////////////////////
    //cout<<" IsoparametricLinearQuadrilateral3D::dN  For given xyz=("<<xyz[1]<<","<<xyz[2]<<","<<xyz[3]<<"), rst=("<<
    //rst[1]<<","<<rst[2]<<","<<rst[3]<<")"<<endl;
    //cout<<" IsoparametricLinearQuadrilateral3D::dN  Matrix DN2: "<<endl;
    //DN2.Out();
    /////////////////////////////// Debug printout ///////////////////////////////////////////////
    return detJ;
...
void IsoparametricLinearQuadrilateral::ConsecutiveNodesAtBoundary(
                                            ...
                                            vector<uint32_t>& fnids )
 {
     ...
     else if ( bnodes.size() < 2 ) {
           cout <<"\nnodes at boundary: ";
           for ( uint32_t j{0U}; j<bnodes.size(); j++ ) cout << bnodes[j] <<" ";
           cout << endl;
           throw csmp::Exception( ERROR, "IsoparametricLinearQuadrilateral::ConsecutiveNodesAtBoundary",
     ...
     if ( (bnodes[0] == 2 && bnodes[1] == 3) || (bnodes[0] == 3 && bnodes[1] == 2) ) {
           fnids[0] = 2;
           fnids[1] = 3;
           return;
       }

     if ( (bnodes[0] == 3 && bnodes[1] == 0) || (bnodes[0] == 0 && bnodes[1] == 3) ) {
           fnids[0] = 3;
           fnids[1] = 0;
           return;
       }
...
void
IsoparametricLinearQuadrilateral::OutputNodeDataToVTK( const char* file_name,
                                       const char* var_name,
                                       DenseMatrix<DM_MIN>& DATA ) const
  {
     ...
     if ( !ofs )
       {
           cout <<"\nIsoparametricLinearQuadrilateral3D::OutputNodeDataToVTK ";
     ...
     ofs <<"Finite-element dataset (CSP3D3.0): variable: "<< var_name << endl;
     ofs <<"ASCII"<< endl << endl;
     ...
     ofs << endl;
     ofs.close();
     cout <<"\nIsoparametricLinearQuadrilateral3D::OutputNodeDataToVTK: file '"<< outfile <<"' written successfully."<< endl;
...
const
{
 ...
 // See Zienlewitch, pp. 351, for example
...
void  IsoparametricLinearQuadrilateral::IntegrationPoint( uint32_t ip,
...
double  IsoparametricLinearQuadrilateral::JacobianInverse()
 {
    ...
    if ( detJ <= 0 ) {
         ...
         throw range_error("IsoparametricLinearQuadrilateral::JacobianInverse");
      }
...
/** Computes the normal to the quadrilateral surface assuming that the quad
...
@attention convention: the normal points towards the front looking at the quadrilateral
in the plane where the nodes appear numbered counter clockwise.

*/
vector<double>  IsoparametricLinearQuadrilateral::UnitNormal() const
 {
    // points perpendicular to 2D plane in Z direction
...
void IsoparametricLinearQuadrilateral::JacobianAtIntegrationPoint( uint32_t gauss_point )
 {
    if ( !use2Dto3Djacobi ) {
         dNr( rr[gauss_point], ss[gauss_point], DNR );
         dNs( rr[gauss_point], ss[gauss_point], DNS );
...
/*
    ...
    cout<<" JAC at Gauss point ["<< gauss_point<<"] "<<endl;
      printf(" rs= ( %20.18lf, %20.18lf ),\n xyz=( %20.18lf, %20.18lf, %20.18lf )\n ",RS[0],RS[1],xyz[0],xyz[1],xyz[2]);
...
void
IsoparametricLinearQuadrilateral::ReferenceCoordinates(DenseMatrix<DM_MIN> & matCoords) const
{
...
void IsoparametricLinearQuadrilateral::JacobianAt( const vector<double>& rst )
{
  if (dim == 3)
    {
      vector<double> EFG(3);
