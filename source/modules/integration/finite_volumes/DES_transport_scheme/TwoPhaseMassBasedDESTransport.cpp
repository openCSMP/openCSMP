...
TwoPhaseMassBasedDESTransport<dim,FLOW_FUNCTIONS>::TwoPhaseMassBasedDESTransport( Model<dim>& m, 
    ...
    : TwoPhaseDESTransport<dim,FLOW_FUNCTIONS> (m,target_region,ff,
                                                with_capillary_spreading,with_gravity_forces,
                                                tensor_k,PEP_multiplier,cfl_multiplier)
{
...
TwoPhaseMassBasedDESTransport<dim,FLOW_FUNCTIONS>::TwoPhaseMassBasedDESTransport( Model<dim>& m, 
    ...
    : TwoPhaseDESTransport<dim,FLOW_FUNCTIONS> (m,target_region,ff,
                                                with_capillary_spreading,with_gravity_forces,
                                                tensor_k,PEP_multiplier,cfl_multiplier, relaxing_factor)
{
    InitializeVariablesAndKeys(m);
    cout<<"TwoPhaseMassBasedDESTransport constructed."<<endl;
} // end constructor 
...
// SKM FIX - should be inside of sat-function: this->flowfunctions_.InitialiseBrooksCoreyParameters(eptr);
        ...
        */     
        ...
        for ( size_t i=0U; i<sector_facets; i++ )
        {
            ...
            double64 fn = sign * ( viscous_velocity_component - gravity_velocity_component - capillary_velocity_component) * facetArea; // kg/s
