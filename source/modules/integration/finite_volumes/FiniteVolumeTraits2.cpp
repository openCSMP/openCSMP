...
FiniteVolumeTraits<2U,SIMPLEX>::FiniteVolumeTraits()
 {
 }
...
template<template<size_t> class SIMPLEX>
size_t FiniteVolumeTraits<2U,SIMPLEX>::Sectors() const
  {
    const SIMPLEX<2U>* eptr( static_cast<const SIMPLEX<2U>*>(this) );
    assert( eptr != NULL );
    if( eptr->FV_Stencil() )
...
template<template<size_t> class SIMPLEX>
size_t FiniteVolumeTraits<2U,SIMPLEX>::Facets() const
  {
    const SIMPLEX<2U>* eptr( static_cast<const SIMPLEX<2U>*>(this) );
    assert( eptr != NULL );
    if( eptr->FV_Stencil() )
...
template<template<size_t> class SIMPLEX>
size_t FiniteVolumeTraits<2U,SIMPLEX>::IntegrationPointsPerSector() const
  {
    const SIMPLEX<2U>* eptr( static_cast<const SIMPLEX<2U>*>(this) );
    assert( eptr != NULL );
    if( eptr->FV_Stencil() )
...
template<template<size_t> class SIMPLEX>
size_t FiniteVolumeTraits<2U,SIMPLEX>::IntegrationPointsPerFacet() const
  {
    const SIMPLEX<2U>* eptr( static_cast<const SIMPLEX<2U>*>(this) );
    assert( eptr != NULL );
    if( eptr->FV_Stencil() )
...
template<template<size_t> class SIMPLEX>
Point<2U>  FiniteVolumeTraits<2U,SIMPLEX>::RstToXYZ( const Point<2U>& rst ) const
{
...
template<template<size_t> class SIMPLEX>
void FiniteVolumeTraits<2U,SIMPLEX>::N_At( const Point<2U>& rst ) const
 {
   const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
...
template<template<size_t> class SIMPLEX>
void FiniteVolumeTraits<2U,SIMPLEX>::N_At( const Point<2U>& rst,
...
template<template<size_t> class SIMPLEX>
void FiniteVolumeTraits<2U,SIMPLEX>::N_AtFacetIntegrationPoint( size_t iFacet, size_t ip ) const
 {
    const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
...
template<template<size_t> class SIMPLEX>
void FiniteVolumeTraits<2U,SIMPLEX>::N_AtSectorIntegrationPoint( size_t iSector, size_t ip ) const
 {
    const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
...
template<template<size_t> class SIMPLEX>
void FiniteVolumeTraits<2U,SIMPLEX>::Local_dN_At( const Point<2U>& rst ) const
 {
    const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
    if ( e->FE()->IsSurfaceElement() ) {
...
template<template<size_t> class SIMPLEX>
double64 FiniteVolumeTraits<2U,SIMPLEX>::dN_At( const Point<2U>& rst,
...
// NOT DONE YET
    // line element
    e->FE()->dNr( rst[0], e->FE()->DNR );
    std::cout <<"\nFiniteVolumeTraits<2U,SIMPLEX>::dN_At: 1D->2D Jacobian is required to get this right."<< std::endl;
...
// only for scalars
template<template<size_t> class SIMPLEX>
double64 FiniteVolumeTraits<2U,SIMPLEX>::PropertyValueAtFacetIntegrationPoint(
                                                 size_t iFacet,
                                                 size_t ip,
...
// only for scalars
template<template<size_t> class SIMPLEX>
double64  FiniteVolumeTraits<2U,SIMPLEX>::PropertyValueAtSectorIntegrationPoint(
                                                            size_t iSector,
                                                            size_t ip,
                                                            const csmp::Index& prop_key ) const
{
...
template<template<size_t> class SIMPLEX>
template<class Var>
void FiniteVolumeTraits<2U,SIMPLEX>::PropertyValueAtFacetIntegrationPoint(
...
template<template<size_t> class SIMPLEX>
template<class Var>
void  FiniteVolumeTraits<2U,SIMPLEX>::PropertyValueAtSectorIntegrationPoint(
                                                            const csmp::Index& prop_key,
                                                            size_t iSector,
                                                            size_t ip,
                                                            Var& var ) const
...
template<template<size_t> class SIMPLEX>
double64  FiniteVolumeTraits<2U,SIMPLEX>::FacetIntegral( size_t iSector,
                                                             const csmp::Index& prop_key ) const
{
    const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
    assert( iSector < e->FV_Stencil()->Sectors());
    ...
    for ( size_t iFacet=0U; iFacet<e->FV_Stencil()->FacetsPerSector(iSector); iFacet++ )
      for ( size_t j=0U; j<e->FV_Stencil()->IntegrationPointsPerFacet(); j++ ) {
           ...
           integral += PropertyValueAtFacetIntegrationPoint( iFacet, j, prop_key ) *
                       e->FV_Stencil()->FacetIntegrationWeight( iFacet, j ) * detJ;
       }
...
template<template<size_t> class SIMPLEX>
double64  FiniteVolumeTraits<2U,SIMPLEX>::SectorIntegral( size_t iSector,
                                                          const csmp::Index& prop_key ) const
 {
    const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
    assert( iSector < e->FV_Stencil()->Sectors());
...
template<template<size_t> class SIMPLEX>
double64  FiniteVolumeTraits<2U,SIMPLEX>::ProjectionOnFacetNormal(
                                                 size_t iFacet,
...
template<template<size_t> class SIMPLEX>
double64  FiniteVolumeTraits<2U,SIMPLEX>::SectorVolume( size_t iSector ) const
 {
     const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
     assert( iSector < e->FV_Stencil()->Sectors());
...
template<template<size_t> class SIMPLEX>
double64  FiniteVolumeTraits<2U,SIMPLEX>::FacetArea( size_t iFacet ) const
{
    const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
    assert( iFacet < e->FV_Stencil()->Facets());
...
template<template<size_t> class SIMPLEX>
Point<2U> FiniteVolumeTraits<2U,SIMPLEX>::FacetNormal( size_t iFacet ) const
{
  const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
  assert( iFacet < e->FV_Stencil()->Facets());
...
template<template<size_t> class SIMPLEX>
double64  FiniteVolumeTraits<2U,SIMPLEX>::FacetAreaMapped( size_t iFacet ) const
{
    const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
    assert( iFacet < e->FV_Stencil()->Facets());
    ...
    const Point<2U> fp0( RstToXYZ(e->FV_Stencil()->FacetPoint(iFacet,0U)) );
    const Point<2U> fp1( RstToXYZ(e->FV_Stencil()->FacetPoint(iFacet,1U)) );
...
template<template<size_t> class SIMPLEX>
Point<2U> FiniteVolumeTraits<2U,SIMPLEX>::FacetNormalMapped( size_t iFacet ) const
{
   const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
   assert( iFacet < e->FV_Stencil()->Facets());

  if(e->FE()->IsLineElement()) return Point<2U>(1.,0);
...
template<template<size_t> class SIMPLEX>
double64  FiniteVolumeTraits<2U,SIMPLEX>::ParametricFacetArea( size_t iFacet ) const
{
  const SIMPLEX<2U>* e( static_cast<const SIMPLEX<2U>*>(this) );
  assert(iFacet<e->FV_Stencil()->Facets());
...
template class FiniteVolumeTraits<2U,Element>;
template class FiniteVolumeTraits<2U,Face>;
template class FiniteVolumeTraits<2U,InterFace>;
