	...
	void PDE_Integrator_CRM<dim, INTEGRATION_DOMAIN>::EstablishMatrixSetup(const INTEGRATION_DOMAIN<dim>& gref)
	{
			...
			(!basic_operands_.empty() && !test_operands_.empty()))
		{
			...
			if (G_.Rows()  > 0 && !retain_matrix_) {
				dirich_.clear(); x_.clear(); RH_.clear(); fill(rh_.begin(), rh_.end(), 0.);
				G_.Resize(rh_.size());
#if defined(_OPENMP )
				for (size_t tid = 0; tid < omp_get_max_threads(); tid++) {
	...
	void PDE_Integrator_CRM<dim, INTEGRATION_DOMAIN>::Accumulate(const INTEGRATION_DOMAIN<dim>& gref)
	{
		...
		for (typename map<string, MathOperatorLHS<dim>*>::iterator
			...
			if (!(*it_lhs).second->AddLater() && !(*it_lhs).second->SubtractLater())
				for (typename vector<typename INTEGRATION_DOMAIN<dim>::CellType*>::const_iterator
...
#pragma omp for
					...
					{
						//                    cout<<"element: "<<e<<endl;
						typename INTEGRATION_DOMAIN<dim>::CellType* eit = gref.E(e);
...
#pragma omp for
		...
		for (operandsConstIterator it = test_operands_.begin(); it != test_operands_.end(); it++)
		{
			...
			case VECTOR: {
				...
				while (niter != gref.NodesEnd()) {
					...
					for (size_t i = 0U; i < dim; i++)
						if (vc.Flag(i) == DIRICH)
							dirich_.push_back(Entry((*niter)->Idx() * dim + i + offset, scale_factor_ * vc(i)));
					niter++;
				}
			}
						 break;
					...
					for (size_t i = 0U; i < dim; i++)
						if (ts.Flag(i) == DIRICH)
							for (size_t j = 0U; j < dim; j++)
								dirich_.push_back(Entry((*niter)->Idx() * dim2_ + i * dim + j + offset, scale_factor_ * ts(i, j)));
					++niter;
				}
			}
						 break;
				...
				while (niter != gref.NodesEnd()) {
					...
					if (ar.Flag() == DIRICH)
						for (size_t i = 0U; i < prop_key.dataDepth; i++)
							dirich_.push_back(Entry((*niter)->Idx() * prop_key.dataDepth + i + offset, scale_factor_ * ar(i)));
					niter++;
				}
			}
						break;
	...
	void PDE_Integrator_CRM<dim, INTEGRATION_DOMAIN>::IntegrateOver(INTEGRATION_DOMAIN<dim>& domain, bool debug)
	{
		...
		/***************************/
		// UNCOMMENT this if you removed in 2.2
