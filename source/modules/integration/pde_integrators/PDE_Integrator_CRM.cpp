...
PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::PDE_Integrator_CRM()
  :
...
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::PDE_Integrator_CRM( Solver& solver )
  : solver_( &solver ),
  dim2_( dim*dim ),
  dof_per_node_( 0 ),
  setup_established_( false ),
...
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::PDE_Integrator_CRM( const PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>& a )
  :
#ifdef CSMP_WITH_SAMG_SOLVER
  solver_( new SAMG_Solver() ),
#else
...
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>& PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::operator=( const PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>& a )
{
  if ( &a != this ) {
    G_ = a.G_;
    rh_ = a.rh_;
...
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::RetainGlobalSolutionMatrix( bool retain )
...
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::SetSolver( Solver* new_solver ) {
...
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
Solver* PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::GetSolver() const {
  return solver_;
}
...
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::AdjustSolverSettings()
{
  ErrorHandler&  csmp_error( ErrorHandler::Instance() );
  csmp_error.notice( WARNING, "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::AdjustSolverSettings",
  ...
  /** Transient PDE_Integrator_CRM

  The accumulation process for a transient PDE_Integrator_CRM is different from a
  steady-state PDE_Integrator_CRM in that initial conditions must be assigned and
  matrices must be multiplied with the time_increment. This method
  sets the PDE_Integrator_CRM state to transient or retrieves its state.

  @section application Application

  Every PDE_Integrator_CRM which you define in your program uses storage for G, x, and
  ...
  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
bool  PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::Transient() const
...
the PDE_Integrator_CRM to transient, this is done as well.

@section application Application

Set the time-increment of an PDE_Integrator_CRM before you Apply() it to the
Model or target Region objects.
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void   PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::TimeIncrement( double64 dt )
{
  time_increment_ = dt;
}
...
/** Prints out the compressed row matrix and the righthand vector

...which were accumulated by the PDE_Integrator_CRM. If you execute this method after passing
the PDE_Integrator_CRM to the Model, the matrix will have been modified by the
Solver object.
...
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void  PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::OutputGlobals( int32 precision )
{
  ...
  The PDE_Integrator_CRM object maintains a list of pointers to MathOperator instances
  which represent parts of the PDE which is being modeled by the PDE_Integrator_CRM.
  ...
  Each PDE_Integrator_CRM needs at least one left and one righthand math operator.
  Define these before you pass the PDE_Integrator_CRM to the Model.

  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::Add( MathOperatorLHS<dim>* op )
{
...
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::Add( MathOperatorRHS<dim>* op )
{
...
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::AddPostProcess( MathOperatorLHS<dim>* op )
{
...
Lists all MathOperator objects which have been defined for the PDE_Integrator_CRM.
...
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::ListMathOperatorsLHS() const
{
...
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::ListMathOperatorsRHS() const
{
...
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void  PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::SolutionVector( vector<double64>& sol ) const {
  sol.resize( x_.size() );
...
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void  PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::FirstGuess( const vector<double64>& guess ) {
...
After an PDE_Integrator_CRM has been applied, the solution matrix G and the righthand
...
PDE_Integrator_CRM, unless it has been set to retain the solution matrix.

In addition, you may want to re-use the PDE_Integrator_CRM and define
new PDE operators, a new computational domain and/or computational
method. In this case, Reset() allows to restore the PDE_Integrator_CRM's default state
and zero's the elements of its matrix and vector storage.

@section application Application

Typically, you will reset an PDE_Integrator_CRM if you can just re-use it for
...
*/
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void  PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::Reset( bool delete_math_operators )
{
  ...
  The method Solve() is called internally, when the PDE_Integrator_CRM object is
  passed to the Model.
  ...
  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::Solve()
{
  ...
  EstablishMatrixSetup() is applied internally when the PDE_Integrator_CRM is
  passed to the Model.
  ...
  @code
  PDE_Integrator_CRM cannot add new variable to database
  @endcode
  ...
  The following Error won't terminate PDE_Integrator_CRM application:
  ...
  @code
  PDE_Integrator_CRM was used before, using same specifications again...
  @endcode

  You forgot to call Reset() before re-using an existing PDE_Integrator_CRM. You will
  ...
  Each PDE_Integrator_CRM needs a lefthand basic operand in order to define
  a PDE problem. Define one before applying the PDE_Integrator_CRM.
  ...
  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::EstablishMatrixSetup( const COMPUTATION_DOMAIN<dim>& gref )
{
        ...
        lhs_it = lhs_operators_.begin(); lhs_it != lhs_operators_.end(); lhs_it++ )
  {
    ...
    if ( unspecified != pkey )
      ...
      throw csmp::Exception( WARNING,
                             "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::EstablishMatrixSetup",
                             "lefthand basic operand not found" );
        ...
        rhs_it = rhs_operators_.begin(); rhs_it != rhs_operators_.end(); rhs_it++ )
  {
    ...
    if ( unspecified != pkey )
      ...
      throw csmp::Exception( ERROR, "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::EstablishMatrixSetup",
        ...
        iter = test_operands_.begin(); iter != test_operands_.end(); iter++ )
  {
    ...
    switch ( (*iter).first.key.place )
    {
      ...
      case ELEMENT:
        ...
        break;
      default:
        throw csmp::Exception( FATAL_ERROR, "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::EstablishMatrixSetup",
  ...
  AssignInitialConditions() is applied internally, when the PDE_Integrator_CRM
  ...
  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::AssignInitialConditions( const COMPUTATION_DOMAIN<dim>& gref )
{
  size_t                  i, j;
  ...
  if ( !setup_established_ )
    throw csmp::Exception( ERROR, "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::AssignInitialConditions",
                           "please call EstablishMatrixSetup() prior to this method." );
        ...
        it = test_operands_.begin(); it != test_operands_.end(); it++ )
  {
    ...
    if ( prop_key.place != NODE ) {
      throw csmp::Exception( WARNING, "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::AssignInitialConditions",
  ...
  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::ScaleEssentialConditions( double64 scale_factor )
{
  ...
  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::AssignEssentialConditions( const COMPUTATION_DOMAIN<dim>& gref )
{
  if ( !setup_established_ )
    throw csmp::Exception( ERROR, "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::AssignEssentialConditions",
                           "please call EstablishMatrixSetup() prior to this method." );
  ...
  // ----------------------------------------------------
  // 2. if the PDE_Integrator_CRM applies to a Region
  // ----------------------------------------------------
    ...
    switch ( prop_key.type )
    {
      ...
      default:
        throw csmp::Exception( FATAL_ERROR,
                               "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::AssignEssentialConditions",
  ...
  Accumulate() is executed internally when the PDE_Integrator_CRM is passed to the
  Model.
  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::Accumulate( const COMPUTATION_DOMAIN<dim>& gref )
{
...
#pragma omp for
  ...
  execution is invoked internally, when the PDE_Integrator_CRM is passed to the
  Model.
  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void  PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::LateAccumulate( const COMPUTATION_DOMAIN<dim>& gref )
{
  ...
  PostProcess() is a virtual PDE_Integrator_CRM interface to be implemented in a
  ...
  PDE_Integrator_CRM class, post-processing is used to calculate flow velocities
  ...
  PostProcess() is executed internally when the PDE_Integrator_CRM is passed to
  the Model.

  @section message Messages

  When you use the PDE_Integrator_CRM baseclass in your computations or if you
  do not define PostProcess(), you will get the info message:

  "no post-processing operations for REGION were defined in derived PDE_Integrator_CRM"
  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void  PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::PostProcess( const COMPUTATION_DOMAIN<dim>& gref )
{
  ...
  OutputResults() is executed internally, when a PDE_Integrator_CRM object is passed
  to the Model.

  */
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::OutputResults( COMPUTATION_DOMAIN<dim>& gref )
{
  Index   prop_key;
  size_t  offset;
        ...
        it = basic_operands_.begin(); it != basic_operands_.end(); it++ )
  {
    ...
    if ( prop_key.place != NODE )
      throw csmp::Exception( ERROR, "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::OutputResults(Model)",
...
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::IntegrateOver( COMPUTATION_DOMAIN<dim>& domain, bool debug )
{
...
template<size_t dim, template<size_t> class COMPUTATION_DOMAIN>
void PDE_Integrator_CRM<dim, COMPUTATION_DOMAIN>::WriteGlobalMatrixBitMapToText( const char* file )
{
  ...
  if ( !ofs )
    throw csmp::Exception( ERROR, "PDE_Integrator_CRM<dim,COMPUTATION_DOMAIN>::WriteGlobalMatrixBitMapToText",
