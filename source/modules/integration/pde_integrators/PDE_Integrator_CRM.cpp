...
PDE_Integrator_CRM<dim,CELLTYPE>::PDE_Integrator_CRM()
 :
   ...
   verbose_(true)
{
   static_assert( !is_same<CELLTYPE<dim>,Region<dim>>::value,
                  "PDE_Integrator_CRM: template template parameter must be Element or Face");
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
PDE_Integrator_CRM<dim,CELLTYPE>::PDE_Integrator_CRM( Solver& solver )
 : solver_(&solver),
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::Verbose( bool verbose )
{ verbose_=verbose; }



template<uint32_t dim,template<uint32_t> class CELLTYPE>
bool PDE_Integrator_CRM<dim,CELLTYPE>::Verbose() const
{ return verbose_; }



template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::RetainGlobalSolutionMatrix( bool retain )
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::TrimExcessCapacityOfVectors( bool trim )
 {
     trim_vectors_ = trim;
 }
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::SetSolver( Solver& new_solver ) {
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
Solver& PDE_Integrator_CRM<dim,CELLTYPE>::GetSolver() const {
  return *solver_;
}
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
bool  PDE_Integrator_CRM<dim,CELLTYPE>::Transient() const
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void   PDE_Integrator_CRM<dim,CELLTYPE>::TimeIncrement( double dt )
 {
    time_increment_ = dt;
 }
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::OutputGlobals( int32_t precision )
 {
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::Add( MathOperatorLHS<dim,CELLTYPE>* op )
 {
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::Add( MathOperatorRHS<dim,CELLTYPE>* op )
 {
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::AddBoundaryIntegral( MathOperatorRHS<dim,Face>* op )
 {
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::AddSplitBoundaryIntegral( MathOperatorRHS<dim,InterFace>* op )
 {
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::AddSplitBoundaryIntegral( MathOperatorLHS<dim,InterFace>* op )
 {
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::AddPostProcess( MathOperatorLHS<dim,CELLTYPE>* op )
 {
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::ListMathOperatorsLHS() const
 {
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::ListMathOperatorsRHS() const
 {
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::SolutionVector( vector<double>& sol ) const {
  sol.resize(x_.size());
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::FirstGuess( const vector<double>& guess ) {
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::Reset( bool delete_math_operators )
 {
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::Solve()
 {
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::EstablishMatrixSetup( const ModelSubDomain<dim,CELLTYPE>& gref )
 {
   // -------------------------------------------------------------------
   ...
   if ( setup_established_ &&
       ...
       !basic_operands_.empty() && !test_operands_.empty() )
     {
        ...
        if ( G_.Rows() > 0 && retain_matrix_ == false ) {
            ...
            //G_.Resize( rh_.size() );
            //added
            G_.Zero(); //sets all values in vector a to zeros, but keep ia and ja unchanged
          }
        return;
     }
       ...
       for ( auto& lhs_it : lhs_operators_ )
         {
            ...
            if ( pkey != unspecified ) basic_operands_[ lhs_it.second->BasicOperand() ] = 0U;
                ...
                throw csmp::Exception( WARNING,
                                       "PDE_Integrator_CRM<dim,CELLTYPE>::EstablishMatrixSetup:",
                                       "lefthand basic operand not found.");
       ...
       if ( !lhs_split_boundary_operators_.empty() ){
           for (auto& lhs_it : lhs_split_boundary_operators_){
             ...
             if (pkey != unspecified) basic_operands_[ lhs_it.second->BasicOperand()] = 0U ; //addition of operand and default initialisation of its offset to 0
               ...
               throw csmp::Exception( ERROR, "PDE_Integrator_CRMExperimental<dim,COMPUTATIONAL_DOMAIN>::EstablishMatrixSetup",
       ...
       for ( auto& rhs_it : rhs_operators_ )
         {
            ...
            if ( pkey != unspecified ) test_operands_[ rhs_it.second->TestOperand() ] = 0;
              ...
              throw csmp::Exception( ERROR, "PDE_Integrator_CRM<dim,CELLTYPE>::EstablishMatrixSetup:",
        ...
        switch ( iter.first.key.place )
         {
            ...
            case ELEMENT:
              ...
              switch ( iter.first.key.type ) {
               ...
               break;
            default:
              throw csmp::Exception( FATAL_ERROR, "PDE_Integrator_CRM<dim,CELLTYPE>::EstablishMatrixSetup",
   ...
   for ( auto& lhs_it : lhs_operators_ )
     ...
     for (auto& lhs_it : lhs_split_boundary_operators_){
       ...
       if( (iter = test_operands_.find( lhs_it.second->BasicOperand() )) != test_operands_.end()  ){
         ...
         throw csmp::Exception(ERROR, "PDE_Integrator_CRM_Experimental::EstablishMatrixSetup",
       ...
       if ( (iter=test_operands_.find(lhs_it.second->TestOperand() ) ) != test_operands_.end() ){
         ...
         throw csmp::Exception(ERROR, "PDE_Integrator_CRMExperimental::EstablishMatrixSetup",
 ...
 */
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::AssignInitialConditions( const ModelSubDomain<dim,CELLTYPE>& gref )
 {
    if ( !setup_established_ )
      throw csmp::Exception( ERROR, "PDE_Integrator_CRM<dim,CELLTYPE>::AssignInitialConditions",
                             "please call EstablishMatrixSetup() prior to this method.");
     ...
     for ( const auto& it : test_operands_ )
       {
          ...
          if ( prop_key.place != NODE )
            throw csmp::Exception( WARNING, "PDE_Integrator_CRM<dim,CELLTYPE>::AssignInitialConditions",
...
*/
template<uint32_t dim, template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim, CELLTYPE>::AssignEssentialConditions( const ModelSubDomain<dim,CELLTYPE>& domain )
 {
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::Accumulate( const ModelSubDomain<dim,CELLTYPE>& gref )
 {
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::AccumulateBoundaries( Model<dim>& model, ModelSubDomain<dim,CELLTYPE>& domain, list<string> shared_boundaries ){
    ...
    for ( const auto& it : shared_boundaries ) {
         ...
         cout <<"\nPDE_Integrator_CRM<dim,CELLTYPE>::IntegrateOver: ";
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::AccumulateSplitBoundaries( Model<dim>& model, ModelSubDomain<dim,CELLTYPE>& domain, list<string> contained_splitboundaries ){
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::AccumulateBoundaryIntegrals( const ModelSubDomain<dim,CELLTYPE>& comp_domain,
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::AccumulateSplitBoundaryIntegrals( const ModelSubDomain<dim,CELLTYPE>& comp_domain,
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::LateAccumulate( const ModelSubDomain<dim,CELLTYPE>& gref )
 {
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::LateAccumulateBoundaryIntegrals( const ModelSubDomain<dim,CELLTYPE>& comp_domain,
                                                                     const Boundary<dim>& boundary )
 {
...
// same but for SplitBoundary objects
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::LateAccumulateSplitBoundaryIntegrals( const ModelSubDomain<dim,CELLTYPE>& comp_domain,
                                                                          const SplitBoundary<dim>& splitboundary )
 {
    // SKM NEW: accumulating into the sparse matrix 'G'
    // ------------------------------------------------
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::CoupleDomainsAcrossSplitBoundary( ModelSubDomain<dim,CELLTYPE>& subdomain )
{
  static_assert( !is_same<CELLTYPE<dim>,SplitBoundary<dim> >::value,
                 "PDE_Integrator_CRM::CoupleDomainsAcrossSplitBoundary: only works for Region or Boundary objects" );
   
  if ( test_operands_.size() > 1U )
    throw csmp::Exception( ERROR, "PDE_Integrator_CRM::CoupleDomainsAcrossSplitBoundary",
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void  PDE_Integrator_CRM<dim,CELLTYPE>::PostProcess( ModelSubDomain<dim,CELLTYPE>& gref )
 {
 ...
 */
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::OutputResults( ModelSubDomain<dim,CELLTYPE>& gref )
 {
   size_t       position;
   const uint32_t dim2(dim * dim);
    ...
    for ( const auto& it : basic_operands_ )
     {
        ...
        if ( prop_key.place != NODE )
            throw csmp::Exception( ERROR, "PDE_Integrator_CRM<dim,CELLTYPE>::OutputResults(Model)",
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::IntegrateOver( ModelSubDomain<dim,CELLTYPE>& domain, bool debug )
 {
    // 1. configure algorithm
    EstablishMatrixSetup( domain );
    if ( !rhs_boundary_operators_.empty() )
      throw csmp::Exception( ERROR, "PDE_Integrator_CRM<>::IntegrateOver(domain):",
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::IntegrateOver( Model<dim>& model,
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
std::list<std::string> PDE_Integrator_CRM<dim,CELLTYPE>::IdentifySharedBoundaries( const Model<dim>& model,
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
std::list<std::string> PDE_Integrator_CRM<dim,CELLTYPE>::IdentifySharedSplitBoundaries( const Model<dim>& model, const ModelSubDomain<dim,CELLTYPE>& subdomain){
...
*/
template<uint32_t dim, template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim, CELLTYPE>::ReduceSystemSizeEliminatingEssentialConditions( const ModelSubDomain<dim,CELLTYPE>& gref, size_t total_degrees_of_freedom )
 {
    ...
    if (!this->setup_established_)
      throw csmp::Exception(ERROR, "PDE_Integrator_CRM<dim,CELLTYPE>::ReduceSystemSizeEliminatingEssentialConditions",
        "please call EstablishMatrixSetup() prior to this method.");
           ...
           break;
            default:
              throw csmp::Exception(FATAL_ERROR,
                "PDE_Integrator_CRM<dim,CELLTYPE>::ReduceSystemSizeEliminatingEssentialConditions",
    ...
    this->G_.Erase();
    //this->G_.Resize(DOF); //added - commented out
    generateSparsityPatternEliminatingEssentialConditions<dim,CELLTYPE>(G_, test_operands_, DOF_indexes_, gref); //added
...
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::WriteGlobalMatrixBitMapToText( const char* file )
 {
    ...
    strcat( outfile, ".txt" );
     ...
     if ( !ofs )
       throw csmp::Exception( ERROR, "PDE_Integrator_CRM<dim,CELLTYPE>::WriteGlobalMatrixBitMapToText",
...
*/
template<uint32_t dim,template<uint32_t> class CELLTYPE>
void PDE_Integrator_CRM<dim,CELLTYPE>::Out() const
 {
     cout <<"\n\n"<<"PDE_Integrator_CRM<"<< dim <<",CELLTYPE>::Out:\n";
...
template class PDE_Integrator_CRM<1U,Element>;
template class PDE_Integrator_CRM<2U,Element>;
template class PDE_Integrator_CRM<3U,Element>;

template class PDE_Integrator_CRM<1U,Face>;
template class PDE_Integrator_CRM<2U,Face>;
template class PDE_Integrator_CRM<3U,Face>;
