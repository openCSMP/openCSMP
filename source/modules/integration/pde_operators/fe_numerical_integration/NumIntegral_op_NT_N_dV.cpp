#include "NumIntegral_op_NT_N_dV.h"
#include "Element.h"
#include "Face.h"
...
NumIntegral_op_NT_N_dV<dim,SIMPLEX>::NumIntegral_op_NT_N_dV( const PropertyDatabase<dim>& pref,
                                                        const char* oper, const char* test )
  : MathOperatorRHS<dim>(pref,oper,test),
    nodal_degrees_of_freedom(1)
 {
    MathOperatorRHS<dim>::Name("NumIntegral_op_NT_N_dV", oper, test );
    ...
    if ( MathOperatorRHS<dim>::MaterialOperandType() != SCALAR && 
      ...
      throw csmp::Exception( CSMP_ERROR, "NumIntegral_op_NT_N_dV<dim>::(constructor)", 
...
template<size_t dim,class SIMPLEX>
void NumIntegral_op_NT_N_dV<dim,SIMPLEX>::ComputeContribution( SIMPLEX& e )
{
    ...
    cout <<"\nNumIntegral_op_NT_N_dV<dim>::ComputeContribution: Consistent formulation not implemented yet !"<< endl;

}

template class NumIntegral_op_NT_N_dV<1U,Element<1U> >;
template class NumIntegral_op_NT_N_dV<2U,Element<2U> >;
template class NumIntegral_op_NT_N_dV<3U,Element<3U> >;

template class NumIntegral_op_NT_N_dV<1U,Face<1U> >;
template class NumIntegral_op_NT_N_dV<2U,Face<2U> >;
template class NumIntegral_op_NT_N_dV<3U,Face<3U> >;
