...
template<uint32_t dim, template<uint32_t> class CELL>
void NumIntegral_op_NT_N_dV<dim,CELL>::ComputeContribution( const CELL<dim>& e )
{
    ...
    if ( MathOperatorRHS<dim,CELL>::LumpedFormulation() )
      {
         ...
         // explicitly  
         else 
          {
             ...
             // point is assembled
             for ( uint32_t n{0u}; n<e.FE()->IntegrationPoints(); n++ )
               {
...
template class NumIntegral_op_NT_N_dV<1U,Element>;
template class NumIntegral_op_NT_N_dV<2U,Element>;
template class NumIntegral_op_NT_N_dV<3U,Element>;

template class NumIntegral_op_NT_N_dV<1U,Face>;
template class NumIntegral_op_NT_N_dV<2U,Face>;
template class NumIntegral_op_NT_N_dV<3U,Face>;
