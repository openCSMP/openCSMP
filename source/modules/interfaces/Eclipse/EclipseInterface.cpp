...
template<size_t dim>
EclipseInterface<dim>::EclipseInterface()
: NX_(0),
  NY_(0),
  NZ_(0)
{
...
template<size_t dim>
void EclipseInterface<dim>::ClearBefore()
{
    /// grid specs
    NX_ = 0;
    NY_ = 0;
...
template<size_t dim>
void EclipseInterface<dim>::ClearAfter()
{
    /// grid
    grid_.Clear();
    block_grid_.Clear();
...
template<size_t dim>
void EclipseInterface<dim>::SetProperties(const std::map<int,csmp::Parameter>& p )
{
    properties_ = p;
}
...
template void EclipseInterface<1U>::GetRegions( std::list<std::string>& );
template void EclipseInterface<1U>::GetRegions( std::set<std::string>& );

template void EclipseInterface<2U>::GetRegions( std::vector<std::string>& );
template void EclipseInterface<2U>::GetRegions( std::list<std::string>& );
template void EclipseInterface<2U>::GetRegions( std::set<std::string>& );

template void EclipseInterface<3U>::GetRegions( std::vector<std::string>& );
template void EclipseInterface<3U>::GetRegions( std::list<std::string>& );
template void EclipseInterface<3U>::GetRegions( std::set<std::string>& );
...
template void EclipseInterface<1U>::GetFaults( std::list<std::string>& );
template void EclipseInterface<1U>::GetFaults( std::set<std::string>& );

template void EclipseInterface<2U>::GetFaults( std::vector<std::string>& );
template void EclipseInterface<2U>::GetFaults( std::list<std::string>& );
template void EclipseInterface<2U>::GetFaults( std::set<std::string>& );

template void EclipseInterface<3U>::GetFaults( std::vector<std::string>& );
template void EclipseInterface<3U>::GetFaults( std::list<std::string>& );
template void EclipseInterface<3U>::GetFaults( std::set<std::string>& );
...
template void EclipseInterface<1U>::GetWells( std::list<std::string>& );
template void EclipseInterface<1U>::GetWells( std::set<std::string>& );

template void EclipseInterface<2U>::GetWells( std::vector<std::string>& );
template void EclipseInterface<2U>::GetWells( std::list<std::string>& );
template void EclipseInterface<2U>::GetWells( std::set<std::string>& );

template void EclipseInterface<3U>::GetWells( std::vector<std::string>& );
template void EclipseInterface<3U>::GetWells( std::list<std::string>& );
template void EclipseInterface<3U>::GetWells( std::set<std::string>& );
...
*/
template<size_t dim>
bool EclipseInterface<dim>::ReadFile( csmp::VSet<dim>& vset,
                                      ...
                                      bool exclude_inactive_cells, bool tetra_mesh )
{
    ...
    if( csmp_error.Verbose() )
        std::cout <<"\nEclipseInterface<"<< dim <<">::ReadFile: Reading ECLIPSE ASCII file '"<<fname<<"'..."<< std::endl;
    ...
    if( csmp_error.Verbose() )
        std::cout <<"\nEclipseInterface<"<< dim <<">::ReadFile: file '"<< fname <<"' read successfully!"<< std::endl;
...
*/
template<size_t dim>
void EclipseInterface<dim>::WritePropertiesToVSet()
{
    /// 1. write rock types
    if( !satnum_.empty() )
...
template<size_t dim>
void EclipseInterface<dim>
::WriteScalarPropertyToVSet( csmp::VSet<dim>&  vset,
                             const CornerPointGrid<dim>& grid,
...
::WriteScalarPropertyToVSet( csmp::VSet<dim>&  vset,
                             const CornerPointGrid<dim>& grid,
                             const std::vector<csmp::VectorVariable<dim> >& vector_data,
                             const std::string& property_name,
                             const csmp::PLACEMENT& place )
 {
...
::WriteScalarPropertyToVSet( csmp::VSet<dim>&  vset,
                             const CornerPointGrid<dim>& grid,
                             const std::vector<csmp::TensorVariable<dim> >& tensor_data,
                             const std::string& property_name,
                             const csmp::PLACEMENT& place )
 {
...
template<size_t dim>
void EclipseInterface<dim>
::WriteVectorPropertyToVSet( csmp::VSet<dim>&  vset,
                             const CornerPointGrid<dim>& grid,
                             const std::vector<csmp::VectorVariable<dim> >& vector_data,
                             const std::string& property_name,
                             const csmp::PLACEMENT& place )
...
template<size_t dim>
void EclipseInterface<dim>
::WriteTensorPropertyToVSet( csmp::VSet<dim>&  vset,
                             const CornerPointGrid<dim>& grid,
                             const std::vector<csmp::TensorVariable<dim> >& tensor_data,
                             const std::string& property_name,
                             const csmp::PLACEMENT& place )
 {
    ...
    grid.template WritePropertyToVSet<csmp::TensorVariable<dim> >(vset,tensor_data,property_name,place);
    return;
 }
...
*/
template<size_t dim>
bool EclipseInterface<dim>::ReadFile( std::ifstream&  ifs, size_t line_length )
{
    TextFileInterface<EclipseInterface<dim> > reader( line_length );
...
//    reader.AddKeyword("TRANZ");
    ...
    reader.ReadFile( ifs, this, (&isEclipseCommentLine), (&EclipseInterface<dim>::ReadBlock) );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadIncludeFile( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
    ...
    // 2. Read file
    if( csmp_error.Verbose() )
        std::cout <<"\nEclipseInterface<"<< dim <<">::ReadIncludeFile: Reading ECLIPSE ASCII file '"<<fname<<"'..."<< std::endl;
    bool result( ReadFile( ifs_include, line_length ) );
    if( csmp_error.Verbose() )
        std::cout <<"\nEclipseInterface<"<< dim <<">::ReadIncludeFile: file '"<< fname <<"'read successfully!"<< std::endl;
    return result;
}
...
template<size_t dim>
bool EclipseInterface<dim>
::ReadUnknownBlock( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );

    if( csmp_error.Verbose() )
        std::cout <<"\nEclipseInterface<"<< dim <<">::ReadUnknownBlock: reading "<<keyword_<<" block of data...";

    if( skipEclipseBlock( ifs, text_line, line_length, csmp_error.Verbose() ) == 0)
        return false;

    if( csmp_error.Verbose() )
        std::cout <<"\nEclipseInterface<"<< dim <<">::ReadUnknownBlock: block "<<keyword_<<" has been skipped!\n";
...
template<size_t dim>
bool EclipseInterface<dim>::ReadBlock( std::ifstream& ifs, char* text_line, size_t line_length )
{
...
template<size_t dim>
bool EclipseInterface<dim>::ReadDimensions( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadGridSpecs( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadCellSizes( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadCellDepths( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadPillarCoordinates( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadCornerDepths( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadActiveCells( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadWellSpecs( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadWellCompletionsData( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadExplicitFaceWellCompletionsData( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
    if ( readEclipseWellCompletionsData( NX_,NY_,NZ_,
                                         well_data_,
...
template<size_t dim>
bool EclipseInterface<dim>::ReadExplicitNodeWellCompletionsData( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
    if ( readEclipseWellCompletionsData( NX_,NY_,NZ_,
                                         well_data_,
...
template<size_t dim>
bool EclipseInterface<dim>::ReadFaultsData( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadFaultTransmissibilityMultipliers( std::ifstream& ifs, char* text_line, size_t line_length )
{
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );
...
template<size_t dim>
bool EclipseInterface<dim>::ReadScalarProperty( std::ifstream& ifs, char* text_line, size_t line_length,
...
template<size_t dim>
bool EclipseInterface<dim>::ReadTensorProperty( std::ifstream& ifs, char* text_line, size_t line_length,
                      std::vector<csmp::TensorVariable<dim> >& prop_data,
...
template<size_t dim>
void EclipseInterface<dim>::SaveVectorProperty( size_t component,
                      const std::vector<csmp::ScalarVariable>& scalar_data ,
                      std::vector<csmp::VectorVariable<dim> >&  vector_data )
 {
...
template<size_t dim>
void EclipseInterface<dim>::SaveTensorProperty( size_t component,
                      const std::vector<csmp::ScalarVariable>& scalar_data ,
                      std::vector<csmp::TensorVariable<dim> >&  tensor_data )
 {
    const size_t data_size( scalar_data.size() );
    if( component == 0 )
...
template<size_t dim>
void EclipseInterface<dim>::AssignGridDimensions()
{
...
*/
template<size_t dim>
bool EclipseInterface<dim>::Read_COORD( std::ifstream& ifs, char* text_line, size_t line_length )
 {
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );

    if ( csmp_error.Verbose() ) std::cout <<"\nEclipseInterface<dim>::Read_COORD: reading COORD...\n";
    assert( NX_ > 0 );
    assert( NZ_ > 0 );
    assert( NY_ > 0 );
...
*/
template<size_t dim>
bool EclipseInterface<dim>::Read_ZCORN( std::ifstream& ifs, char* text_line, size_t line_length )
 {
    csmp::ErrorHandler& csmp_error( csmp::ErrorHandler::Instance() );

    if ( csmp_error.Verbose() ) std::cout <<"\nEclipseInterface<dim>::Read_ZCORN: reading ZCORN...\n";
    assert( NX_ > 0 );
    assert( NZ_ > 0 );
    assert( NY_ > 0 );
...
template class EclipseInterface<1U>;
template class EclipseInterface<2U>;
template class EclipseInterface<3U>;
