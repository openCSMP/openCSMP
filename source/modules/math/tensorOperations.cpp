...
double magnitude( const PLACE<dim>& site, const csmp::Index& prop_key )
 {
    ...
    csmp_error.Note( ERROR, "propertyMagnitude:", parseType(prop_key.type).c_str(),
                      "method for calculating magnitude of this variable type undefined; returning NaN.");

    return std::numeric_limits<double>::signaling_NaN();
 }

template double magnitude( const Region<3>&, const csmp::Index& );
template double magnitude( const Element<3>&, const csmp::Index& );
template double magnitude( const Node<3>&, const csmp::Index& );

template double magnitude( const Region<2>&, const csmp::Index& );
template double magnitude( const Element<2>&, const csmp::Index& );
template double magnitude( const Node<2>&, const csmp::Index& );
...
*/
double tensorProjectionOnPlane( const TensorVariable<3U>& ts,
                                  const Point<3U>& un,
                                  double& component_n )
 {
    // Cauchy's formula applied to find traction vector components, P&F, p. 213
    // VectorVariable<3U> t = ts * un.Coordinates();
    const double  tx = ts(0,0) * un[0] + ts(1,0) * un[1] + ts(2,0) * un[2];
    const double  ty = ts(0,1) * un[0] + ts(1,1) * un[1] + ts(2,1) * un[2];
    const double  tz = ts(0,2) * un[0] + ts(1,2) * un[1] + ts(2,2) * un[2];
    ...
    const double  vpx = ((1. - un[0] * un[0]) * tx - un[0] * un[1] * ty - un[0] * un[2] * tz);  // * ex;
    const double  vpy = (-un[0] * un[1] * tx + (1. - un[1] * un[1]) * ty - un[1] * un[2] * tz); // * ey;
    const double  vpz = (-un[2] * un[0] * tx - un[2] * un[1] * ty + (1. - un[2] * un[2]) * tz); // * ez;
...
double tangentialTensorProjection( const TensorVariable<3U>& ts, const Point<3U>& un, Point<3U>& tc )
 {
    // Cauchy's formula applied to find traction vector components, P&F, p. 213
    // VectorVariable<3U> t = ts * un.Coordinates();
    const double  tx = ts(0,0) * un[0] + ts(1,0) * un[1] + ts(2,0) * un[2];
