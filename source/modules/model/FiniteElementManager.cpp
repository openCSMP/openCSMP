...
FiniteElement*  FiniteElementManager::E( int8_t e_type ) const
 {
    if ( dimensions == 3 ) {
	     if ( hexa_ptr != nullptr && e_type == hexa_ptr->ElementType() ) return hexa_ptr;
	     if ( pyra_ptr != nullptr && e_type == pyra_ptr->ElementType() ) return pyra_ptr;
	     if ( pris_ptr != nullptr && e_type == pris_ptr->ElementType() ) return pris_ptr;
	     if ( tetr_ptr != nullptr && e_type == tetr_ptr->ElementType() ) return tetr_ptr;
      }
    if ( quad_ptr != nullptr && e_type == quad_ptr->ElementType() ) return quad_ptr;
    if ( tria_ptr != nullptr && e_type == tria_ptr->ElementType() ) return tria_ptr;
    if ( line_ptr != nullptr && e_type == line_ptr->ElementType() ) return line_ptr;
...
void FiniteElementManager::InitializeElements( uint32_t dim, uint32_t interpolation_order, bool isoparametric )
 {
    dimensions = dim;
    mixed_element_formulation = false;
