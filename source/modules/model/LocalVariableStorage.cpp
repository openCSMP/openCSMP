...
void LocalVariableStorage<dim,STOREE>::AddProperty( const csmp::Index& prop_key )
  {
    ...
    const std::pair<size_t,size_t> newTotaDataDepth = lvsCompileTimeDispatch::containerTotalDataDepth( storeePtr, prop_key );
    ...
    // Offset
    const std::pair<size_t,size_t> newOffset = lvsCompileTimeDispatch::containerOffset( storeePtr, prop_key );
...
void LocalVariableStorage<dim,STOREE>::OutLVS() const
  {
    ...
    if ( data_.arrays > 0U ) {
        ...
        const size_t  array_variables( data_.arrays );
        for ( size_t i=0U; i<array_variables; ++i )
          {
    ...
    if ( data_.flaggedArrays > 0U ) {
        std::cout <<"\n\n\tstored array variables: ";
        const size_t  flaggged_array_variables( data_.flaggedArrays );
        for ( size_t i=0U; i<flaggged_array_variables; ++i )
          {
