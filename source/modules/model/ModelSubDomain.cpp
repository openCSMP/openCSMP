...
/**
    ...
    @attention if this is non-unique region, it may contain lower-dimensional regions, but unique regions
    normally consist only of volume, surface or line elements
    
    @note only model subdomains of the same cell types are considered;
    therefore, iif this is a region, (lower-dim) boundaries or split-boundaries are ignored.
    However, since Elements are disconnected across split-boundaries, such duplicated nodes are considered as
    perimeter cells.
...
void  ModelSubDomain<dim,CELL>::IdentifyPerimeter()
 {
    ...
    // 3. SPLIT_BOUNDARY case: same as above but for lower-dim region
    // --------------------------------------------------------------
    if constexpr ( is_same<CELL<dim>,InterFace<dim> >::value ) {
        const size_t first_bd_elmt = PartitionCellVectorForSplitBoundary();
        assert( first_bd_elmt <= cell_vec_.size() );
      }
...
size_t  ModelSubDomain<dim,CELL>::PartitionCellVector()
 {
    ...
    // -----------------------------------------------------------------
    // 1. distinguishing perimter from interior cells, same for nodes
...
cout.flush();
         ...
         // 1.3.2 finding lesser dimensional cells on the region boundary
...
assert( elmts_with_bfaces.size() == boundary_elmts.size() );
    ...
    // --------------------------------------------------
    if constexpr ( is_same<CELL<dim>,InterFace<dim> >::value == false )
      {
        assert( node_vec_.size() >= boundary_nodes.size() );
        // however, are there any manifold nodes that must be boundary nodes but were overlooked?
        for ( auto& nit : node_vec_ )
          if ( nit->IsManifold() )
            boundary_nodes.insert( nit );
        
        // rebuilding and sorting the node vector (noting that set nodes are already sorted)
        // ---------------------------------------------------------------------------------
        // partition: interior first, perimeter last (stable for interior order)
        auto split = stable_partition(
            node_vec_.begin(),
            node_vec_.end(),
            [&]( csmp::Node<dim>* n )
            {
                assert(n);
                return boundary_nodes.find(n) == boundary_nodes.end(); // true = interior
            }
        );

        // now sort only the perimeter part (the interior part is already sorted)
        sort(split, node_vec_.end());

        // (optional but useful) record first boundary index
        first_bd_node_ = static_cast<size_t>(distance(node_vec_.begin(), split));
...
/* OLD NODEVEC CREATION

        // rebuilding and sorting the node vector (noting that set nodes are already sorted)
        // ---------------------------------------------------------------------------------
        sort( node_vec_.begin(), node_vec_.end() ); // may already be sorted in which case this algo costs nothing
...
size_t  ModelSubDomain<dim,CELL>::PartitionCellVectorForBoundary()
 {
    ...
    // -----------------------------------------------------------------
    // 1. distinguishing perimeter from interior cells, same for nodes
    //   (at this point the cells and nodes are already known)
    // -----------------------------------------------------------------
    ...
    // 1.1 Assuming that all cells have the same spatial dimension
    // -----------------------------------------------------------
      {
        for ( auto& eit : cell_vec_ )
          {
...
template<uint32_t dim, template<uint32_t> class CELL>
size_t  ModelSubDomain<dim,CELL>::PartitionCellVectorForSplitBoundary()
 {
    ErrorHandler&  csmp_error(ErrorHandler::Instance());
        ...
        for ( auto& eit : cell_vec_ )
          {
            if constexpr( dim == 2u ) {
                 if ( !eit->IsLine() )
                   throw logic_error( (string("ModelSubDomain<dim>::PartitionCellVectorForSplitBoundary: line elements required for a 2D-split-boundary: ") + Name()).c_str() );
              }
            else if constexpr( dim == 3u ) {
                 // in 3D models, boundaries must consist out of surface or line elements
                 if ( eit->IsVolume() )
                   throw logic_error( (string("ModelSubDomain<dim>::PartitionCellVectorForSplitBoundary: surface elements required for a 3D-split boundary: ") + Name()).c_str() );
              }
            // identifying the perimeter interfaces
            // (each interface potentially has a neighbor cell)
            auto nbors_that_belong_to_group{ eit->Neighbors() };
            const auto n_faces{ eit->Faces() };
            for ( uint32_t i{0U}; i<n_faces; i++ )
              // if the interface is at a model boundary or has a neighbor that does not belong to the region
              if ( !eit->Neighbor(i) || !binary_search( cell_vec_.begin(), cell_vec_.end(), eit->Neighbor(i)) )
                {
                  perimeter_faces.insert( make_pair( eit, i ) );
                  assert( eit->FE() != nullptr );
    ...
    // --------------------------------------------------
    // 4. SplitBoundary objects do not have a node vector
    // --------------------------------------------------
    first_bd_node_ = 0;
 ...
 } // end PartitionCellVectorForSplitBoundary
...
/**
    ...
    @todo replace with Eigen:: functionality
*/
                                                   ...
                                                   SUBDOMAIN_PART sdp )
 {
     ...
     if ( sdp == COMPLETE ) {
           if ( prop_key.place == ELEMENT or prop_key.place == FACE or prop_key.place == INTER_FACE ) {
             ...
             for ( auto& it : cell_vec_ )
               for ( uint32_t i{0U}; i<it->Facets(); i++ )
     ...
     else { // INTERIOR: property is assigned only to interior of subdomain
           if ( prop_key.place == ELEMENT or prop_key.place == FACE or prop_key.place == INTER_FACE ) {
                for ( auto it=CellsBegin(); it!=PerimeterCellsBegin(); ++it ) (*it)->Store( prop_key, var );
             }
           else if ( prop_key.place == ELEMENT_INTEGRATION_POINT || prop_key.place == FACE_INTEGRATION_POINT || prop_key.place == INTER_FACE_INTEGRATION_POINT ) {
    ...
    if ( prop_key.type == VECTOR || prop_key.type == TENSOR || prop_key.type == FLAGGEDARRAY )
    {
        ...
        if ( group_flag == COMPLETE ) {
            if ( prop_key.place == ELEMENT or
                ...
                for ( const auto& eit : cell_vec_ )
                  if ( status0 != eit->Status(prop_key,component) ) return ANY;
                return status0;
            }
            if ( prop_key.place == ELEMENT_INTEGRATION_POINT ) {
            ...
            if ( prop_key.place == NODE ) {
                ...
                for ( const auto& nit : node_vec_ )
                  if ( status0 != nit->Status(prop_key,component) ) return ANY;
                return status0;
            }
        }
        // if the property shall only be changed on the Subdomain boundary
    ...
    if ( prop_key.type == VECTOR || prop_key.type == TENSOR || prop_key.type == FLAGGEDARRAY )
      {
          ...
          if ( group_flag == COMPLETE ) {
               if ( prop_key.place == ELEMENT or
                    ...
                    prop_key.place == INTER_FACE ) {
                    for ( auto& eit : cell_vec_ )
                      for ( uint32_t n=0U; n<status.size(); n++ )
...
/**
       The method can be applied to scalars, vectors, tensors, array and flagged arays, but not to node variables in SplitBoundary objects
       
       @todo make more efficient using new ReadArray() and other functions.
*/
template<uint32_t dim, template<uint32_t> class CELL>
void ModelSubDomain<dim,CELL>::ChangePropertyStatusWhere( const char* property,
                                                          uint32_t position,
                  ...
                  for ( uint32_t i{0U}; i<eit->IntegrationPoints(); i++ )
                    {
                       ...
                       for ( uint32_t j{1U}; j<eit->Nodes(); j++ ) sc += IPOL[j] * eit->N(j)->Read( n_key );
         ...
         for ( auto& eit : cell_vec_ )
             {
                 ...
                 for ( uint32_t i{0U}; i<eit->FV()->Facets(); i++ )
                   for ( uint32_t j{0U}; j<eit->FV()->IntegrationPointsPerFacet(); j++ )
...
void  ModelSubDomain<dim,CELL>::ExtrapolateIntegrationPointToNodeProperty( const char* cprop, const char* nprop )
 {
    ...
    if ( n_key.type == SCALAR ) {
         ...
         for ( auto& eit : cell_vec_ ) {
              ...
              for ( uint32_t i=0U; i<eit->IntegrationPoints(); i++ ) cp_var[i] = eit->Read( i, c_key );
         ...
         for ( auto& it : cell_vec_ ) {
              ...
              for ( uint32_t i{0U}; i<it->IntegrationPoints(); i++ )
                for ( uint32_t j{0U}; j<dim; j++ )
                  for ( uint32_t k{0U}; k<dim; k++ ) cp_var[ i * tcomponents + j * dim + k ] = cpts[i](j,k);
              // extrapolating constraint point property to nodes
            ...
            if ( idx.type == TENSOR ) {
                 ...
                 for ( typename vector<CELL<dim>*>::const_iterator
                   ...
                   for ( uint32_t i{0U}; i<(*eit)->IntegrationPoints(); i++ ) {
                        ...
                        ts.EigenValues( evals );
                        double ts_avg(0.);
                        for ( uint32_t j{0U}; j<dim; j++ ) ts_avg += evals[j];
                        avg += ts_avg / static_cast<double>(dim);
                     }
                 return avg / static_cast<double>(counter);
              }
...
template<uint32_t dim, template<uint32_t> class CELL>
Point<dim> ModelSubDomain<dim,CELL>::AverageUnitNormal() const
 {
     ErrorHandler& csmp_error( ErrorHandler::Instance() );
     Point<dim>    unrml; unrml = 0.;
     
     if constexpr ( dim == 3 ) {
         for ( const auto& it : cell_vec_ ) {
             if ( !it->IsSurface() ) {
                   csmp_error.Note( ERROR, "ModelSubDomain<3,CELL>::AverageUnitNormal",
                                    Name(), "is not a lower (2-dimensional) subdomain, nothing was done...");
                  return Point<dim>( numeric_limits<double>::quiet_NaN(), numeric_limits<double>::quiet_NaN(), numeric_limits<double>::quiet_NaN() );
               }
             unrml += it->UnitNormal();
          }
      }
         ...
         for ( const auto& it : cell_vec_ ) {
             if ( !it->IsLine() ) {
                   csmp_error.Note( ERROR, "ModelSubDomain<2,CELL>::AverageUnitNormal",
                                    Name(), "is not a lower (1-dimensional) subdomain, nothing was done...");
                   return Point<dim>( numeric_limits<double>::quiet_NaN(), numeric_limits<double>::quiet_NaN() );
               }
             unrml += it->UnitNormal();
          }
       }
     else return Point<1>(1.); // 1d unit normal aligned with x+

     unrml /= static_cast<double>(cell_vec_.size());
     return unrml;
 }
...
size_t ModelSubDomain<dim,CELL>::SharedPerimeterNodes( typename vector<csmp::Node<dim>*>::const_iterator start,
                                                       typename vector<csmp::Node<dim>*>::const_iterator end ) const
 {
    ...
    while ( first1 != last1 && start != end )
      {
        if (*first1 < *start) {
              ++first1;
          }
        else if (*start < *first1) {
              ++start;
          }
        else {                // equal → shared node
            ++shared_nodes;
            ++first1;
            ++start;
         }
...
PLACEMENT modelSubdomainType( const std::string& subdomain_name ) noexcept
  {
     ...
     lowercase_name.reserve(subdomain_name.size());
     for ( auto& it : subdomain_name )
       lowercase_name += static_cast<char>(tolower( it ));
