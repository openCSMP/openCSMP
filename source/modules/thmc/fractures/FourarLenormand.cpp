...
template<size_t dim>
double64 FourarLenormand<dim>::krw_Phase() const
{
...
template<size_t dim>
double64 FourarLenormand<dim>::krn_Phase() const
{
  double64 se = TwoPhaseModel<dim>::seff_;
  if ( se <= 0.) return static_cast<double64>(1.);
...
template<size_t dim>
double64 FourarLenormand<dim>::dkrwds_Phase() const
{
  double64 se = TwoPhaseModel<dim>::seff_;
...
template<size_t dim>
double64 FourarLenormand<dim>::dkrnds_Phase() const
{
  double64 se = TwoPhaseModel<dim>::seff_;
  const double64 seff_mult( 1.0/ (1.0 - TwoPhaseModel<dim>::swr_ - TwoPhaseModel<dim>::snr_ ) );
...
template<size_t dim>
double64 FourarLenormand<dim>::pc_Phase(  ) const
{
  double64 se = TwoPhaseModel<dim>::seff_;
  ...
  /*
    double64 se = TwoPhaseModel<dim>::seff;
    double64 pcEntry = 0.001 * ( 4 * 75 * cos(45)  / fractureAperture_  ); //for single fracture, water-air in [Pa]
    double64 pcMax = pcEntry * 10;
    double64 parameter = 2.;
...
//dummy as above
template<size_t dim>
double64 FourarLenormand<dim>::dpcds_Phase( ) const
{
  double64 se = TwoPhaseModel<dim>::seff_;
...
template<size_t dim>
double64 FourarLenormand<dim>::Sw_Phase( double64 pc ) const
{
    // not unique solution
...
template<size_t dim>
double64 FourarLenormand<dim>::dsdpc_Phase( double64 pc ) const
{
    return 0.0;
}
