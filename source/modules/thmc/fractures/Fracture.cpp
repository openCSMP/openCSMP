...
//Causes displacements normal to the fracture plan to open up by half the aperture on both sides.
//Maintains the flag status of the VectorVariables.
template<uint32_t dim>
  ...
  for (typename std::vector<InterFace<dim>*>::iterator it = sb_ref_.ElementsBegin(); it != sb_ref_.ElementsEnd(); ++it){
    for (size_t i = 0; i < (*it)->Nodes() ; ++i){
      if ((*it)->N(i,MIDDLE) == n_ptr){
        ...
        (*it)->N(i,OUTSIDE)->Store(d_key, disp_out );
      }
    }
  }
}
...
template<uint32_t dim>
double Fracture<dim>::FluidLag( BOX_BOUNDARY side_of_tip, const char* pressure){
  Index p_key = model_->Database().StorageKey(pressure);
  std::map<Point<dim>,Node<dim>*> nodemap = NodeMap(MIDDLE);
  double p_at_start =  nodemap.begin()->second->Read(p_key);
  assert(p_at_start != 0.0);
  Node<dim>* n_negative = nullptr;
  for (typename std::map<Point<dim>,Node<dim>*>::const_iterator it = nodemap.begin(); it != nodemap.end(); ++it){
    if ( it->second->Read( p_key) * p_at_start < 0.0  ){
      if ( it->second->Read(p_key) < 0.0 ){
        n_negative = it->second;
        break;
      } else {
          --it;
          assert( it->second->Read(p_key) < 0.0);
          n_negative = it->second;
          break;
      }
    }
  ...
  if (n_negative == nullptr)
    return 0.0; //no front found

  double fluid_lag_length = DistanceFromTip(nodemap, n_negative, side_of_tip);

  return fluid_lag_length;
}//end of fluid lag


template<uint32_t dim>
void Fracture<dim>::SetToCohesiveTips(){
  tiptype_ = COHESIVE_TIP;
}//end of SetToCohesiveTips

template<uint32_t dim>
void Fracture<dim>::SetToDC_Tips(){
  tiptype_ = DC_TIP;
}

template<uint32_t dim>
void Fracture<dim>::SetToHydraulicAsymptoteTips(){
  tiptype_ = HF_TIP;

}//end of SetToHydraulicAsymptoteTips
...
template<uint32_t dim>
double Fracture<dim>::MaxAperture(const char* aperture, INTERFACE_SIDE side){

  Index ap_key =  model_->Database().StorageKey(aperture);
