    ...
    application_cycle_(1)
{
    ...
    if ( conductivity_key_.type != SCALAR || conductivity_key_.place != ELEMENT)
        err_handler_.notice( FATAL_ERROR, "SinglePhaseVelocityVisitor::(constructor)",
...
//    model.Region("Model").E
    for ( auto e = model.Region("Model").ElementsBegin() ; e != model.Region("Model").ElementsEnd(); e++ ){
        ...
        {
            err_handler_.notice(FATAL_ERROR,"SinglePhaseVelocityVisitor, constructor","You have lower dimensional elements in your model",
...
void SinglePhaseVelocityVisitor<dim>::ComputeContribution(Element<dim>* e)
{
    ...
    if ( with_gravity_ ) {
        ...
        if ( rho_key_.place == ELEMENT )
            ...
            err_handler_.notice( FATAL_ERROR, "SinglePhaseVelocityVisitor<dim>::GetOperands",
    ...
    if ( e->Interpolation() == 1U && velo_key_.place == ELEMENT){
        ...
        if (with_gravity_) {
            ...
            if ( e->Interpolation() == 1U && velo_key_.place == ELEMENT){
                ...
                err_handler_.notice(FATAL_ERROR,"SinglePhaseVelocityVisitor<dim>::AddGravitationalComponentToVelocity( Element<dim>)",
        ...
        err_handler_.notice(FATAL_ERROR,"SinglePhaseVelocityVisitor<dim>::ComputeVelocity( Element<dim>)",
                            " Interpolation for this element is higher than 1, or velocity is not placed on the element",
                            " Whatever functionality you are attempting to use probably needs implementation.");
    }
}
