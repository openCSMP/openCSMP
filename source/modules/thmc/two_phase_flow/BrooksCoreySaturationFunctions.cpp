...
template<size_t dim, template<size_t> class USER>
double64 BrooksCoreySaturationFunctions<dim,USER>::krw_at( const Element<dim>* const e, double64 S) const
  {
    //  switch to linear relperm model if lambda = 0
    if ( e->Read(User()->key_bcp) == static_cast<double64>(0.) )
      return EffectiveSaturation_at(e,S);

    // pm2 = lambda, the Brooks-Corey parameter
    return std::pow( EffectiveSaturation_at(e,S), 2. / e->Read(User()->key_bcp) + 3. );
}
...
template<size_t dim, template<size_t> class USER>
double64 BrooksCoreySaturationFunctions<dim,USER>::krn_at( const Element<dim>* const e, double64 S) const
  {
    //  switch to linear relperm model if lambda = 0
    if ( e->Read(User()->key_bcp) == static_cast<double64>(0.) )
      return 1. - EffectiveSaturation_at(e,S);

    const double64  seffn(1. - EffectiveSaturation_at(e,S));
    return (seffn * seffn) * (1. - pow(  EffectiveSaturation_at(e,S), 2./ e->Read(User()->key_bcp) + 1.) );
}
...
*/
template<size_t dim, template<size_t> class USER>
double64 BrooksCoreySaturationFunctions<dim,USER>::dkrwds_at( const Element<dim>* const e, double64 S) const
  {
    const double64 seff_mult( 1./ (1. - e->Read(User()->key_srH2O) - e->Read(User()->key_srCO2)) );
    //  switch to linear relperm model if lambda = 0
    if ( e->Read(User()->key_bcp) == 0. ) return seff_mult;
    ...
    return ( 2./ e->Read(User()->key_bcp) + 3. ) *
           std::pow( EffectiveSaturation_at(e,S), 2./ e->Read(User()->key_bcp) + 2. ) * seff_mult;
}
...
*/
template<size_t dim, template<size_t> class USER>
double64 BrooksCoreySaturationFunctions<dim,USER>::dkrnds_at( const Element<dim>* const e, double64 S ) const
  {
    const double64 seff_mult( 1./ (1. - e->Read(User()->key_srH2O) - e->Read(User()->key_srCO2)) );
    ...
    const double64  seff(EffectiveSaturation_at(e,S));
    const double64  bcp(e->Read(User()->key_bcp));
    return 2.*(seff - 1.) * (1. + pow(seff, 2./bcp) * ( 1./bcp + 1./2. - seff * (1./bcp + 3./2.))) * seff_mult;
