...
template<size_t dim>
FlowFunctions<dim>::FlowFunctions( const PropertyDatabase<dim>& database,
   ...
   tensor_permeability_(false)
 {
    ...
    if ( perm_key_.place != ELEMENT || (perm_key_.type != SCALAR && perm_key_.type != TENSOR) )
      throw csmp::Exception( FATAL_ERROR, "FlowFunctions(constructor)",
...
template<size_t dim>
double64 csmp::FlowFunctions<dim>::Swr() const
  {
    return swr_;
  }

template<size_t dim>
double csmp::FlowFunctions<dim>::Snr() const
  {
    return snr_;
  }
...
template<size_t dim>
double64 FlowFunctions<dim>::Permeability() const
{ return k_; }

template<size_t dim>
TensorVariable<dim> FlowFunctions<dim>::TensorPermeability() const
  { return K_; }

template<size_t dim>
double64 FlowFunctions<dim>::ViscosityNonWettingPhase() const
{ return mun_; }

template<size_t dim>
double64 FlowFunctions<dim>::ViscosityWettingPhase() const
{ return muw_; }

template<size_t dim>
double64 FlowFunctions<dim>::DensityNonWettingPhase() const
{ return rhn_; }

template<size_t dim>
double64 FlowFunctions<dim>::DensityWettingPhase() const
{ return rhw_; }

template<size_t dim>
double64 FlowFunctions<dim>::MaxCapillaryPressure( size_t phase ) const
{ 
...
template<size_t dim>
double64 FlowFunctions<dim>::Saturation( size_t phase ) const
{ 
   assert( phase == 1U or phase == 2U );
...
template<size_t dim>
double64 FlowFunctions<dim>::MobilityPhase( size_t phase ) const 
 {
    assert( phase == 1U or phase == 2U );
...
template<size_t dim>
double64 FlowFunctions<dim>::TotalMobilityMultiplier() const 
 {
    return krn_Phase() / mun_ + krw_Phase() / muw_;
 }

template<size_t dim>
double64 FlowFunctions<dim>::TotalMobility() const 
 {
    return k_ * TotalMobilityMultiplier();
 }

template<size_t dim>
double64 FlowFunctions<dim>::ViscosityRatio() const
{
  return mun_ / muw_;
}
...
*/
template<size_t dim>
void FlowFunctions<dim>::Initialize( const Element<dim>& e )
 {
...
template<size_t dim>
void FlowFunctions<dim>::InitializeForBaryCenter( const Element<dim>& e )
 {
    if (sw_ro_mu_placement_) {
...
template<size_t dim>
void FlowFunctions<dim>::InitializeForNode( const Element<dim>& e,
                                                   size_t fem_node )
 {
    if (sw_ro_mu_placement_) {
...
template<size_t dim>
void FlowFunctions<dim>::InitializeForIntegrationPoint( size_t ip, 
                                                               const Element<dim>& e )
 {
    if (sw_ro_mu_placement_) {
...
template<size_t dim>
void FlowFunctions<dim>::InitializeForFacetIntegrationPoint( size_t facet, size_t ip,
                                                                    const Element<dim>& e )
 {
    if (sw_ro_mu_placement_) {
...
template<size_t dim>
void FlowFunctions<dim>::InitializeForSectorIntegrationPoint( size_t sector, size_t ip,
                                                                     const Element<dim>& e )
 {
    if (sw_ro_mu_placement_) {
...
*/
template<size_t dim>
double64 FlowFunctions<dim>::EffectiveSaturation() const 
 {
...
template<size_t dim>
double64 FlowFunctions<dim>::SeffToSw() const
 {
...
template<size_t dim>
double64 FlowFunctions<dim>::SeffToSw( double64 seff ) const
 {
...
*/
template<size_t dim>
double64 FlowFunctions<dim>::f_Phase( size_t phase ) const
 {
    assert( phase == 1U or phase == 2U );
      if ( phase == 1U )
 ...
 */
template<size_t dim>
double64 FlowFunctions<dim>::G() const
 {
...
*/
template<size_t dim>
double64 FlowFunctions<dim>::DiffusionMultiplier( size_t phase ) const
{
     assert( phase == 1U or phase == 2U );
...
*/
template<size_t dim>
double64 FlowFunctions<dim>::CapillaryDiffusionMultiplier( ) const
{
...
*/
template<size_t dim>
double64 FlowFunctions<dim>::AdvectionMultiplier( ) const
 {
    return dfds();
 } 

template<size_t dim>
double64 FlowFunctions<dim>::GravityTerm() const
{
...
*/
template<size_t dim>
double64 FlowFunctions<dim>::GravityMultiplier_G( ) const
{
  // note that the projected gravity acts opposite the y-axis
  const double64 k_g_drho = k_ * -acc_gravity_ * (rhw_ - rhn_);
...
*/
template<size_t dim>
double64 FlowFunctions<dim>::GravityMultiplier_dGds( ) const
{
...
template<size_t dim>
csmp::Index FlowFunctions<dim>::WettingPhaseSaturationKey() const
{
  return sat_key_;
}

template<size_t dim>
csmp::Index FlowFunctions<dim>::NonWettingPhaseSaturationKey() const
{
    throw csmp::Exception( ERROR, "FlowFunctions<dim>::NonWettingPhaseSaturationKey()",
                           "It appears someone decided this should be the irreducible non-wet. phase saturation. Wrong." );
    return snr_key_;
}

template<size_t dim>
csmp::Index FlowFunctions<dim>::TotalMobilityKey() const
{
  return lt_key_;
}
...
template<size_t dim>
void FlowFunctions<dim>::InterpolateNodeProperties( const Element<dim>& e )
 {
...
template<size_t dim>
double64 FlowFunctions<dim>::krw_Phase() const
 {
    cout <<"\nFlowFunctions<dim>::krw_Phase (base class): ";
    cout <<"This method needs to be defined in this subclass to achieve desired functionality."<< endl;
    throw logic_error("FlowFunctions<dim>::krw_Phase: Method not defined in subclass");
...
template<size_t dim>
double64 FlowFunctions<dim>::krn_Phase() const
 {
    cout <<"\nFlowFunctions<"<< dim <<">::krn_Phase (base class): ";
    cout <<"This method needs to be defined in this subclass to achieve desired functionality."<< endl;
    throw logic_error("FlowFunctions<dim>::krn_Phase: Method not defined in subclass");
    return std::numeric_limits<double64>::quiet_NaN();
 }

/// derivatives of relative permeabilities
template<size_t dim>
double64 FlowFunctions<dim>::dkrwds_Phase() const
 {
...
template<size_t dim>
double64 FlowFunctions<dim>::dkrnds_Phase() const
 {
...
/// capillary pressure
template<size_t dim>
double64 FlowFunctions<dim>::pc_Phase() const
 {
    cout <<"\nFlowFunctions<"<< dim <<">::pc_Phase (base class): ";
    cout <<"This method needs to be defined in this subclass to achieve desired functionality."<< endl;
    throw logic_error("FlowFunctions<dim>::pc_Phase: Method not defined in subclass");
    return std::numeric_limits<double64>::quiet_NaN();
 }

/// inverse capillary pressure function
template<size_t dim>
double64 FlowFunctions<dim>::Sw_Phase(double64 ) const
 {
    cout <<"\nFlowFunctions<"<< dim <<">::SwFromPc(base class): ";
    cout <<"This method needs to be defined in this subclass to achieve desired functionality."<< endl;
    throw logic_error("FlowFunctions<dim>::SwFromPc: Method not defined in subclass");
    return std::numeric_limits<double64>::quiet_NaN();
 }

    /// capillary pressure derivatives
template<size_t dim>
double64 FlowFunctions<dim>::dpcds_Phase( ) const
 {
...
template<size_t dim>
double64 FlowFunctions<dim>::dsdpc_Phase( double64 ) const
 {
    cout <<"\nFlowFunctions<"<<  dim <<">::dpcdsw_Phase (base class): ";
    cout <<"This method needs to be defined in this subclass to achieve desired functionality."<< endl;
    throw logic_error("FlowFunctions<dim>::dpcdsw_Phase: Method not defined in subclass");
    return std::numeric_limits<double64>::quiet_NaN();
 }
...
template<size_t dim>
double64 FlowFunctions<dim>::dlwds() const
 {
...
template<size_t dim>
double64 FlowFunctions<dim>::dlnds() const
 {
...
template<size_t dim>
double64 FlowFunctions<dim>::dfds() const
 {
...
template<size_t dim>
double64 FlowFunctions<dim>::dGds( ) const
 {
...
/// derivative of wetting phase mobility
template<size_t dim>
double64 FlowFunctions<dim>::dlwds_numerical( double64 h ) const
 {
...
/// derivative of non-wetting phase mobility
template<size_t dim>
double64 FlowFunctions<dim>::dlnds_numerical( double64 h) const
 {
...
template<size_t dim>
double64 FlowFunctions<dim>::dkrwds_numerical( double64 h ) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::dkrnds_numerical( double64 h ) const
{
  //if ( seff_ < 0.0 || seff_ > 1.0 )
  //      return static_cast<double64>(0.);
...
template<size_t dim>
double64 FlowFunctions<dim>::dfds_numerical( double64 h) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::dGds_numerical( double64 h ) const
{
  //if ( seff_ < 0.0 || seff_ > 1.0 )
  //      return static_cast<double64>(0.);
...
template<size_t dim>
double64 FlowFunctions<dim>::dpcds_numerical( double64 h) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::mobility_w_at( double64 se ) const
{
  return krw_at( se ) / muw_;
}

template<size_t dim>
double64 FlowFunctions<dim>::mobility_n_at( double64 se ) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::fw_at( double64 se ) const
{
  return 1.0 / ( 1.0 + mobility_n_at( se ) / mobility_w_at( se ) );
}

template<size_t dim>
double64 FlowFunctions<dim>::fn_at( double64 se ) const
{
  return 1.0 / ( 1.0 + mobility_w_at( se ) / mobility_n_at( se ) );
}

template<size_t dim>
double64 FlowFunctions<dim>::krw_at( double64 se) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::krn_at( double64 se) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::dkrwds_at( double64 se) const
{
  const double64 cache = seff_;
  seff_ = se;
...
template<size_t dim>
double64 FlowFunctions<dim>::dkrnds_at( double64 se) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::dfds_at( double64 se ) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::G_at( double64 se ) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::pc_at( double64 se ) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::dpcds_at( double64 se) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::Sw_at( double64 pc ) const
{
  const double64 cache = seff_;
...
template<size_t dim>
double64 FlowFunctions<dim>::dsdpc_at( double64 pc ) const
{
    const double64 cache = seff_;
...
/// Interpolations
template<size_t dim>
double64 FlowFunctions<dim>::spline_value( double64 x, double64 x1, double64 x2, double64 y1, double64 y2, double64 k1, double64 k2) const
{
...
template<size_t dim>
double64 FlowFunctions<dim>::spline_derivative( double64 x, double64 x1, double64 x2, double64 y1, double64 y2, double64 k1, double64 k2) const
{
    const double64 a =  k1*( x2-x1 ) - ( y2 - y1 );
    const double64 b = -k2*( x2-x1 ) + ( y2 - y1 );
    const double64 t = ( x - x1) / ( x2 - x1 );
...
template<size_t dim>
double64 FlowFunctions<dim>::spline_second_derivative( double64 x, double64 x1, double64 x2, double64 y1, double64 y2, double64 k1, double64 k2) const
{
    const double64 a =  k1*( x2-x1 ) - ( y2 - y1 );
    const double64 b = -k2*( x2-x1 ) + ( y2 - y1 );
    const double64 t = ( x - x1) / ( x2 - x1 );
...
template<size_t dim>
void FlowFunctions<dim>::Out( size_t phase ) const
 { 
    assert( phase == 1U or phase == 2U );
    cout <<"\nFlowFunctions<" << dim <<">::Out: ";
