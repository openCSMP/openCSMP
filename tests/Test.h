...
class Test
  {
  public:
    explicit Test( std::ostream* osptr = nullptr );
    Test(const Test&) = delete;
    Test& operator=(const Test&) = delete;
    virtual ~Test()= default;
    virtual void run() = 0;

    [[nodiscard]] long getNumPassed() const;
    [[nodiscard]] long getNumFailed() const;
    [[nodiscard]] const std::ostream* getStream() const;
    void setStream(std::ostream* osptr);
    void setName(const std::string &testName ) { testName_ = testName; }
    [[nodiscard]] std::string getName() const { return testName_; }
    [[nodiscard]] bool hasName() const { return !testName_.empty(); }
    
    void do_succeed();
    [[nodiscard]] long report() const;
    virtual void reset();
