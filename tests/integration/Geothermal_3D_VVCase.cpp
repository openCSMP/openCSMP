...
void Geothermal_3D_VVCase::outputToVTU( Model<3U>& model,string model_name, const list<string>& props, size_t timestep )
  {
...
void Geothermal_3D_VVCase::ComputeMassGravityTerm (Model<3U>& model)
  ...
  void Geothermal_3D_VVCase::run()
  {
    ...
    //string geometry_name ("BoxHalfs3D");
    string geometry_name ("Cube200");
    string regions_name (this->getName()); 
    string config_name (this->getName()); 
    string vars_name (this->getName()+".txt");
    
    //const char* geometry_name ("10x10_mesh"); 
    //const char* regions_name ("GeoTest_3D"); 
    //const char* config_name ("geothermal_3D"); 
    //const char* vars_name  ("geothermal-variables.txt");
    ...
    VelocityAndVolumeFlux<DIM>  velocity( model, "conductivity", "porosity", "fluid pressure", "density liquid", false );
    ...
    VelocityAndVolumeFlux<DIM>  t_velocity( model, "conductivity", "porosity", "fluid pressure", "density liquid", false );
    ...
    double max_time           = 31536000; // 1 year
    double time_increment     = 86400; // 1 day
    ...
    //model.Boundary("TOP").ChangePropertyStatus("fluid pressure", PLAIN);
    model.Boundary("BACK").ChangePropertyStatus("fluid pressure", PLAIN);
    model.Boundary("LEFT").ChangePropertyStatus("fluid pressure", DIRICH);
    model.Boundary("RIGHT").ChangePropertyStatus("fluid pressure", DIRICH);
    ...
    //model.Boundary("BOTTOM").ChangePropertyStatus("fluid density", DIRICH);
    //model.Boundary("BOTTOM").ChangePropertyStatus("enthalpy content liquid", DIRICH);
