...
ImplicitTransport_Test::ImplicitTransport_Test( const char* test_model, const char* test_variables )
 : test_model_(test_model),
   test_variable_file_(test_variables)
 {
 }

ImplicitTransport_Test::~ImplicitTransport_Test()
{
   delete model_ptr_;
}
...
*/
Model<3U>*  ImplicitTransport_Test::CreateModel( const char* ansys_input_data )
 {
    delete model_ptr_;
    model_ptr_ = new ANSYS_Model3D( ansys_input_data, ansys_input_data, "ImplicitTransport_Test-variables.txt" );
...
*/
Model<3U>*  ImplicitTransport_Test::CreateTetrahedralModel()
 {
...
/**
    ...
    // =========================================
*/
Model<3U>*  ImplicitTransport_Test::CreateHexahedralModel()
 {
    delete model_ptr_;
    VSet<3U>  vset;
...
void ImplicitTransport_Test::run()
 {
    ...
    GoverningEquation<3U>  ADE(model_ptr_->Database()); // default form
    ImplicitTransport<3U>  transport( *model_ptr_, "Model", ADE, second_order_in_space );
...
void  ImplicitTransport_Test::AssignFlowProperties()
 {
    ...
    if ( verbose_ ) {
         cout <<"\nImplicitTransport_Test::AssignFlowProperties: 'total velocity' magnitude: "<< velo.Length() <<"\n";
...
*/
void  ImplicitTransport_Test::DivergenceFreeTotalVelocityField( double64 delta_pf )
 {
...
*/
double64 ImplicitTransport_Test::FluxMultiplier( const Element<3U>* const eptr, size_t sector, size_t facet ) const
 {
...
*/
void ImplicitTransport_Test::Test_initializeFiniteVolumeProperties( double64 tolerance_relaxation_factor )
 {
    // output variables
    ...
    if ( verbose_ && fabs(max_value) >= numeric_limits<double64>::epsilon() ) {
         cerr <<"\nImplicitTransport_Test::run: error in flux balance for elements:\n";
...
*/
void ImplicitTransport_Test::TestInteriorFluxBalance( double64 tolerance_relaxation_factor )
 {
...
*/
void ImplicitTransport_Test::TestNoFlowBoundaryFluxBalance( double64 tolerance_relaxation_factor )
 {
    const csmp::Index  pf_key(model_ptr_->Database().StorageKey("fluid pressure"));
    const csmp::Index  flux_key(model_ptr_->Database().StorageKey("facet flux"));
    Region<3U>         model_domain(model_ptr_->Region("Model"));
...
void  ImplicitTransport_Test::TestFlowThroughModel( const char* model, bool prescribed_velocity )
 {
    ...
    ImplicitTransport<3U>  transport( *model_ptr_, "Model", ADE, second_order_in_space );
     ...
     if ( verbose_ ) {
         cout <<"\nImplicitTransport_Test::TestFlowThroughModel: key variable ranges in current model '"<< model <<"'\n";
...
*/
bool  ImplicitTransport_Test::TestForTracerArrival( const char* boundary, double64 threshold_value ) const
 {
