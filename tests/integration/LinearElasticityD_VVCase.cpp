#include "LinearElasticityD_VVCase.h"
...
namespace csmp
  ...
  */
  ...
  void LinearElasticityD_VVCase::run()
    {
      // some constants
      enum{DIM=2};
      const double displacementInX( 0.5 );
      const double relativeToleranceStrain(0.01), relativeToleranceStress(0.1), relativeTolerancePoisson(0.01);
      ...
      // model configuration
      const VectorVariable<DIM> displacementRight( DIRICH, DIRICH, displacementInX, 0. );
      ...
      model.Boundary( "RIGHT" ).InputPropertyValue( "displacement", displacementRight );
      ...
      const double tensileForceInX( sigmaX*heightPost );
      cout << endl << endl;
