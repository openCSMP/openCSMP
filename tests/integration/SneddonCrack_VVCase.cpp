...
void SneddonCrack_VVCase::Verify(std::map<Point<dim>,double>& PressureProfile, bool at_angle, T F , double tol){
        ...
        if (!at_angle){                                                                  //If not anglend use this as verification
          for (typename std::map<Point<dim>,double>::iterator it = PressureProfile.begin(); it != PressureProfile.end(); it++){
            ...
            for (auto it = PressureProfile.begin(); it != --(PressureProfile.end()); ){
                auto it_before = it++;
            ...
            for (typename std::map<Point<dim>,double>::iterator it = PressureProfile.begin(); it != PressureProfile.end(); it++){
                double x = 0.0;
                for (typename std::map<Point<dim>,double>::iterator point_it = it; point_it != mid_i; ){
                  if (it->first[0] < 0) {
