...
namespace csmp {
  ...
  Placement_Test_3D::~Placement_Test_3D() {
    delete _model;
  }
    ...
    floatTolerance(1e-6)
  {
    ...
    ArrayVariable arrayVar(4, 0., ANY);
    // node
    vector<Node<3U>*>::const_iterator nodeIterator;
    for (nodeIterator = region.NodesBegin(); nodeIterator != region.NodesEnd(); nodeIterator++) {
      Point<3U> coord = (*nodeIterator)->Coordinate();
      ...
      arrayVar.Component(2, coord[2]); 
      arrayVar.Component(3, coord.Length());      
      (*nodeIterator)->Store(keyNodeScalar, makeScalar(ANY, nodeScalar));
      (*nodeIterator)->Store(keyNodeVector, makeVector(ANY, ANY, ANY, vx, vy, vz));
      (*nodeIterator)->Store(keyNodeTensor, makeTensor(ANY, ANY, ANY, xx, xy, xz, yx, yy, yz, zx, zy, zz));
      (*nodeIterator)->Store(keyNodeArray, arrayVar); 
    }

    // element
    vector<Element<3U>*>::const_iterator eleIter;
    for (eleIter = region.ElementsBegin(); eleIter != region.ElementsEnd(); eleIter++) {
      //barycenter
      Point<3U> byct = (*eleIter)->BaryCenter();
      ...
      arrayVar.Component(2, byct[2]); 
      arrayVar.Component(3, byct.Length());     
      (*eleIter)->Store(keyElementScalar, makeScalar(ANY, elementScalar));
      (*eleIter)->Store(keyElementVector, makeVector(ANY, ANY, ANY, vx, vy, vz));
      (*eleIter)->Store(keyElementTensor, makeTensor(ANY, ANY, ANY, xx, xy, xz, yx, yy, yz, zx, zy, zz));
      (*eleIter)->Store(keyElementArray, arrayVar);
      //integration points
      for (size_t i(0); i < (*eleIter)->IntegrationPoints(); ++i) {
        Point<3U> igpt = (*eleIter)->IntegrationPoint(i);
        ...
        arrayVar.Component(2, igpt[2]); 
        arrayVar.Component(3, igpt.Length());       
        (*eleIter)->Store(i, keyElementIPScalar, makeScalar(ANY, elementIPScalar));
        (*eleIter)->Store(i, keyElementIPVector, makeVector(ANY, ANY, ANY, vx, vy, vz));
        (*eleIter)->Store(i, keyElementIPTensor, makeTensor(ANY, ANY, ANY, xx, xy, xz, yx, yy, yz, zx, zy, zz));
      ...
      for (size_t i(0); i < (*eleIter)->Sectors(); ++i) {
        for (size_t ip(0); ip < (*eleIter)->IntegrationPointsPerSector(); ip++) {
          Point<3U> ipCoord = (*eleIter)->FV()->SectorIntegrationPoint(i, ip);
          ...
          arrayVar.Component(2, ipCoord[2]); 
          arrayVar.Component(3, ipCoord.Length());      
          (*eleIter)->Store(i, ip, keySectorScalar, makeScalar(ANY, sectorIPScalar));
          (*eleIter)->Store(i, ip, keySectorVector, makeVector(ANY, ANY, ANY, vx, vy, vz));
          (*eleIter)->Store(i, ip, keySectorTensor, makeTensor(ANY, ANY, ANY, xx, xy, xz, yx, yy, yz, zx, zy, zz));
      ...
      for (size_t i(0); i < (*eleIter)->Facets(); ++i) {
        for (size_t ip(0); ip < (*eleIter)->IntegrationPointsPerFacet(); ip++) {
          Point<3U> ipCoord = (*eleIter)->FV()->FacetIntegrationPoint(i, ip);
          ...
          (*eleIter)->Store(i, ip, keyFacetVector, makeVector(ANY, ANY, ANY, vx, vy, vz));
          (*eleIter)->Store(i, ip, keyFacetTensor, makeTensor(ANY, ANY, ANY, xx, xy, xz, yx, yy, yz, zx, zy, zz));
  ...
  void Placement_Test_3D::NodeToElementTest()
  {
    ...
    for (auto eit = region.ElementsBegin(); eit != region.ElementsEnd(); eit++ ) {
      ...
      VectorVariable<3U> bctrVector;
      ePlacement.Obtain(keyVector, bctrVector);
      TensorVariable<3U> bctrTensor;
      ePlacement.Obtain(keyTensor, bctrTensor);
      ArrayVariable bctrArray(4, 0., ANY);
      ePlacement.Obtain(keyArray, bctrArray);

      ScalarVariable scalVal(ANY, 0.);
      ScalarVariable scalTemp;
      VectorVariable<3U> vecVal;
      VectorVariable<3U> vecTemp;
      TensorVariable<3U> tenVal;
      TensorVariable<3U> tenTemp;
      ArrayVariable arrVal(4, 0., ANY);
      ...
      _equal(vecVal[2], bctrVector[2], floatTolerance);

      //tensor
      _equal(tenVal(0, 0), bctrTensor(0, 0), floatTolerance);
      _equal(tenVal(0, 1), bctrTensor(0, 1), floatTolerance);
      _equal(tenVal(0, 2), bctrTensor(0, 2), floatTolerance);
      _equal(tenVal(1, 0), bctrTensor(1, 0), floatTolerance);
      _equal(tenVal(1, 1), bctrTensor(1, 1), floatTolerance);
      _equal(tenVal(1, 2), bctrTensor(1, 2), floatTolerance);
      _equal(tenVal(2, 0), bctrTensor(2, 0), floatTolerance);
      _equal(tenVal(2, 1), bctrTensor(2, 1), floatTolerance);
      _equal(tenVal(2, 2), bctrTensor(2, 2), floatTolerance);      
      ...
      _equal(arrVal[3], bctrArray[3], floatTolerance);
    }
  ...
  //
  void Placement_Test_3D::ElementToElementTest()
  {
    //scalar and vector
    const Region<3U>& region = _model->Region("Model");
  ...
  void Placement_Test_3D::ElementIPToElementTest()
  {
    ...
    for (auto eit = region.ElementsBegin(); eit != region.ElementsEnd(); eit++) {
      ...
      (*eit)->PropertyValueAtBaryCenter(keyScalar, scalVal);
      VectorVariable<3U> vecVal(ANY, ANY, ANY, 0., 0., 0.);
      (*eit)->PropertyValueAtBaryCenter(keyVector, vecVal);
      TensorVariable<3U> tenVal(ANY, ANY, ANY, 0., 0., 0., 0., 0., 0., 0., 0., 0.);
      (*eit)->PropertyValueAtBaryCenter(keyTensor, tenVal);
      ArrayVariable arrVal(4, 0., ANY);
      (*eit)->PropertyValueAtBaryCenter(keyArray, arrVal);
  ...
  void Placement_Test_3D::SectorIPToElementTest()
  {
  ...
  void Placement_Test_3D::FacetIPToElementTest()
  {
    const Region<3U>& region = _model->Region("Model");
  ...
  // target elementIP
  void Placement_Test_3D::NodeToElementIPTest()
  {
    //FE_NODE_TO_EIP
    ...
    for (auto eit = region.ElementsBegin(); eit != region.ElementsEnd(); eit++) {
      ...
      for (auto ip = ipPlacementCollection.begin(); ip != ipPlacementCollection.end(); ip++) {
        ...
        VectorVariable<3U> ipVector;
        (*ip).Obtain(keyVector, ipVector);
        TensorVariable<3U> ipTensor;
        (*ip).Obtain(keyTensor, ipTensor);
        ArrayVariable ipArray(4, 0., ANY);
        ...
        _equal(vecVal[2], ipVector[2], floatTolerance);

        //tensor
        _equal(tenVal(0, 0), ipTensor(0, 0), floatTolerance);
        _equal(tenVal(0, 1), ipTensor(0, 1), floatTolerance);
        _equal(tenVal(0, 2), ipTensor(0, 2), floatTolerance);
        _equal(tenVal(1, 0), ipTensor(1, 0), floatTolerance);
        _equal(tenVal(1, 1), ipTensor(1, 1), floatTolerance);
        _equal(tenVal(1, 2), ipTensor(1, 2), floatTolerance);
        _equal(tenVal(2, 0), ipTensor(2, 0), floatTolerance);
        _equal(tenVal(2, 1), ipTensor(2, 1), floatTolerance);
        _equal(tenVal(2, 2), ipTensor(2, 2), floatTolerance); 

        //array
        _equal(arrVal[0], ipArray[0], floatTolerance);
        _equal(arrVal[1], ipArray[1], floatTolerance);
        _equal(arrVal[2], ipArray[2], floatTolerance);
        _equal(arrVal[3], ipArray[3], floatTolerance);
      }
    }
  }
  ...
  void Placement_Test_3D::ElementToElementIPTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, ELEMENT> keyScalar(_model->Database().StorageKey("elementScalar"));
    csmp::INDEX<VECTOR, ELEMENT> keyVector(_model->Database().StorageKey("elementVector"));
  ...
  // target sectorIP
  void Placement_Test_3D::NodeToSectorIPTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, NODE> keyScalar(_model->Database().StorageKey("nodeScalar"));
    csmp::INDEX<VECTOR, NODE> keyVector(_model->Database().StorageKey("nodeVector"));
    ...
    for (auto eit = region.ElementsBegin(); eit != region.ElementsEnd(); eit++) {
      ...
      for (auto sPlacement = fit.begin(); sPlacement != fit.end(); sPlacement++)
      {
        for (size_t ip(0); ip < (*eit)->IntegrationPointsPerFacet(); ip++)
        {
          ...
          VectorVariable<3U> secVector;
          (*sPlacement).Obtain(keyVector, secVector);
          TensorVariable<3U> secTensor;
          (*sPlacement).Obtain(keyTensor, secTensor);
          ArrayVariable secArray(4, 0., ANY);
          ...
          Point<3U> ipCoord((*eit)->FV()->SectorIntegrationPoint(iSector, ip));
          (*eit)->N_At(ipCoord, NN); 
          ...
          _equal(vecVal[2], secVector[2], floatTolerance);

          //tensor
          _equal(tenVal(0, 0), secTensor(0, 0), floatTolerance);
          _equal(tenVal(0, 1), secTensor(0, 1), floatTolerance);
          _equal(tenVal(0, 2), secTensor(0, 2), floatTolerance);
          _equal(tenVal(1, 0), secTensor(1, 0), floatTolerance);
          _equal(tenVal(1, 1), secTensor(1, 1), floatTolerance);
          _equal(tenVal(1, 2), secTensor(1, 2), floatTolerance);
          _equal(tenVal(2, 0), secTensor(2, 0), floatTolerance);
          _equal(tenVal(2, 1), secTensor(2, 1), floatTolerance);
          _equal(tenVal(2, 2), secTensor(2, 2), floatTolerance); 
          ...
          _equal(arrVal[3], secArray[3], floatTolerance);
  ...
  void Placement_Test_3D::ElementToSectorIPTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, ELEMENT> keyScalar(_model->Database().StorageKey("elementScalar"));
    csmp::INDEX<VECTOR, ELEMENT> keyVector(_model->Database().StorageKey("elementVector"));
  ...
  // target facetIP
  void Placement_Test_3D::NodeToFacetIPTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, NODE> keyScalar(_model->Database().StorageKey("nodeScalar"));
    csmp::INDEX<VECTOR, NODE> keyVector(_model->Database().StorageKey("nodeVector"));
    ...
    for (auto eit = region.ElementsBegin(); eit != region.ElementsEnd(); eit++) {
      ...
      for (auto fPlacement = fit.begin(); fPlacement != fit.end(); fPlacement++)
      {
        for (size_t ip(0); ip < (*eit)->IntegrationPointsPerFacet(); ip++) 
        {
          ...
          VectorVariable<3U> facVector;
          (*fPlacement).Obtain(keyVector, facVector);
          TensorVariable<3U> facTensor;
          (*fPlacement).Obtain(keyTensor, facTensor);
          ArrayVariable facArray(4, 0., ANY);
          ...
          Point<3U> ipCoord((*eit)->FV()->FacetIntegrationPoint(iFacet, ip));
          ...
          _equal(vecVal[2], facVector[2], floatTolerance);

          //tensor
          _equal(tenVal(0, 0), facTensor(0, 0), floatTolerance);
          _equal(tenVal(0, 1), facTensor(0, 1), floatTolerance);
          _equal(tenVal(0, 2), facTensor(0, 2), floatTolerance);
          _equal(tenVal(1, 0), facTensor(1, 0), floatTolerance);
          _equal(tenVal(1, 1), facTensor(1, 1), floatTolerance);
          _equal(tenVal(1, 2), facTensor(1, 2), floatTolerance);
          _equal(tenVal(2, 0), facTensor(2, 0), floatTolerance);
          _equal(tenVal(2, 1), facTensor(2, 1), floatTolerance);
          _equal(tenVal(2, 2), facTensor(2, 2), floatTolerance); 
          ...
          _equal(arrVal[3], facArray[3], floatTolerance);
        }
  ...
  void Placement_Test_3D::ElementToFacetIPTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, ELEMENT> keyScalar(_model->Database().StorageKey("elementScalar"));
    csmp::INDEX<VECTOR, ELEMENT> keyVector(_model->Database().StorageKey("elementVector"));
  ...
  void Placement_Test_3D::ElementIPToElementIPTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, ELEMENT_INTEGRATION_POINT> keyScalar(_model->Database().StorageKey("elementIPScalar"));
    csmp::INDEX<VECTOR, ELEMENT_INTEGRATION_POINT> keyVector(_model->Database().StorageKey("elementIPVector"));
    ...
    for (auto eit = region.ElementsBegin(); eit != region.ElementsEnd(); eit++) {
      ...
      std::vector<VectorVariable<3U>> vecValVector;
      std::vector<TensorVariable<3U>> tenValVector;
      ...
      for (auto ip = ipElementPlacement.begin(); ip != ipElementPlacement.end(); ip++) {
        ...
        VectorVariable<3U> vecVal = vecValVector[ipit];
        TensorVariable<3U> tenVal = tenValVector[ipit];
  ...
  void Placement_Test_3D::FacetIPToFacetIPTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, FACET_INTEGRATION_POINT> keyScalar(_model->Database().StorageKey("facetIPScalar"));
    csmp::INDEX<VECTOR, FACET_INTEGRATION_POINT> keyVector(_model->Database().StorageKey("facetIPVector"));
  ...
  void Placement_Test_3D::SectorIPToSectorIPTest()
  {
  ...
  void Placement_Test_3D::NodeToNodeTest()
  {
    //scalar, vector and tensor
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, NODE> keyScalar(_model->Database().StorageKey("nodeScalar"));
    ...
    for (auto eit = region.ElementsBegin(); eit != region.ElementsEnd(); eit++ ) {
      ...
      {      
        ...
        VectorVariable<3U> ndVector;
        (*nPlacement).Obtain(keyVector, ndVector);
        TensorVariable<3U> ndTensor;
        (*nPlacement).Obtain(keyTensor, ndTensor);
        ArrayVariable ndArray(4, 0., ANY);
        (*nPlacement).Obtain(keyArray, ndArray);


        ScalarVariable scalVal = scalValVector[iNode];
        VectorVariable<3U> vecVal = vecValVector[iNode];
        TensorVariable<3U> tenVal = tenValVector[iNode];
        ...
        _equal(vecVal[2], ndVector[2], floatTolerance);

        //tensor
        _equal(tenVal(0, 0), ndTensor(0, 0), floatTolerance);
        _equal(tenVal(0, 1), ndTensor(0, 1), floatTolerance);
        _equal(tenVal(0, 2), ndTensor(0, 2), floatTolerance);
        _equal(tenVal(1, 0), ndTensor(1, 0), floatTolerance);
        _equal(tenVal(1, 1), ndTensor(1, 1), floatTolerance);
        _equal(tenVal(1, 2), ndTensor(1, 2), floatTolerance);
        _equal(tenVal(2, 0), ndTensor(2, 0), floatTolerance);
        _equal(tenVal(2, 1), ndTensor(2, 1), floatTolerance);
        _equal(tenVal(2, 2), ndTensor(2, 2), floatTolerance);        
        ...
        _equal(arrVal[3], ndArray[3], floatTolerance);
  ...
  void Placement_Test_3D::GradientTest()
  {
    //scalar
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, NODE> keyScalar(_model->Database().StorageKey("nodeScalar"));
  ...
  void Placement_Test_3D::SectorVolumeTest()
  {
    const Region<3U>& region = _model->Region("Model");
  ...
  void Placement_Test_3D::FacetNormalTest()
  {
    const Region<3U>& region = _model->Region("Model");
    ...
    for (auto eit = region.ElementsBegin(); eit != region.ElementsEnd(); eit++) {
      ...
      {
        Point<3U> fn = fip.FacetNormal();
        Point<3U> facet_normal = (*eit)->FacetNormal(iFacet);
        _equal(fn[0], facet_normal[0], floatTolerance);
        _equal(fn[1], facet_normal[1], floatTolerance);
        _equal(fn[2], facet_normal[2], floatTolerance);       
        iFacet++;
       }
     }
   }
  ...
  void Placement_Test_3D::FV_ElementToElementTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, ELEMENT> keyScalar(_model->Database().StorageKey("elementScalar"));
    csmp::INDEX<VECTOR, ELEMENT> keyVector(_model->Database().StorageKey("elementVector"));
  ...
  void Placement_Test_3D::FV_FacetAreaAndNormalTest()
  {
    const Region<3U>& region = _model->Region("Model");
    ...
    for (auto nit = region.NodesBegin(); nit != region.NodesEnd(); nit++) {
      ...
      std::vector<Point<3U>> facetNormalVec;
      ...
      for ( size_t i=0U; i<parent_elements; ++i ) {
         ...
         for ( size_t j=0U; j<eptr->FV()->FacetsPerSector(sector_node); ++j ) {
           ...
           Point<3U> facetNormal = eptr->FacetNormal(facet);
      ...
      for (size_t n=0U;n<vecSize;n++) {
        ...
        Point<3U> fn = fnVec[n];
        Point<3U> facet_normal = facetNormalVec[n];
  ...
  void Placement_Test_3D::FV_ProjectOntoFacetNormalTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<VECTOR, ELEMENT> keyVector(_model->Database().StorageKey("elementVector"));
  ...
  void Placement_Test_3D::FV_SectorVolumeTest()
  {
    const Region<3U>& region = _model->Region("Model");
  ...
  void Placement_Test_3D::FV_InsideOutsideNodeTest()
  {
    const Region<3U>& region = _model->Region("Model");
    for (auto nit = region.NodesBegin(); nit != region.NodesEnd(); nit++) {
  ...
  void Placement_Test_3D::FV_GradientTest()
  {
    const Region<3U>& region = _model->Region("Model");
    csmp::INDEX<SCALAR, NODE> keyScalar(_model->Database().StorageKey("nodeScalar"));
    
    for (auto nit = region.NodesBegin(); nit != region.NodesEnd(); nit++ ) { 
      std::vector<Point<3U>> gradVec;
      for (auto fip : (*nit)->AllFacetIntegrationPoints()) {
        Element<3U>& e = fip.TheElement();
        Point<3U> grad = fip.Gradient(keyScalar);
        gradVec.push_back(grad);
      }
      
      std::vector<Point<3U>> gradientVec;
      const size_t parent_elements((*nit)->Parents());      
      for ( size_t i=0U; i<parent_elements; ++i ) {
         const Element<3U>* eptr = (*nit)->Parent(i);
         const size_t sector_node = (*nit)->ParentNodeNumber(i);
         for ( size_t j=0U; j<eptr->FV()->FacetsPerSector(sector_node); ++j ) {
           ...
           for (size_t ip(0); ip < eptr->IntegrationPointsPerFacet(); ip++) {
             ...
             if (eptr->IsLineElement()) {
               ...
               Point<3U> coord = eptr->FV()->FacetIntegrationPoint(facet, ip);
               eptr->dN_At (coord, DN);
             }
             ...
             for ( size_t j=0U; j<eptr->Nodes(); j++ ) {
               ...
               for ( size_t k=0U; k<3U; k++ ) gradient[k] += DN(k,j) * v;
             }
      ...
      for (size_t n=0U;n<vecSize;n++) {      
        Point<3U> grad = gradVec[n];
        Point<3U> gradient = gradientVec[n];
        _equal(grad[0], gradient[0], floatTolerance);
        _equal(grad[1], gradient[1], floatTolerance);
        _equal(grad[2], gradient[2], floatTolerance);
