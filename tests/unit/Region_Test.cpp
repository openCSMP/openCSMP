...
void Region_Test::run()
{
    ...
    const bool with_VTK_output(false);
    ...
    if ( with_VTK_output ) vtk_output.OutputDataToVTK( model, "fluid-pressure", "fluid pressure", 0 );
    ...
    {
        cout << (*it) <<" ";
        if ( with_VTK_output ) vtk_output.OutputDataToVTK( model, (*it).c_str(), "permeability", "permeability", 1, true );
        count++;
    ...
    if ( with_VTK_output ) vtk_output.OutputDataToVTK( model, "permeability_regions", "permeability_regions", "permeability", 1, true );
    ...
    if ( with_VTK_output ) vtk_output.OutputDataToVTK( model, "pressure_permeability_overlap1",  "pressure_permeability_overlap1","fluid pressure", 1, true );
    ...
    if ( with_VTK_output ) vtk_output.OutputDataToVTK( model, "pf_window", "pf_window", "fluid pressure", 1, true );
    ...
    if ( with_VTK_output ) vtk_output.OutputDataToVTK( model, "rectangular region", "box", "fluid pressure", 1, true );
    ...
    if ( with_VTK_output ) vtk_output.OutputDataToVTK( model, "FRAC_VOLUMES", "volumetric_fractures", "fluid pressure", 1, true );
    ...
    if ( with_VTK_output ) vtk_output.OutputDataToVTK( model, "difference", "fluid-pressure", "fluid pressure", 1, true );
