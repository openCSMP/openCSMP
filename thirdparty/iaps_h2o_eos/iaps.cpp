...
/*
 * Modified by "The openCSMP project", 2026:
 * - Ported from iaps.c to iaps.cpp for C++ compilation
 * - Replaced C headers (float.h, math.h, stdlib.h) with
 *   C++ equivalents (cfloat, cmath, cstdlib)
 * - Use of namespace std
 * - Renamed variable PMIN to P_MIN
 * - Instantiate static const int imax as integer (line 1185)
 */

#include <cfloat>   /* DBL_EPSILON */
#include <cmath>    /* fabs(), sqrt(), pow(), exp(), log() */
...
*/
{
	...
	if (h > 2460.0) {
        ph(P_MIN, h, t, d, dp, dh, mPro, prop);
    } else if (p < P_MIN) {
	    /* h = 2460, p = 1e-6 */
...
*/
{
    ...
    if ((t < 260.0) || (t > 2500.0) || (p < P_MIN) || (p > 3000.0)) {
	return val;
    }
    if (p < 209.9) {
...
*/
{
    ...
    if ((p < P_MIN) || (p > 3000.0) || (h > 9950.0) || (h < 0.95 * p - 20.0)) {
	val = 0;
...
*/
{
    ...
    if ((p < P_MIN) || (p > 3000.0) || (s < 0.00045 * p - 0.28)) {
	val = 0;
