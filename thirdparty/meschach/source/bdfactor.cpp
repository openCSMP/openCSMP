  ...
  BAND *bd_get(int lb,int  ub,int  n)
{
   ...
   lb = A->lb = ms_min(n-1,lb);
   ub = A->ub = ms_min(n-1,ub);
  ...
  BAND *bd_resize(BAND *A,int new_lb, int new_ub, int new_n)
{
   ...
   new_lb = A->lb = ms_min(new_lb,new_n-1);
   new_ub = A->ub = ms_min(new_ub,new_n-1);
  ...
  MAT *band2mat(BAND *bA, MAT *A)
{
   ...
   for (j=0; j < n; j++)
     for (i=ms_min(n1,j+lb),l=lb+j-i; i >= ms_max(0,j-ub); i--,l++)
       A->me[i][j] = bmat[l][j];
  ...
  BAND *bd_transp(BAND *in, BAND *out)
{
   ...
   if (! in_situ) {
	 ...
	 **********************************/
	 ...
	 lbi = lb-i;
	 for (j=l-lb, jj=0, p=ms_max(-lbi,0), pp = ms_max(l-ub,0); j <= n1; 
	      j++,jj++,p++,pp++) {
	    in_v[l][pp] = in_v[i][p];
	    in_v[i][jj] = in_v[l][j];
	 }
	 for (  ; p <= n1-ms_max(lbi,0); p++,pp++)
	   in_v[l][pp] = in_v[i][p];
      }
      
      if (lub%2 == 0) { /* shift only */
	 i = lub/2;
	 for (j=ms_max(i-lb,0), jj=0; jj <= n1-ub+i; j++,jj++) 
	   in_v[i][jj] = in_v[i][j];
      }
   }
      ...
      for (i=0, l=lub; i < (lub+1)/2; i++,l--) {
	 ubi = i-ub;
	 for (j=n1-ms_max(lb-l,0), jj=n1-ms_max(-ubi,0), p=n1-lb+i, pp=n1;
	      p >= 0; j--, jj--, pp--, p--) {
	    in_v[i][jj] = in_v[l][j];
	    in_v[l][pp] = in_v[i][p];
	 }
	 for (  ; jj >= ms_max(ubi,0); j--, jj--)
	   in_v[i][jj] = in_v[l][j];
      }

      if (lub%2 == 0) {  /* shift only */
	 i = lub/2;
	 for (j=n1-lb+i, jj=n1-ms_max(ub-i,0); j >= 0; j--, jj--) 
	    in_v[i][jj] = in_v[i][j];
      }
   }
...
/* bdLUfactor -- gaussian elimination with partial pivoting
   -- on entry, the matrix A in band storage with elements 
      ...
      ms_max(0,j-lb) <= i <= ms_min(A->n-1,j+ub);
  ...
  BAND	*bdLUfactor(BAND *bA, PERM *pivot)
{
   ...
   bA = bd_resize(bA,lb,ms_min(n1,lub),n);
   bA_v = bA->mat->me;
   ...
   for ( k=0; k < n1; k++ )
   {
      k_end = ms_max(0,lb+k-n1);
      k_lub = ms_min(k+lub,n1);
  ...
  VEC	*bdLUsolve(BAND	*bA, PERM *pivot, VEC *b,VEC *x)
{
   ...
   for (i=n-2; i >= 0; i--) {
      c = x->ve[i];
      for (j=ms_min(n1,i+ub), l=lb+j-i; j > i; j--,l--)
	c -= bA_v[l][j]*x->ve[j];
      x->ve[i] = c/bA_v[lb][i];
   }
  ...
  BAND *bdLDLfactor(BAND *A)
{
   ...
   for (k=0; k < n; k++) {    
      ...
      /* matrix D */
      c = Av[lb][k];
      for (j=ms_max(0,-lbkm), jk=lbkm+j; j < k; j++, jk++) {
	 cc = Av[jk][j];
	 c -= Av[lb][j]*cc*cc;
      }
      if (c == 0.0)
	error(E_SING,"bdLDLfactor");
      ...
      for (i=ms_min(n1,lbkp), ki=lbkp-i; i > k; i--,ki++) {
	 c = Av[ki][k];
	 for (j=ms_max(0,i-lb), ji=lb+j-i, jk=lbkm+j; j < k;
	      j++, ji++, jk++)
  ...
  VEC *bdLDLsolve(BAND *A, VEC *b, VEC *x)
{
   ...
   for (i=1; i < n; i++) {
      ilb = i-lb;
      c = b->ve[i];
      for (j=ms_max(0,ilb), l=j-ilb; j < i; j++,l++)
	c -= Av[l][j]*x->ve[j];
      x->ve[i] = c;
   }
   ...
   for (i=n-2; i >= 0; i--) {
      ilb = i+lb;
      c = x->ve[i];
      for (j=ms_min(n1,ilb), l=ilb-j; j > i; j--,l++)
	c -= Av[l][i]*x->ve[j];
      x->ve[i] = c;
   }
  ...
  VEC *bd_mv_mlt(BAND *A, VEC *x, VEC *out)
{
  ...
  for (i=0; i<n; i++, start_idx--, end_idx--) {
    j = ms_max(0, start_idx);
    k = ms_max(0, -start_idx);
    j_end = ms_min(m, end_idx);
    x_ve = x->ve + k;
    sum = 0.0;	     
    for (; j < j_end; j++, k++)
      sum += A_me[j][k] * *x_ve++;
