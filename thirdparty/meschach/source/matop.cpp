...
static	char	rcsid[] = "$Id: matop.c,v 1.1.1.1 2006/08/09 13:57:52 apaluszn Exp $";
...
/* m_mlt -- matrix-matrix multiplication */
  MAT	*m_mlt(MAT *A, MAT *B, MAT *OUT_MESCH)
{
	u_int	i, /* j, */ k, m, n, p;
	Real	**A_v, **B_v /*, *B_row, *OUT_MESCH_row, sum, tmp */;
	...
	if ( OUT_MESCH==(MAT *)NULL || OUT_MESCH->m != A->m || OUT_MESCH->n != B->n )
		OUT_MESCH = m_resize(OUT_MESCH,A->m,B->n);
...
****************************************************************/
	...
	for ( i=0; i<m; i++ )
		...
		{
		    if ( A_v[i][k] != 0.0 )
		        __mltadd__(OUT_MESCH->me[i],B_v[k],A_v[i][k],(int)p);
		    /**************************************************
...
/* mmtr_mlt -- matrix-matrix transposed multiplication
	-- A.B^T is returned, and stored in OUT_MESCH */
  MAT	*mmtr_mlt(MAT *A, MAT *B, MAT *OUT_MESCH)
{
	int	i, j, limit;
	...
	if ( ! OUT_MESCH || OUT_MESCH->m != A->m || OUT_MESCH->n != B->m )
		OUT_MESCH = m_resize(OUT_MESCH,A->m,B->m);

	limit = A->n;
	for ( i = 0; i < A->m; i++ )
		for ( j = 0; j < B->m; j++ )
		{
		    OUT_MESCH->me[i][j] = __ip__(A->me[i],B->me[j],(int)limit);
		    /**************************************************
		    sum = 0.0;
		    A_row = A->me[i];
		    B_row = B->me[j];
...
/* mtrm_mlt -- matrix transposed-matrix multiplication
	-- A^T.B is returned, result stored in OUT_MESCH */
  MAT	*mtrm_mlt(MAT *A, MAT *B, MAT *OUT_MESCH)
{
	int	i, k, limit;
	/* Real	*B_row, *OUT_MESCH_row, multiplier; */
	...
	if ( ! OUT_MESCH || OUT_MESCH->m != A->n || OUT_MESCH->n != B->n )
		OUT_MESCH = m_resize(OUT_MESCH,A->n,B->n);
	...
	for ( k = 0; k < A->m; k++ )
		...
		{
		    if ( A->me[k][i] != 0.0 )
			__mltadd__(OUT_MESCH->me[i],B->me[k],A->me[k][i],(int)limit);
		    /**************************************************
		    multiplier = A->me[k][i];
		    OUT_MESCH_row = OUT_MESCH->me[i];
		    B_row   = B->me[k];
		    for ( j = 0; j < limit; j++ )
			*(OUT_MESCH_row++) += multiplier*(*B_row++);
		    **************************************************/
		}
