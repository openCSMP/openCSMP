...
/* SKM 11/5/98 this seemed to be necessary only in the past CW versions
...
static char rcsid[] = "$Id: matrixio.c,v 1.1.1.1 2006/08/09 13:57:52 apaluszn Exp $";
  ...
  void  m_dump(FILE *fp, MAT *a)
{
	u_int   i, j, tmp;
     ...
     for ( i=0; i<a->m; i++ )   /* for each row... */
     {
      fprintf(fp,"row %u: @ 0x%lx ",i,(long long)(a->me[i]));
	  for ( j=0, tmp=2; j<a->n; j++, tmp++ )
	  {             /* for each col in row... */
	       fprintf(fp,format,a->me[i][j]);
	       if ( ! (tmp % 5) )       putc('\n',fp);
