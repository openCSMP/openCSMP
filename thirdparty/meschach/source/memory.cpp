...
static	char	rcsid[] = "$Id: memory.c,v 1.1.1.1 2006/08/09 13:57:52 apaluszn Exp $";
...
/* m_resize -- returns the matrix A of size new_m x new_n; A is zeroed
  ...
  MAT	*m_resize(MAT *A, u_int new_m, u_int new_n )
{
   int	i;
   ...
   new_max_m = ms_max(new_m,A->max_m);
   new_max_n = ms_max(new_n,A->max_n);
   ...
   if ( old_n > new_n )
   {
      for ( i = 1; i < ms_min(old_m,new_m); i++ )
   ...
   {
      for ( i = (int)(ms_min(old_m,new_m))-1; i > 0; i-- )
...
/* px_resize -- returns the permutation px with size new_size
  ...
  PERM	*px_resize(PERM *px, u_int new_size)
{
   int	i;
...
/* v_resize -- returns the vector x with dim new_dim
  ...
  VEC	*v_resize(VEC *x, u_int new_dim)
{
