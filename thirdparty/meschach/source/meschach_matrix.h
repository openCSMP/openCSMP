...
/* RCS id: $Id: matrix.h,v 1.1.1.1 2006/08/09 13:57:52 apaluszn Exp $ */
...
/* type independent ms_min and ms_max operations */
#ifndef ms_max
...
/* get/resize vector to given dimension */
	VEC *v_get( u_int), *v_resize(VEC *,u_int);
/* get/resize matrix to be m x n */
	MAT *m_get(u_int,u_int), *m_resize(MAT *,u_int,u_int);
/* get/resize permutation to have the given size */
	PERM *px_get(u_int), *px_resize(PERM *,u_int);
/* get/resize an integer vector to given dimension */
	IVEC *iv_get(u_int), *iv_resize(IVEC *,u_int);
/* get/resize a band matrix to given dimension */
  BAND *bd_get(int,int,int), *bd_resize(BAND *,int,int,int);

/* free (de-allocate) (band) matrices, vectors, permutations and 
   integer vectors */
int iv_free(IVEC *), m_free(MAT *), v_free(VEC *), px_free(PERM *);
int bd_free(BAND *);
