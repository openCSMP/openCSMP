  ...
  SPMAT *sp_copy(SPMAT *A)
{
   ...
   for ( i = 0; i < A->m; i++ )
   {
      row1 = &(A->row[i]);
      row2 = &(out->row[i]);
      if ( ! (row2->elt=NEW_A(ms_max(row1->len,3),row_elt)) )
	error(E_MEM,"sp_copy");
      else if (mem_info_is_on()) {
	 mem_bytes(TYPE_SPMAT,0,ms_max(row1->len,3)*sizeof(row_elt));
      }
      row2->len = row1->len;
      row2->maxlen = ms_max(row1->len,3);
      row2->diag = row1->diag;
...
/* sp_copy2 -- copy sparse matrix (type 2) 
   -- keeps structure of the OUT_MESCH matrix */
  SPMAT	*sp_copy2(SPMAT *A, SPMAT *OUT_MESCH)
{
   int	i /* , idx, len1, len2 */;
   SPROW	*r1, *r2;
   ...
   if ( OUT_MESCH->m < A->m )
   {
	 ...
	 mem_bytes(TYPE_SPMAT,A->max_m*sizeof(SPROW),
      ...
      OUT_MESCH->row = RENEW(OUT_MESCH->row,A->m,SPROW);
      if ( ! OUT_MESCH->row )
	error(E_MEM,"sp_copy2");
      
      for ( i = OUT_MESCH->m; i < A->m; i++ )
      {
	 OUT_MESCH->row[i].elt = NEW_A(MINROWLEN,row_elt);
	 if ( ! OUT_MESCH->row[i].elt )
	   error(E_MEM,"sp_copy2");
	 else if (mem_info_is_on()) {
	    mem_bytes(TYPE_SPMAT,0,MINROWLEN*sizeof(row_elt));
	 }
	 
	 OUT_MESCH->row[i].maxlen = MINROWLEN;
	 OUT_MESCH->row[i].len = 0;
      }
      OUT_MESCH->m = A->m;
   }
   
   OUT_MESCH->flag_col = OUT_MESCH->flag_diag = FALSE;
   /* sp_zero(OUT_MESCH); */

   for ( i = 0; i < A->m; i++ )
   {
      r1 = &(A->row[i]);	r2 = &(OUT_MESCH->row[i]);
