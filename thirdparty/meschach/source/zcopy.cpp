...
static	char	rcsid[] = "$Id: zcopy.c,v 1.1.1.1 2006/08/09 13:57:52 apaluszn Exp $";
...
ZMAT	*zm_move(ZMAT *in,int i0,int j0,int m0,int n0,ZMAT *out,int i1,int j1)
{
	...
	out = zm_resize(out,ms_max(out->m,i1+m0),ms_max(out->n,j1+n0));
...
ZMAT	*zvm_move(ZVEC *in,int i0,ZMAT *out,int i1,int j1,int m1,int n1)
{
	...
	out = zm_resize(out,ms_max(i1+m1,out->m),ms_max(j1+n1,out->n));
