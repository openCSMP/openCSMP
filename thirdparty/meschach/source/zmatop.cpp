...
static	char	rcsid[] = "$Id: zmatop.c,v 1.1.1.1 2006/08/09 13:57:52 apaluszn Exp $";
...
ZMAT	*zm_mlt(ZMAT *A,ZMAT *B,ZMAT *OUT_MESCH)
{
    u_int	i, /* j, */ k, m, n, p;
    complex	**A_v, **B_v /*, *B_row, *OUT_MESCH_row, sum, tmp */;
    
    if ( A==ZMNULL || B==ZMNULL )
	error(E_NULL,"zm_mlt");
    if ( A->n != B->m )
	error(E_SIZES,"zm_mlt");
    if ( A == OUT_MESCH || B == OUT_MESCH )
	error(E_INSITU,"zm_mlt");
    m = A->m;	n = A->n;	p = B->n;
    A_v = A->me;		B_v = B->me;
    
    if ( OUT_MESCH==ZMNULL || OUT_MESCH->m != A->m || OUT_MESCH->n != B->n )
	OUT_MESCH = zm_resize(OUT_MESCH,A->m,B->n);
    ...
    ****************************************************************/
	...
	{
	    if ( ! is_zero(A_v[i][k]) )
		__zmltadd__(OUT_MESCH->me[i],B_v[k],A_v[i][k],(int)p,Z_NOCONJ);
	    /**************************************************
	      B_row = B_v[k];	OUT_MESCH_row = OUT_MESCH->me[i];
	      for ( j=0; j<p; j++ )
	      (*OUT_MESCH_row++) += tmp*(*B_row++);
	    **************************************************/
	}
...
/* zmma_mlt -- matrix-matrix adjoint multiplication
   -- A.B* is returned, and stored in OUT_MESCH */
ZMAT	*zmma_mlt(ZMAT *A,ZMAT *B,ZMAT *OUT_MESCH)
{
    int	i, j, limit;
    ...
    if ( ! OUT_MESCH || OUT_MESCH->m != A->m || OUT_MESCH->n != B->m )
	OUT_MESCH = zm_resize(OUT_MESCH,A->m,B->m);
    
    limit = A->n;
    for ( i = 0; i < A->m; i++ )
	for ( j = 0; j < B->m; j++ )
	{
	    OUT_MESCH->me[i][j] = __zip__(B->me[j],A->me[i],(int)limit,Z_CONJ);
	    /**************************************************
	      sum = 0.0;
	      A_row = A->me[i];
	      B_row = B->me[j];
...
/* zmam_mlt -- matrix adjoint-matrix multiplication
   -- A*.B is returned, result stored in OUT_MESCH */
ZMAT	*zmam_mlt(ZMAT *A,ZMAT *B,ZMAT *OUT_MESCH)
{
    int	i, k, limit;
    /* complex	*B_row, *OUT_MESCH_row, multiplier; */
    complex	tmp;
    ...
    if ( ! OUT_MESCH || OUT_MESCH->m != A->n || OUT_MESCH->n != B->n )
	OUT_MESCH = zm_resize(OUT_MESCH,A->n,B->n);
	...
	for ( i = 0; i < A->n; i++ )
	{
	    ...
	    tmp.im = - A->me[k][i].im;
	    if ( ! is_zero(tmp) )
		__zmltadd__(OUT_MESCH->me[i],B->me[k],tmp,(int)limit,Z_NOCONJ);
	}
